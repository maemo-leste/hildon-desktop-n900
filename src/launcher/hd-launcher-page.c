...
G_DEFINE_TYPE_WITH_CODE(HdLauncherPage, hd_launcher_page, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(HdLauncherPage));
...
static void hd_launcher_page_tile_clicked(HdLauncherTile * tile, gpointer data);
static void hd_launcher_page_new_frame(ClutterTimeline * timeline, gint frame_num, gpointer data);
static void hd_launcher_page_transition_end(ClutterTimeline * timeline, gpointer data);

static void hd_launcher_page_class_init(HdLauncherPageClass * klass)
{
	...
	launcher_page_signals[TILE_CLICKED] =
	    g_signal_new(I_("tile-clicked"),
			 ...
			 G_SIGNAL_RUN_LAST,
			 0, NULL, NULL, g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, HD_TYPE_LAUNCHER_TILE);
}

static void hd_launcher_page_init(HdLauncherPage * page)
{
	page->priv = HD_LAUNCHER_PAGE_GET_PRIVATE(page);

	clutter_actor_set_size(CLUTTER_ACTOR(page), HD_LAUNCHER_PAGE_WIDTH, HD_LAUNCHER_PAGE_HEIGHT);
	clutter_actor_set_reactive(CLUTTER_ACTOR(page), FALSE);
	g_signal_connect(page, "show", G_CALLBACK(hd_launcher_page_show), 0);
}

static gboolean captured_event_cb(TidyFingerScroll * scroll, ClutterEvent * event, HdLauncherPage * page)
{
...
static gboolean motion_event_cb(TidyFingerScroll * scroll, ClutterMotionEvent * event, HdLauncherPage * page)
{
	HdLauncherPagePrivate *priv;
	gint dx, dy;
...
static void hd_launcher_page_constructed(GObject * object)
{
	...
	hd_gtk_style_get_text_color(HD_GTK_BUTTON_SINGLETON, GTK_STATE_NORMAL, &text_color);
	font_string = hd_gtk_style_get_font_string(HD_GTK_BUTTON_SINGLETON);
	priv->empty_label = clutter_label_new_full(font_string, _("tana_li_of_noapps"), &text_color);
	clutter_label_set_line_wrap(CLUTTER_LABEL(priv->empty_label), TRUE);
	clutter_label_set_ellipsize(CLUTTER_LABEL(priv->empty_label), PANGO_ELLIPSIZE_NONE);
	clutter_label_set_alignment(CLUTTER_LABEL(priv->empty_label), PANGO_ALIGN_CENTER);
	clutter_label_set_line_wrap_mode(CLUTTER_LABEL(priv->empty_label), PANGO_WRAP_WORD);
	...
	y1 = ((page_height - HD_LAUNCHER_PAGE_YMARGIN - label_height) / 2) + HD_LAUNCHER_PAGE_YMARGIN;
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(page), priv->scroller);
	...
	g_signal_connect(priv->scroller, "captured-event", G_CALLBACK(captured_event_cb), object);
	g_signal_connect(priv->scroller, "motion-event", G_CALLBACK(motion_event_cb), object);
}

ClutterActor *hd_launcher_page_new(void)
{
	return g_object_new(HD_TYPE_LAUNCHER_PAGE, NULL);
}
...
void hd_launcher_page_add_tile(HdLauncherPage * page, HdLauncherTile * tile)
{
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(priv->grid), CLUTTER_ACTOR(tile));

	g_signal_connect(tile, "clicked", G_CALLBACK(hd_launcher_page_tile_clicked), page);
}

static void hd_launcher_page_tile_clicked(HdLauncherTile * tile, gpointer data)
{
	g_signal_emit(HD_LAUNCHER_PAGE(data), launcher_page_signals[TILE_CLICKED], 0, tile);
}
...
void hd_launcher_page_transition(HdLauncherPage * page, HdLauncherPageTransition trans_type)
{
	...
	if (!CLUTTER_ACTOR_IS_VISIBLE(page) && trans_type == HD_LAUNCHER_PAGE_TRANSITION_OUT)
		return;
	...
	priv->transition =
	    clutter_timeline_new_for_duration(hd_transition_get_int
					      (hd_launcher_page_get_transition_string(priv->transition_type),
					       "duration", 500 /* default value */ ));
	g_signal_connect(priv->transition, "new-frame", G_CALLBACK(hd_launcher_page_new_frame), page);
	g_signal_connect(priv->transition, "completed", G_CALLBACK(hd_launcher_page_transition_end), page);
...
static void hd_launcher_page_new_frame(ClutterTimeline * timeline, gint frame_num, gpointer data)
{
	...
	hd_launcher_grid_transition(HD_LAUNCHER_GRID(priv->grid), page, priv->transition_type, amt);
...
static void hd_launcher_page_transition_end(ClutterTimeline * timeline, gpointer data)
{
	...
	hd_launcher_grid_transition(HD_LAUNCHER_GRID(priv->grid), page, priv->transition_type, 1.0f);
