...
G_DEFINE_TYPE_WITH_CODE(HdLauncherApp, hd_launcher_app, HD_TYPE_LAUNCHER_ITEM, G_ADD_PRIVATE(HdLauncherApp));

gboolean hd_launcher_app_parse_keyfile(HdLauncherItem * item, GKeyFile * key_file, GError ** error);

static void hd_launcher_app_finalize(GObject * gobject)
{
...
static void hd_launcher_app_class_init(HdLauncherAppClass * klass)
{
...
static void hd_launcher_app_init(HdLauncherApp * launcher)
{
	launcher->priv = HD_LAUNCHER_APP_GET_PRIVATE(launcher);
}

static HdLauncherAppPrestartMode hd_launcher_app_parse_prestart_mode(gchar * mode)
{
...
static gchar *hd_launcher_app_parse_service_name(gchar * name)
{
	if (name == NULL)
		return name;
...
gboolean hd_launcher_app_parse_keyfile(HdLauncherItem * item, GKeyFile * key_file, GError ** error)
{
	...
	priv->loading_image = g_key_file_get_string(key_file,
						    HD_DESKTOP_ENTRY_GROUP, HD_DESKTOP_ENTRY_LOADING_IMAGE, NULL);

	priv->switcher_icon = g_key_file_get_string(key_file,
						    HD_DESKTOP_ENTRY_GROUP, HD_DESKTOP_ENTRY_SWITCHER_ICON, NULL);

	priv->prestart_mode =
	    hd_launcher_app_parse_prestart_mode(g_key_file_get_string(key_file,
								      HD_DESKTOP_ENTRY_GROUP,
								      HD_DESKTOP_ENTRY_PRESTART_MODE, NULL));

	priv->wm_class = g_key_file_get_string(key_file, HD_DESKTOP_ENTRY_GROUP, HD_DESKTOP_ENTRY_WM_CLASS, NULL);

	priv->priority = g_key_file_get_integer(key_file, HD_DESKTOP_ENTRY_GROUP, HD_DESKTOP_ENTRY_PRIORITY, NULL);

	priv->ignore_lowmem = g_key_file_get_boolean(key_file,
						     HD_DESKTOP_ENTRY_GROUP, HD_DESKTOP_ENTRY_IGNORE_LOWMEM, NULL);

	priv->ignore_load = g_key_file_get_boolean(key_file,
						   HD_DESKTOP_ENTRY_GROUP, HD_DESKTOP_ENTRY_IGNORE_LOAD, NULL);
...
gboolean hd_launcher_app_match_window(HdLauncherApp * app, const gchar * res_name, const gchar * res_class)
{
	...
	if (res_class && priv->wm_class && g_strcmp0(priv->wm_class, res_class) == 0)
		return TRUE;

	/* Now try the app's id with the class name, ignoring case. */
	if (res_class &&
	    g_ascii_strncasecmp(res_class, hd_launcher_item_get_id(HD_LAUNCHER_ITEM(app)), strlen(res_class)) == 0)
		return TRUE;

	/* Try the executable as a last resource. */
	if (res_name && g_strcmp0(res_name, priv->exec) == 0)
		return TRUE;
