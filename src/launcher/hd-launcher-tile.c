...
static void hd_launcher_tile_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec);
static void hd_launcher_tile_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec);
/* ClutterActor */
...
static void hd_launcher_on_glow_frame(ClutterTimeline * timeline, gint frame_num, ClutterActor * actor);

static void hd_launcher_tile_allocate(ClutterActor * self,
				      const ClutterActorBox * box, gboolean absolute_origin_changed);

G_DEFINE_TYPE_WITH_CODE(HdLauncherTile, hd_launcher_tile, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(HdLauncherTile));

static void hd_launcher_tile_class_init(HdLauncherTileClass * klass)
{
	...
	pspec = g_param_spec_string("icon-name",
				    ...
				    HD_LAUNCHER_DEFAULT_ICON, G_PARAM_CONSTRUCT | HD_PARAM_READWRITE);
	g_object_class_install_property(gobject_class, PROP_LAUNCHER_TILE_ICON_NAME, pspec);
	pspec = g_param_spec_string("text",
				    "Text", "Text to display", "Unknown", G_PARAM_CONSTRUCT | HD_PARAM_READWRITE);
...
static void hd_launcher_tile_init(HdLauncherTile * tile)
{
	HdLauncherTilePrivate *priv = tile->priv = HD_LAUNCHER_TILE_GET_PRIVATE(tile);

	clutter_actor_set_name(CLUTTER_ACTOR(tile), "HdLauncherTile");
	clutter_actor_set_size(CLUTTER_ACTOR(tile), HD_LAUNCHER_TILE_WIDTH, HD_LAUNCHER_TILE_HEIGHT);
	...
	clutter_actor_set_size(priv->click_area, HD_LAUNCHER_TILE_WIDTH, HD_LAUNCHER_TILE_HEIGHT);
	...
	g_signal_connect(tile->priv->glow_timeline, "new-frame", G_CALLBACK(hd_launcher_on_glow_frame), tile);
}

HdLauncherTile *hd_launcher_tile_new(const gchar * icon_name, const gchar * text)
{
	return g_object_new(HD_TYPE_LAUNCHER_TILE, "icon-name", icon_name, "text", text, NULL);
}
...
static void hd_launcher_tile_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec)
{
	...
	case PROP_LAUNCHER_TILE_ICON_NAME:
		g_value_set_string(value, hd_launcher_tile_get_icon_name(tile));
		break;

	case PROP_LAUNCHER_TILE_TEXT:
		g_value_set_string(value, hd_launcher_tile_get_text(tile));
		break;
...
void hd_launcher_tile_set_icon_name(HdLauncherTile * tile, const gchar * icon_name)
{
	...
	if (g_file_test(fname, G_FILE_TEST_EXISTS)
		...
		info = gtk_icon_theme_lookup_icon(icon_theme, priv->icon_name,
						  HD_LAUNCHER_TILE_ICON_REAL_SIZE, GTK_ICON_LOOKUP_NO_SVG);
	...
	if (pixbuf) {
		...
		GdkPixbuf *pixbufb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, w + 2, h + 2);

		gdk_pixbuf_fill(pixbufb, 0);
		gdk_pixbuf_copy_area(pixbuf, 0, 0, w, h, pixbufb, 1, 1);
	...
	clutter_actor_set_size(priv->icon, HD_LAUNCHER_TILE_ICON_SIZE, HD_LAUNCHER_TILE_ICON_SIZE);
	clutter_actor_set_position(priv->icon, (HD_LAUNCHER_TILE_WIDTH - HD_LAUNCHER_TILE_ICON_SIZE) / 2, 0);
	...
	clutter_actor_set_size(CLUTTER_ACTOR(priv->icon_glow), HD_LAUNCHER_TILE_GLOW_SIZE, HD_LAUNCHER_TILE_GLOW_SIZE);
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(tile), CLUTTER_ACTOR(priv->icon_glow));
...
void hd_launcher_tile_set_text(HdLauncherTile * tile, const gchar * text)
{
	...
	clutter_label_set_line_wrap_mode(CLUTTER_LABEL(priv->label), PANGO_WRAP_CHAR);

	label_height = HD_LAUNCHER_TILE_HEIGHT - (64 + HILDON_MARGIN_HALF);

	clutter_actor_get_preferred_width(priv->label, CLUTTER_UNITS_FROM_DEVICE(label_height), NULL, &label_width);
	label_width_px = MIN(CLUTTER_UNITS_TO_DEVICE(label_width), HD_LAUNCHER_TILE_WIDTH);
	...
	if (CLUTTER_UNITS_TO_DEVICE(label_width) > HD_LAUNCHER_TILE_WIDTH)
		clutter_actor_set_clip(priv->label, 0, 0, HD_LAUNCHER_TILE_WIDTH, label_height);
}
...
static void hd_launcher_tile_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec)
{
	switch (prop_id) {
	case PROP_LAUNCHER_TILE_ICON_NAME:
		hd_launcher_tile_set_icon_name(HD_LAUNCHER_TILE(gobject), g_value_get_string(value));
		break;

	case PROP_LAUNCHER_TILE_TEXT:
		hd_launcher_tile_set_text(HD_LAUNCHER_TILE(gobject), g_value_get_string(value));
		break;
...
static void hd_launcher_on_glow_frame(ClutterTimeline * timeline, gint frame_num, ClutterActor * actor)
{
	...
	if (priv->icon_glow)
		tidy_highlight_set_amount(priv->icon_glow, priv->glow_amount * priv->glow_radius);
	else
		return;
...
static void hd_launcher_tile_set_glow(HdLauncherTile * tile, gboolean glow, gboolean hard)
{
	HdLauncherTilePrivate *priv = HD_LAUNCHER_TILE_GET_PRIVATE(tile);
	...
	if ((glow && priv->glow_amount == 1) || (!glow && priv->glow_amount == 0))
		return;
	...
	clutter_timeline_set_duration(priv->glow_timeline,
				      hd_transition_get_int("launcher_glow",
							    glow ? "duration_in" : "duration_out", 500));
	...
	clutter_timeline_advance(priv->glow_timeline, (int)(priv->glow_amount * n_frames));

	/* set our glow colour from the theme */
	glow_brightness = hd_transition_get_double("launcher_glow", "brightness", 1);
	hd_gtk_style_get_text_color(HD_GTK_BUTTON_SINGLETON, GTK_STATE_NORMAL, &glow_col);
...
static gboolean _hd_launcher_tile_long_timeout(gpointer data)
{
...
static gboolean hd_launcher_tile_button_press(ClutterActor * actor)
{
	...
	priv->press_timeout = g_timeout_add(HD_LAUNCHER_TILE_LONG_PRESS_DUR, _hd_launcher_tile_long_timeout, actor);
	return TRUE;
}
...
static void
hd_launcher_tile_allocate(ClutterActor * self, const ClutterActorBox * box, gboolean absolute_origin_changed)
{
	...
	CLUTTER_ACTOR_CLASS(hd_launcher_tile_parent_class)->allocate(self, box, absolute_origin_changed);
}
