...
G_DEFINE_TYPE_WITH_CODE(HdLauncherEditor, hd_launcher_editor, HILDON_TYPE_WINDOW, G_ADD_PRIVATE(HdLauncherEditor));

static void _hd_launcher_editor_load(HdLauncherEditor * editor)
{
	...
	while (entries) {
		...
		gtk_list_store_insert_with_values(GTK_LIST_STORE(priv->model),
						  ...
						  COL_DESKTOP_ID, hd_launcher_item_get_id(item), -1);

		entries = entries->next;
	}
}

static gboolean _hd_launcher_editor_ensure_dirs()
{
	gboolean ret = FALSE;
	gchar *dirname;

	dirname = g_build_filename(g_get_user_config_dir(), "menus", NULL);
...
static void _hd_launcher_editor_selection_changed(HdLauncherEditor * editor, GtkIconView * icon_view)
{
	HdLauncherEditorPrivate *priv = HD_LAUNCHER_EDITOR(editor)->priv;
...
static gboolean _hd_launcher_editor_pressed(HdLauncherEditor * editor, GdkEventButton * event, GtkIconView * icon_view)
{
	HdLauncherEditorPrivate *priv = HD_LAUNCHER_EDITOR(editor)->priv;
...
static gboolean _hd_launcher_editor_released(HdLauncherEditor * editor, GdkEventButton * event, GtkIconView * icon_view)
{
	HdLauncherEditorPrivate *priv = HD_LAUNCHER_EDITOR(editor)->priv;
	...
	if (moved_x >= DRAG_THRESHOLD || moved_y >= DRAG_THRESHOLD)
		return FALSE;

	clicked_path = gtk_icon_view_get_path_at_pos(icon_view, (gint) event->x, (gint) event->y);
	if (!clicked_path) {
	...
	if (gtk_icon_view_path_is_selected(icon_view, clicked_path)) {
		...
		if (priv->selected == NULL) {
			...
			if (comp) {
				...
				if (comp < 0)
					gtk_list_store_move_after(GTK_LIST_STORE(priv->model), &oldpos, &newpos);
				else
					gtk_list_store_move_before(GTK_LIST_STORE(priv->model), &oldpos, &newpos);
				priv->dirty = TRUE;
			}
...
static void _hd_launcher_editor_close(HdLauncherEditor * editor)
{
	g_signal_emit(editor, launcher_editor_signals[DONE], 0, FALSE);
}

static void _hd_launcher_editor_save(HdLauncherEditor * editor)
{
	HdLauncherEditorPrivate *priv = HD_LAUNCHER_EDITOR(editor)->priv;
	gchar *menu_filename;
	GString *menu;
	...
	if (!gtk_tree_model_get_iter_first(priv->model, &iter)) {
		g_warning("%s: App menu ordering model is empty", __FUNCTION__);
		return;
	}

	if (!_hd_launcher_editor_ensure_dirs()) {
		g_warning("%s: Couldn't create directory for applications menu", __FUNCTION__);
		return;
	}

	menu_filename = g_build_filename(g_get_user_config_dir(), "menus", HD_LAUNCHER_MENU_FILE, NULL);
	menu = g_string_new(NULL);
	g_string_append_printf(menu, HD_LAUNCHER_MENU_START, g_get_user_data_dir(), g_get_user_data_dir());
	g_string_append(menu, LAYOUT_START);

	do {
		gchar *name;
		gtk_tree_model_get(priv->model, &iter, COL_DESKTOP_ID, &name, -1);
	...
	g_string_append_printf(menu, HD_LAUNCHER_MENU_END, g_get_user_config_dir());
...
static void hd_launcher_editor_dispose(GObject * object)
{
...
static void hd_launcher_editor_constructed(GObject * object)
{
...
static void hd_launcher_editor_class_init(HdLauncherEditorClass * klass)
{
...
static void hd_launcher_editor_init(HdLauncherEditor * editor)
{
	...
	/* Create the model */
	priv->model = (GtkTreeModel *) gtk_list_store_new(NUM_COLS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING);

	/* and the icon view. */
	priv->icon_view = hildon_gtk_icon_view_new_with_model(HILDON_UI_MODE_EDIT, priv->model);

	renderer = gtk_cell_renderer_pixbuf_new();
	g_object_set(G_OBJECT(renderer), "xalign", 0.5, "yalign", 0.5, "width", 64, "height", 64, NULL);
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(priv->icon_view), renderer, FALSE);
	gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(priv->icon_view), renderer, "pixbuf", COL_ICON);
	...
	g_object_set(G_OBJECT(renderer),
		     ...
		     "wrap-width", 142, "wrap-mode", PANGO_WRAP_WORD_CHAR, "font", "Nokia Sans 15", NULL);
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(priv->icon_view), renderer, FALSE);
	gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(priv->icon_view), renderer, "text", COL_LABEL);

	if (STATE_IS_PORTRAIT(hd_render_manager_get_state())) {
		gtk_icon_view_set_columns(GTK_ICON_VIEW(priv->icon_view), 3);

		/* Force portrait mode */
		hildon_gtk_window_set_portrait_flags(GTK_WINDOW(editor), HILDON_PORTRAIT_MODE_REQUEST);
	} else
		gtk_icon_view_set_columns(GTK_ICON_VIEW(priv->icon_view), 5);

	gtk_icon_view_set_item_width(GTK_ICON_VIEW(priv->icon_view), 142);
	gtk_icon_view_set_column_spacing(GTK_ICON_VIEW(priv->icon_view), HILDON_MARGIN_DEFAULT);
	gtk_icon_view_set_row_spacing(GTK_ICON_VIEW(priv->icon_view), 40);
	gtk_icon_view_set_spacing(GTK_ICON_VIEW(priv->icon_view), HILDON_MARGIN_HALF);
	...
	g_signal_connect_swapped(priv->icon_view, "selection-changed",
				 G_CALLBACK(_hd_launcher_editor_selection_changed), editor);
	g_signal_connect_swapped(priv->icon_view, "button-press-event",
				 G_CALLBACK(_hd_launcher_editor_pressed), editor);
	g_signal_connect_swapped(priv->icon_view, "button-release-event",
				 G_CALLBACK(_hd_launcher_editor_released), editor);

	area = hildon_pannable_area_new();

	toolbar = hildon_edit_toolbar_new_with_text(gettext("tana_fi_reorder"),
						    dgettext("hildon-libs", "wdgt_bd_done"));

	g_signal_connect_swapped(toolbar, "button-clicked", G_CALLBACK(_hd_launcher_editor_save), editor);

	g_signal_connect_swapped(toolbar, "arrow-clicked", G_CALLBACK(_hd_launcher_editor_close), editor);

	hildon_window_set_edit_toolbar(HILDON_WINDOW(editor), HILDON_EDIT_TOOLBAR(toolbar));
...
GtkWidget *hd_launcher_editor_new(void)
{
	...
	editor = g_object_new(HD_TYPE_LAUNCHER_EDITOR, NULL);
...
void hd_launcher_editor_show(GtkWidget * window)
{
	...
	no_trans = gdk_x11_get_xatom_by_name_for_display(gdk_display, "_HILDON_WM_ACTION_NO_TRANSITIONS");

	XChangeProperty(display, xwindow, no_trans, XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&one, 1);
	XSync(display, FALSE);
...
void hd_launcher_editor_unselect_all(HdLauncherEditor * editor)
{
...
static gboolean
_hd_launcher_editor_select_label(GtkTreeModel * model, GtkTreePath * path, GtkTreeIter * iter, gpointer data)
{
	...
	if (!g_strcmp0(desktop_id, priv->select_label)) {
		...
		gtk_icon_view_scroll_to_path(GTK_ICON_VIEW(priv->icon_view), path, TRUE, priv->y_align, priv->x_align);
		return TRUE;
	}
...
void hd_launcher_editor_select(HdLauncherEditor * editor, const gchar * desktop_id, gfloat x_align, gfloat y_align)
{
	HdLauncherEditorPrivate *priv = editor->priv;
	...
	gtk_tree_model_foreach(priv->model, _hd_launcher_editor_select_label, editor);
