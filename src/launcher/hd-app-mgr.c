...
G_DEFINE_TYPE_WITH_CODE(HdAppMgr, hd_app_mgr, G_TYPE_OBJECT, G_ADD_PRIVATE(HdAppMgr));
...
static void hd_app_mgr_populate_tree_finished(HdLauncherTree * tree, gpointer data);
...
static gboolean hd_app_mgr_service_top(const gchar * service, const gchar * param);

void hd_app_mgr_prestartable(HdRunningApp * app, gboolean prestartable);

static void hd_app_mgr_add_to_queue(HdAppMgrQueue queue, HdRunningApp * app);
static void hd_app_mgr_remove_from_queue(HdAppMgrQueue queue, HdRunningApp * app);
static void hd_app_mgr_move_queue(HdAppMgrQueue queue_from, HdAppMgrQueue queue_to, HdRunningApp * app);

static size_t hd_app_mgr_read_lowmem(const gchar * filename);
static HdAppMgrPrestartMode
hd_app_mgr_setup_prestart(size_t low_pages, size_t nr_decay_pages, size_t *prestart_required_pages);
static void hd_app_mgr_setup_launch(size_t high_pages, size_t nr_decay_pages, size_t *launch_required_pages);
...
static void hd_app_mgr_hdrm_state_change(gpointer hdrm, GParamSpec * pspec, HdAppMgrPrivate * priv);
...
static void hd_app_mgr_dbus_name_owner_changed(DBusGProxy * proxy,
					       const char *name,
					       const char *old_owner, const char *new_owner, gpointer data);
static DBusHandlerResult hd_app_mgr_dbus_app_died(DBusConnection * conn, DBusMessage * msg, void *data);
static DBusHandlerResult hd_app_mgr_dbus_signal_handler(DBusConnection * conn, DBusMessage * msg, void *data);
static void hd_app_mgr_gconf_value_changed(GConfClient * client, guint cnxn_id, GConfEntry * entry, gpointer user_data);
...
{
	if (G_UNLIKELY(!the_app_mgr)) {	/* "Protect" against reentrancy. */
...
static void hd_app_mgr_class_init(HdAppMgrClass * klass)
{
	...
	app_mgr_signals[APP_LAUNCHED] =
	    g_signal_new(I_("application-launched"),
			 HD_TYPE_APP_MGR,
			 G_SIGNAL_RUN_FIRST,
			 0, NULL, NULL, g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, HD_TYPE_LAUNCHER_APP);
	...
	/* Bind D-Bus info. */
	dbus_g_object_type_install_info(HD_TYPE_APP_MGR, &dbus_glib_hd_app_mgr_object_info);
}

static gchar *_hd_app_mgr_build_match(const gchar * interface, const gchar * member)
{
	gchar *arg;
	if (member)
		arg = g_strdup_printf("type='signal', interface='%s', member='%s'", interface, member);
	else
 ...
 * name.
 */
static void hd_app_mgr_dbus_add_signal_match(DBusConnection * conn, const gchar * interface, const gchar * member)
{
...
static void hd_app_mgr_dbus_remove_signal_match(DBusConnection * conn, const gchar * interface, const gchar * member)
{
	gchar *arg = _hd_app_mgr_build_match(interface, member);
...
static void hd_app_mgr_init(HdAppMgr * self)
{
	...
	g_signal_connect(priv->tree, "finished", G_CALLBACK(hd_app_mgr_populate_tree_finished), self);
	...
	if (priv->gconf_client) {
		priv->slide_closed = !gconf_client_get_bool(priv->gconf_client, GCONF_SLIDE_OPEN_KEY, NULL);
		gconf_client_add_dir(priv->gconf_client, GCONF_SLIDE_OPEN_DIR, GCONF_CLIENT_PRELOAD_NONE, NULL);
		gconf_client_notify_add(priv->gconf_client, GCONF_SLIDE_OPEN_KEY,
					hd_app_mgr_gconf_value_changed, (gpointer) self, NULL, NULL);
		...
		gconf_client_notify_add(priv->gconf_client, GCONF_KEY_ACTIONS_DIR,
					hd_app_mgr_gconf_value_changed, (gpointer) self, NULL, NULL);
		gconf_client_add_dir(priv->gconf_client, GCONF_VIEWS_CURRENT_DIR, GCONF_CLIENT_PRELOAD_NONE, NULL);
		...
		if (gconf_client_dir_exists(priv->gconf_client, GCONF_KEY_ACTIONS_DIR, NULL)) {
			conf_enable_home_contacts_phone = gconf_client_get_bool(priv->gconf_client,
										GCONF_KEY_ACTIONS_DIR
										"/home_contacts_phone", NULL);
			conf_enable_launcher_navigator_accel =
			    gconf_client_get_bool(priv->gconf_client, GCONF_KEY_ACTIONS_DIR "/launcher_navigator_accel",
						  NULL);
			conf_enable_dbus_launcher_navigator =
			    gconf_client_get_bool(priv->gconf_client, GCONF_KEY_ACTIONS_DIR "/dbus_launcher_navigator",
						  NULL);
			conf_default_launcher_positions =
			    gconf_client_get_bool(priv->gconf_client,
						  GCONF_KEY_ACTIONS_DIR "/default_launcher_positions", NULL);
			conf_disable_edit =
			    gconf_client_get_bool(priv->gconf_client, GCONF_KEY_ACTIONS_DIR "/disable_edit", NULL);
		}
		priv->disable_callui = gconf_client_get_bool(priv->gconf_client, GCONF_DISABLE_CALLUI_KEY, NULL);

		gconf_client_notify_add(priv->gconf_client, GCONF_UI_CAN_ROTATE_KEY,
					hd_app_mgr_gconf_value_changed, (gpointer) self, NULL, NULL);
		priv->ui_can_rotate = gconf_client_get_bool(priv->gconf_client, GCONF_UI_CAN_ROTATE_KEY, NULL);
	...
	priv->prestart_mode = hd_app_mgr_setup_prestart(priv->notify_low_pages,
							priv->nr_decay_pages, &priv->prestart_required_pages);
	hd_app_mgr_setup_launch(priv->notify_high_pages, priv->nr_decay_pages, &priv->launch_required_pages);
	...
	if (connection) {
		...
		if (priv->dbus_proxy) {
			...
			dbus_g_proxy_connect_signal(priv->dbus_proxy,
						    DBUS_NAMEOWNERCHANGED_SIGNAL_NAME,
						    (GCallback) hd_app_mgr_dbus_name_owner_changed, NULL, NULL);

			/* Serve the AppMgr interface. */
			guint result;
			if (!org_freedesktop_DBus_request_name(priv->dbus_proxy,
							       HD_APP_MGR_DBUS_NAME,
							       DBUS_NAME_FLAG_DO_NOT_QUEUE, &result, NULL)) {
				g_warning("%s: Could not register name.\n", __FUNCTION__);
			} else {
				dbus_g_connection_register_g_object(connection, HD_APP_MGR_DBUS_PATH, G_OBJECT(self));
			}
		} else
			g_warning("%s: Failed to proxy session dbus.", __FUNCTION__);

		/* Connect to the maemo launcher dbus interface. */
		hd_app_mgr_dbus_add_signal_match(dbus_g_connection_get_connection(connection),
						 MAEMO_LAUNCHER_IFACE, MAEMO_LAUNCHER_APP_DIED_SIGNAL_NAME);
		dbus_connection_add_filter(dbus_g_connection_get_connection(connection),
					   hd_app_mgr_dbus_app_died, self, NULL);
	} else
	...
	if (sys_conn) {
		hd_app_mgr_dbus_add_signal_match(sys_conn, LOWMEM_ON_SIGNAL_INTERFACE, LOWMEM_ON_SIGNAL_NAME);
		hd_app_mgr_dbus_add_signal_match(sys_conn, LOWMEM_OFF_SIGNAL_INTERFACE, LOWMEM_OFF_SIGNAL_NAME);
		hd_app_mgr_dbus_add_signal_match(sys_conn, BGKILL_ON_SIGNAL_INTERFACE, BGKILL_ON_SIGNAL_NAME);
		hd_app_mgr_dbus_add_signal_match(sys_conn, BGKILL_OFF_SIGNAL_INTERFACE, BGKILL_OFF_SIGNAL_NAME);
		hd_app_mgr_dbus_add_signal_match(sys_conn, INIT_DONE_SIGNAL_INTERFACE, INIT_DONE_SIGNAL_NAME);
		dbus_connection_add_filter(sys_conn, hd_app_mgr_dbus_signal_handler, self, NULL);
	} else
	 ...
	 */
	g_timeout_add_seconds(INIT_DONE_TIMEOUT, (GSourceFunc) hd_app_mgr_init_done_timeout, self);
}
...
void hd_app_mgr_set_render_manager(GObject * rendermgr)
{
	...
	g_signal_connect(rendermgr, "notify::state", G_CALLBACK(hd_app_mgr_hdrm_state_change), priv);
}

static void _hd_app_mgr_kill_prestarted(HdRunningApp * app, gpointer user_data)
{
	hd_app_mgr_kill(app);
}

static void hd_app_mgr_kill_all_prestarted()
{
	GQueue *prestarted;
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());

	prestarted = g_queue_copy(priv->queues[QUEUE_PRESTARTED]);
	g_queue_foreach(prestarted, (GFunc) _hd_app_mgr_kill_prestarted, NULL);
	g_queue_free(prestarted);
}

/* Called when exiting main() to close all prestarted apps. */
void hd_app_mgr_stop()
{
	if (!the_app_mgr)
		return;
...
static gint _hd_app_mgr_compare_app_priority(gconstpointer a, gconstpointer b, gpointer user_data)
{
...
static void hd_app_mgr_add_to_queue(HdAppMgrQueue queue, HdRunningApp * app)
{
	...
	g_queue_insert_sorted(priv->queues[queue], g_object_ref(app), _hd_app_mgr_compare_app_priority, NULL);
}
...
static void hd_app_mgr_move_queue(HdAppMgrQueue queue_from, HdAppMgrQueue queue_to, HdRunningApp * app)
{
	...
	if (link) {
		...
		g_queue_insert_sorted(priv->queues[queue_to], app, _hd_app_mgr_compare_app_priority, NULL);
...
void hd_app_mgr_hibernatable(HdRunningApp * app, gboolean hibernatable)
{
	...
	g_debug("%s: Making app %s %s hibernatable", __FUNCTION__,
		hd_launcher_app_get_service(launcher), hibernatable ? "really" : "not");
...
static gint _hd_app_mgr_compare_app_launcher(HdRunningApp * a, HdLauncherApp * b)
{
 ...
 */
gboolean hd_app_mgr_launch(HdLauncherApp * launcher)
{
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());
	...
	link = g_list_find_custom(priv->running_apps, launcher, (GCompareFunc) _hd_app_mgr_compare_app_launcher);
	if (link)
...
static gdouble hd_app_mgr_timeout_backoff_factor()
{
	return hd_transition_get_double("loading_timeout", "load_average_factor", 0.0);
}
 ...
 * cannot be found.
 */
static gdouble hd_app_mgr_system_load_average(void)
{
...
gboolean hd_app_mgr_activate(HdRunningApp * app)
{
	...
	case HD_APP_STATE_SHOWN:
		result = !STATE_IS_APP(hd_render_manager_get_state())
		    || app != hd_comp_mgr_client_get_app(hd_comp_mgr_get_current_client(hd_comp_mgr_get()))
	...
	case LAUNCH_OK:
		if (timer) {
			...
			time_t now;
			gint timeout = (gint) (hd_app_mgr_timeout_backoff_factor() * hd_app_mgr_system_load_average());
			...
			g_timeout_add_seconds(timeout, (GSourceFunc) hd_app_mgr_loading_timeout, g_object_ref(app));
		}
		break;
	case LAUNCH_FAILED:
...
static void _hd_app_mgr_child_exit(GPid pid, gint status, HdRunningApp * app)
{
...
HdAppMgrLaunchResult hd_app_mgr_start(HdRunningApp * app)
{
	...
	if (service) {
		...
		if (exec) {
			...
			if (result) {
				...
				/* Watch the child. */
				g_child_watch_add(pid, (GChildWatchFunc) _hd_app_mgr_child_exit, g_object_ref(app));
			}
		}
	}
	...
	if (result) {
		...
		g_signal_emit(hd_app_mgr_get(), app_mgr_signals[APP_LAUNCHED], 0, launcher, NULL);
	}
...
HdAppMgrLaunchResult hd_app_mgr_relaunch(HdRunningApp * app)
{
	...
	HdLauncherApp *launcher = hd_running_app_get_launcher_app(app);
	if (launcher)
		g_signal_emit(hd_app_mgr_get(), app_mgr_signals[APP_RELAUNCHED], 0, launcher, NULL);
...
static gboolean hd_app_mgr_loading_timeout(HdRunningApp * app)
{
	...
	if ((state == HD_APP_STATE_LOADING || state == HD_APP_STATE_WAKING) &&
		...
		if (hd_running_app_get_service(app) != NULL) {
			g_signal_emit(hd_app_mgr_get(), app_mgr_signals[APP_LOADING_FAIL], 0, launcher, NULL);
		}
	}
...
void hd_app_mgr_app_opened(HdRunningApp * app)
{
	 ...
	 */
	if (launcher)
		g_signal_emit(hd_app_mgr_get(), app_mgr_signals[APP_SHOWN], 0, launcher, NULL);
...
void hd_app_mgr_app_closed(HdRunningApp * app)
{
	...
	if (launcher && hd_launcher_app_get_prestart_mode(launcher) == HD_APP_PRESTART_ALWAYS) {
...
static void hd_app_mgr_populate_tree_finished(HdLauncherTree * tree, gpointer data)
{
	...
	for (; apps; apps = apps->next) {
		...
		new = HD_LAUNCHER_APP(hd_launcher_tree_find_item(tree, hd_running_app_get_id(app)));
	...
	for (; items; items = items->next) {
		...
		if (hd_launcher_item_get_item_type(HD_LAUNCHER_ITEM(items->data)) != HD_APPLICATION_LAUNCHER)
			continue;
...
static void _hd_app_mgr_prestart_cb(DBusGProxy * proxy, guint result, GError * error, gpointer data)
{
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());
	...
	if (error || !result) {
		g_warning("%s: Couldn't prestart service %s, error: %s\n",
			  __FUNCTION__, hd_running_app_get_service(app), error ? error->message : "no result");
...
gboolean hd_app_mgr_prestart(HdRunningApp * app)
{
	...
	org_freedesktop_DBus_start_service_by_name_async(priv->dbus_proxy,
							 service, 0, _hd_app_mgr_prestart_cb, (gpointer) app);
...
static void _hd_app_mgr_child_setup(gpointer user_data)
{
	int priority;
	int fd;
...
gboolean hd_app_mgr_execute(const gchar * exec, GPid * pid, gboolean auto_reap)
{
	...
	if (!exec_cmd || !g_shell_parse_argv(exec_cmd, &argc, &argv, NULL)) {
		g_free(exec_cmd);
		if (argv)
			g_strfreev(argv);
	...
	res = g_spawn_async(NULL,
			    argv, NULL,
			    auto_reap ? 0 : G_SPAWN_DO_NOT_REAP_CHILD, _hd_app_mgr_child_setup, NULL, pid, NULL);
	g_free(exec_cmd);
...
static gboolean hd_app_mgr_service_top(const gchar * service, const gchar * param)
{
	...
	g_debug("%s: service: %s, param: %s", __FUNCTION__, service, param ? param : "null");
	dbus_message_unref(msg);
	return TRUE;
}
 ...
 */
static gboolean hd_app_mgr_check_loadavg(void)
{
	gdouble load = hd_app_mgr_system_load_average();

	return (load >= 0.0 && load <= LOADAVG_MAX);
}
...
hd_app_mgr_setup_prestart(size_t low_pages, size_t nr_decay_pages, size_t *prestart_required_pages)
{
	...
	*prestart_required_pages = NSIZE;
	...
	if (!prestart_env || !*prestart_env || !g_strcmp0(prestart_env, "no") || !g_strcmp0(prestart_env, "false")) {
		...
		*prestart_required_pages = NSIZE;
		...
		if (reserved == 0)
			*prestart_required_pages = low_pages + hd_app_mgr_read_lowmem(LOWMEM_PROC_NR_DECAY);
		else
...
static void hd_app_mgr_state_check(void)
{
	...
	g_timeout_add_seconds(STATE_CHECK_INTERVAL, hd_app_mgr_state_check_loop, NULL);
}
...
static gboolean hd_app_mgr_state_check_loop(gpointer data)
{
	 ...
	 */
		 ...
		 !priv->prestarting_stopped && !g_queue_is_empty(priv->queues[QUEUE_PRESTARTABLE])
	    ) {
...
hd_app_mgr_dbus_name_owner_changed(DBusGProxy * proxy,
				   const char *name, const char *old_owner, const char *new_owner, gpointer data)
{
	GList *apps;
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());
	...
	while (apps) {
		...
		if (!g_strcmp0(name, hd_running_app_get_service(app))) {
			if (!new_owner[0]) {	/* Disconnection */
				g_debug("%s: App %s has fallen\n", __FUNCTION__, hd_running_app_get_id(app));
...
static gint _hd_app_mgr_compare_launcher_exec(HdLauncherItem * item, gchar * filename)
{
	HdLauncherApp *launcher;
...
static DBusHandlerResult hd_app_mgr_dbus_app_died(DBusConnection * conn, DBusMessage * msg, void *data)
{
	...
	if (!dbus_message_is_signal(msg, MAEMO_LAUNCHER_IFACE, MAEMO_LAUNCHER_APP_DIED_SIGNAL_NAME))
	...
	dbus_message_get_args(msg, &err,
			      ...
			      DBUS_TYPE_INT32, &pid, DBUS_TYPE_INT32, &status, DBUS_TYPE_INVALID);
	...
	link = g_list_find_custom(hd_launcher_tree_get_items(priv->tree),
				  (gconstpointer) filename, (GCompareFunc) _hd_app_mgr_compare_launcher_exec);

	/* NOTE: Should we report crashes of app we don't know about? */
	g_debug("%s: app: %s, filename: %s", __FUNCTION__,
		link ? hd_launcher_item_get_id(HD_LAUNCHER_ITEM(link->data)) : "<unknown>", filename);

	if (link) {
		launcher = link->data;
		g_signal_emit(hd_app_mgr_get(), app_mgr_signals[APP_CRASHED], 0, launcher, NULL);
	}
...
gboolean hd_app_mgr_dbus_launch_app(HdAppMgr * self, const gchar * id)
{
	...
	if (!id || !strlen(id)) {	/* If no ID was specified, all we want to do is trigger the
...
static gboolean _hd_app_mgr_should_show_callui()
{
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());
	...
	if (STATE_SHOW_CALLUI(hd_render_manager_get_state()) &&
	    ...
	    priv->slide_closed && !priv->disable_callui && !hd_wm_has_modal_blockers(hd_mb_wm)) {
		return TRUE;
	}
...
{
	if (_hd_app_mgr_should_show_callui()) {
		g_timeout_add_seconds(CALLUI_PORTRAIT_TIMEOUT, (GSourceFunc) hd_app_mgr_show_callui_cb, NULL);
		return TRUE;
	}
	return FALSE;
}
...
static gboolean _hd_app_mgr_dbus_check_value(DBusMessage * msg, const gchar * value)
{
	DBusError err;
	gchar *arg;
...
static void hd_app_mgr_update_portraitness(HdAppMgr * self)
{
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(self);
	...
	if (!hmgr)
		...
		return;
	hd_comp_mgr_set_pip_flags(hmgr, priv->accel_enabled, priv->portrait && priv->slide_closed);
...
static DBusHandlerResult hd_app_mgr_dbus_signal_handler(DBusConnection * conn, DBusMessage * msg, void *data)
{
	...
	if (dbus_message_is_signal(msg, LOWMEM_ON_SIGNAL_INTERFACE, LOWMEM_ON_SIGNAL_NAME))
		priv->lowmem = TRUE;
	else if (dbus_message_is_signal(msg, LOWMEM_OFF_SIGNAL_INTERFACE, LOWMEM_OFF_SIGNAL_NAME))
		priv->lowmem = FALSE;
	else if (dbus_message_is_signal(msg, BGKILL_ON_SIGNAL_INTERFACE, BGKILL_ON_SIGNAL_NAME))
		priv->bg_killing = TRUE;
	else if (dbus_message_is_signal(msg, BGKILL_OFF_SIGNAL_INTERFACE, BGKILL_OFF_SIGNAL_NAME))
		priv->bg_killing = FALSE;
	else if (dbus_message_is_signal(msg, INIT_DONE_SIGNAL_INTERFACE, INIT_DONE_SIGNAL_NAME))
		priv->init_done = TRUE;
	else {
		/* Check for showing CallUI flags. */
		changed = FALSE;
		if (dbus_message_is_signal(msg, MCE_SIGNAL_IF, MCE_TKLOCK_MODE_SIG)) {
			priv->unlocked = _hd_app_mgr_dbus_check_value(msg, MCE_DEVICE_UNLOCKED);
			...
			} else if (hd_app_mgr_ui_can_rotate() && STATE_IS_LAUNCHER(hd_render_manager_get_state())) {
...
void hd_app_mgr_mce_activate_accel_if_needed(gboolean update_portraitness)
{
	...
	gboolean activate = ((!priv->disable_callui && STATE_SHOW_CALLUI(state)) ||
			     (hd_app_mgr_ui_can_rotate() &&
			      (STATE_IS_TASK_NAV(state) ||
			       ...
			       STATE_IS_HOME(state) || STATE_IS_EDIT_MODE(state) || STATE_IS_APP(state))
			     )
	    );
	...
	if (activate) {
		hd_app_mgr_dbus_add_signal_match(conn, MCE_SIGNAL_IF, MCE_TKLOCK_MODE_SIG);
		hd_app_mgr_dbus_add_signal_match(conn, MCE_SIGNAL_IF, MCE_DEVICE_ORIENTATION_SIG);
	} else {
		hd_app_mgr_dbus_remove_signal_match(conn, MCE_SIGNAL_IF, MCE_TKLOCK_MODE_SIG);
		hd_app_mgr_dbus_remove_signal_match(conn, MCE_SIGNAL_IF, MCE_DEVICE_ORIENTATION_SIG);
	}

	msg = dbus_message_new_method_call(MCE_SERVICE,
					   MCE_REQUEST_PATH,
					   MCE_REQUEST_IF,
					   activate ? MCE_ACCELEROMETER_ENABLE_REQ : MCE_ACCELEROMETER_DISABLE_REQ);
	if (!msg) {
		...
		if ((reply = dbus_connection_send_with_reply_and_block(conn, msg, -1, NULL)) != NULL) {
			if (STATE_IS_PORTRAIT(hd_render_manager_get_state()) &&
				...
				if (hd_orientation_lock_is_locked_to_portrait())
					...
					priv->portrait = _hd_app_mgr_dbus_check_value(reply, MCE_ORIENTATION_PORTRAIT);
			}
			dbus_message_unref(reply);
		} else
			g_warning("%s: Couldn't send message.", __FUNCTION__);
		dbus_message_unref(msg);
	} else {		/* Deactivate, expect no reply.  We can only deactivate in lscape. */
		dbus_message_set_no_reply(msg, TRUE);
		if (!dbus_connection_send(conn, msg, NULL))
			g_warning("%s: Couldn't send message.", __FUNCTION__);
		dbus_message_unref(msg);
	...
	if (update_portraitness)
		...
		hd_comp_mgr_set_pip_flags(hd_comp_mgr_get(), priv->accel_enabled, priv->portrait && priv->slide_closed);
}
...
static void hd_app_mgr_gconf_value_changed(GConfClient * client, guint cnxn_id, GConfEntry * entry, gpointer user_data)
{
	...
	if (!g_strcmp0(gconf_entry_get_key(entry), GCONF_VIEWS_CURRENT_KEY)) {
		HdHome *home = hd_render_manager_get_home();

		if (hd_home_view_container_get_current_view(hd_home_get_view_container(home)) !=
		    gconf_value_get_int(gvalue) - 1)
			hd_home_view_container_set_current_view(hd_home_get_view_container(home),
								gconf_value_get_int(gvalue) - 1);
	}

	if (!g_strcmp0(gconf_entry_get_key(entry), GCONF_SLIDE_OPEN_KEY)) {
		...
		if (!priv->slide_closed &&
		    STATE_ONE_OF(hd_render_manager_get_state(),
				 HDRM_STATE_LAUNCHER_PORTRAIT | HDRM_STATE_TASK_NAV_PORTRAIT)) {
			...
			priv->portrait = FALSE;
			hd_render_manager_set_state(hd_render_manager_get_state() ==
						    HDRM_STATE_LAUNCHER_PORTRAIT ? HDRM_STATE_LAUNCHER :
						    HDRM_STATE_TASK_NAV);
			...
			hd_render_manager_set_state(hd_render_manager_get_state() ==
						    ...
						    HDRM_STATE_TASK_NAV_PORTRAIT);
		else
			hd_app_mgr_update_portraitness(self);
	}
	if (gconf_client_dir_exists(priv->gconf_client, GCONF_KEY_ACTIONS_DIR, NULL)) {
		conf_enable_home_contacts_phone = gconf_client_get_bool(priv->gconf_client,
									GCONF_KEY_ACTIONS_DIR "/home_contacts_phone",
									NULL);
		conf_enable_launcher_navigator_accel =
		    gconf_client_get_bool(priv->gconf_client, GCONF_KEY_ACTIONS_DIR "/launcher_navigator_accel", NULL);
		conf_enable_dbus_launcher_navigator =
		    gconf_client_get_bool(priv->gconf_client, GCONF_KEY_ACTIONS_DIR "/dbus_launcher_navigator", NULL);
		conf_default_launcher_positions =
		    gconf_client_get_bool(priv->gconf_client, GCONF_KEY_ACTIONS_DIR "/default_launcher_positions",
					  NULL);
		conf_disable_edit =
		    gconf_client_get_bool(priv->gconf_client, GCONF_KEY_ACTIONS_DIR "/disable_edit", NULL);
	}

	if (!g_strcmp0(gconf_entry_get_key(entry), GCONF_DISABLE_CALLUI_KEY)) {
		priv->disable_callui = value;

		/* Check if h-d needs to track the orientation. */
		hd_app_mgr_mce_activate_accel_if_needed(TRUE);
	} else if (!g_strcmp0(gconf_entry_get_key(entry), GCONF_UI_CAN_ROTATE_KEY)) {
...
static void _hd_app_mgr_request_app_pid_cb(DBusGProxy * proxy, guint pid, GError * error, gpointer data)
{
	HdRunningApp *app = HD_RUNNING_APP(data);

	if (error) {
		g_warning("%s: Couldn't get pid for service %s because %s\n",
			  __FUNCTION__, hd_running_app_get_service(app), error->message);
		return;
	}

	g_debug("%s: Got pid %d for %s\n", __FUNCTION__, pid, hd_running_app_get_service(app));
	hd_running_app_set_pid(app, pid);
}
...
HdRunningApp *hd_app_mgr_match_window(const char *res_name, const char *res_class, GPid pid)
{
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());
	HdRunningApp *app = NULL;
	HdLauncherApp *launcher = NULL;
...
HdLauncherTree *hd_app_mgr_get_tree()
{
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());
	return priv->tree;
}
	...
	for (; apps; apps = apps->next) {
		...
		if (!only_running || hd_running_app_get_state(app) == HD_APP_STATE_SHOWN) {
			g_debug("\tapp=%p, id=%s, pid=%d, state=%d\n",
				app,
				hd_running_app_get_id(app), hd_running_app_get_pid(app), hd_running_app_get_state(app));
		}
	}
}

void hd_app_mgr_dump_tree()
{
	HdAppMgrPrivate *priv = HD_APP_MGR_GET_PRIVATE(hd_app_mgr_get());
	...
	for (; items; items = items->next) {
		...
		g_debug("\titem=%p, id=%s, category=%s\n",
			item, hd_launcher_item_get_id(item), hd_launcher_item_get_category(item));
	}
}
