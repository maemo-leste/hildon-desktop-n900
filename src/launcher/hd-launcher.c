...
G_DEFINE_TYPE_WITH_CODE(HdLauncher, hd_launcher, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(HdLauncher));

/* Forward declarations */
static void hd_launcher_constructed(GObject * gobject);
static void hd_launcher_dispose(GObject * gobject);
static void hd_launcher_category_tile_clicked(HdLauncherTile * tile, gpointer data);
static void hd_launcher_application_tile_clicked(HdLauncherTile * tile, gpointer data);
static void hd_launcher_application_tile_long_clicked(HdLauncherTile * tile, gpointer data);
static gboolean hd_launcher_captured_event_cb(HdLauncher * launcher, ClutterEvent * event, gpointer data);
static gboolean hd_launcher_background_clicked(HdLauncher * self, ClutterButtonEvent * event, gpointer * data);
static gboolean hd_launcher_key_pressed(HdLauncher * self, ClutterButtonEvent * event, gpointer * data);
static void hd_launcher_populate_tree_starting(HdLauncherTree * tree, gpointer data);
static void hd_launcher_populate_tree_finished(HdLauncherTree * tree, gpointer data);
static void hd_launcher_lazy_traverse_cleanup(gpointer data);
static void hd_launcher_transition_new_frame(ClutterTimeline * timeline, gint frame_num, gpointer data);
...
static void hd_launcher_class_init(HdLauncherClass * klass)
{
	...
	launcher_signals[CAT_LAUNCHED] =
	    g_signal_new(I_("category-launched"),
			 HD_TYPE_LAUNCHER,
			 G_SIGNAL_RUN_FIRST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, NULL);
...
static void hd_launcher_init(HdLauncher * self)
{
...
static void hd_launcher_constructed(GObject * gobject)
{
	...
	clutter_actor_set_size(self, HD_LAUNCHER_PAGE_WIDTH, HD_LAUNCHER_PAGE_HEIGHT);

	priv->tree = g_object_ref(hd_app_mgr_get_tree());
	g_signal_connect(priv->tree, "starting", G_CALLBACK(hd_launcher_populate_tree_starting), gobject);
	g_signal_connect(priv->tree, "finished", G_CALLBACK(hd_launcher_populate_tree_finished), gobject);

	/* Add callback for clicked background */
	clutter_actor_set_reactive(self, TRUE);
	g_signal_connect(self, "captured-event", G_CALLBACK(hd_launcher_captured_event_cb), 0);
	g_signal_connect(self, "button-release-event", G_CALLBACK(hd_launcher_background_clicked), 0);
	g_signal_connect(self, "key-press-event", G_CALLBACK(hd_launcher_key_pressed), 0);

	/* App launch transition */
	priv->launch_image = 0;
	priv->launch_transition = g_object_ref(clutter_timeline_new_for_duration(400));
	g_signal_connect(priv->launch_transition, "new-frame", G_CALLBACK(hd_launcher_transition_new_frame), gobject);
...
static void _hd_launcher_hide_page(GQuark key_id, gpointer data, gpointer user_data)
{
	HdLauncherPage *page = HD_LAUNCHER_PAGE(data);
	clutter_actor_hide(CLUTTER_ACTOR(page));
}

static void _hd_launcher_update_orientation_cb(GQuark key_id, gpointer data, gpointer user_data)
{
	HdLauncherPage *page = HD_LAUNCHER_PAGE(data);
	HdLauncherGrid *grid;
	...
	if (portraited)
		clutter_actor_set_size(scroller, HD_LAUNCHER_PAGE_HEIGHT, HD_LAUNCHER_PAGE_WIDTH);
	else
		clutter_actor_set_size(scroller, HD_LAUNCHER_PAGE_WIDTH, HD_LAUNCHER_PAGE_HEIGHT);
...
void hd_launcher_update_orientation(gboolean portraited)
{
	...
	g_datalist_foreach(&priv->pages, _hd_launcher_update_orientation_cb, GBOOLEAN_TO_POINTER(portraited));
}
...
void hd_launcher_show(gboolean is_top_page)
{
	...
	hd_launcher_page_transition(HD_LAUNCHER_PAGE(priv->active_page), HD_LAUNCHER_PAGE_TRANSITION_IN);
...
void hd_launcher_hide(void)
{
	...
	if (priv->active_page) {
		...
		if (priv->active_page != top_page) {
			hd_launcher_page_transition(HD_LAUNCHER_PAGE(top_page), HD_LAUNCHER_PAGE_TRANSITION_OUT_BACK);
		}

		hd_launcher_page_transition(HD_LAUNCHER_PAGE(priv->active_page), HD_LAUNCHER_PAGE_TRANSITION_OUT);
...
gboolean hd_launcher_back_button_clicked()
{
	HdLauncherPrivate *priv = HD_LAUNCHER_GET_PRIVATE(hd_launcher_get());
	ClutterActor *top_page = g_datalist_get_data(&priv->pages,
						     HD_LAUNCHER_ITEM_TOP_CATEGORY);
	...
	if (priv->active_page == top_page)
		...
		if (top_page)
			hd_launcher_page_transition(HD_LAUNCHER_PAGE(top_page), HD_LAUNCHER_PAGE_TRANSITION_FORWARD);
		priv->active_page = top_page;
		g_signal_emit(hd_launcher_get(), launcher_signals[CAT_HIDDEN], 0, NULL);
	}
...
static void hd_launcher_category_tile_clicked(HdLauncherTile * tile, gpointer data)
{
	...
	hd_launcher_page_transition(HD_LAUNCHER_PAGE(priv->active_page), HD_LAUNCHER_PAGE_TRANSITION_BACK);
	hd_launcher_page_transition(HD_LAUNCHER_PAGE(page), HD_LAUNCHER_PAGE_TRANSITION_IN_SUB);
	priv->active_page = page;
	g_signal_emit(hd_launcher_get(), launcher_signals[CAT_LAUNCHED], 0, NULL);
}
...
static void hd_launcher_application_tile_clicked(HdLauncherTile * tile, gpointer data)
{
	...
	hd_launcher_page_transition(HD_LAUNCHER_PAGE(priv->active_page), HD_LAUNCHER_PAGE_TRANSITION_LAUNCH);
	/* also do animation for the topmost pane if we had it... */
	top_page = g_datalist_get_data(&priv->pages, HD_LAUNCHER_ITEM_TOP_CATEGORY);
	/* if we're not at the top page, we must transition that out too.
	...
	g_signal_emit(hd_launcher_get(), launcher_signals[APP_LAUNCHED], 0, data, NULL);
}
...
static void _hd_launcher_editor_is_topmost(HildonWindow * editor, GParamSpec * pspec, HdLauncher * launcher)
{
	HdLauncherPrivate *priv = launcher->priv;

	g_debug("%s: topmost: %d", __FUNCTION__, hildon_window_get_is_topmost(editor));
...
static void _hd_launcher_editor_done(HdLauncherEditor * editor, gboolean modified, HdLauncher * launcher)
{
	HdLauncherPrivate *priv = launcher->priv;

	g_debug("%s: modified: %d", __FUNCTION__, modified);
...
static gboolean _hd_launcher_editor_destroyed(GtkWidget * widget, HdLauncher * launcher)
{
	HdLauncherPrivate *priv = launcher->priv;
...
static void hd_launcher_application_tile_long_clicked(HdLauncherTile * tile, gpointer data)
{
	...
	if (priv->active_page) {
		ClutterActor *scroll = hd_launcher_page_get_scroller(HD_LAUNCHER_PAGE(priv->active_page));
	...
	g_signal_connect(priv->editor, "destroy", G_CALLBACK(_hd_launcher_editor_destroyed), launcher);
	g_signal_connect(priv->editor, "notify::is-topmost", G_CALLBACK(_hd_launcher_editor_is_topmost), launcher);
	g_signal_connect(priv->editor, "done", G_CALLBACK(_hd_launcher_editor_done), launcher);

	priv->is_editor_in_landscape = !STATE_IS_PORTRAIT(hd_render_manager_get_state());

	hd_launcher_editor_show(priv->editor);
	hd_launcher_editor_select(HD_LAUNCHER_EDITOR(priv->editor), hd_launcher_tile_get_text(tile), x_align, y_align);
}

static void _hd_launcher_clear_page(GQuark key_id, gpointer data, gpointer user_data)
{
	HdLauncherPage *page = HD_LAUNCHER_PAGE(data);
	hd_launcher_grid_clear(HD_LAUNCHER_GRID(hd_launcher_page_get_grid(page)));
}

static void _hd_launcher_layout_page(GQuark key_id, gpointer data, gpointer user_data)
{
	HdLauncherPrivate *priv = HD_LAUNCHER_GET_PRIVATE(hd_launcher_get());
	HdLauncherPage *page = HD_LAUNCHER_PAGE(data);
...
static void hd_launcher_create_page(HdLauncherItem * item, gpointer data)
{
	ClutterActor *self = CLUTTER_ACTOR(hd_launcher_get());
	HdLauncherPrivate *priv = HD_LAUNCHER_GET_PRIVATE(self);
	...
	g_datalist_set_data_full(&priv->pages, hd_launcher_item_get_id(item), newpage,
				 (GDestroyNotify) clutter_actor_destroy);
}

static gboolean hd_launcher_lazy_traverse_tree(gpointer data)
{
	HdLauncherPrivate *priv = HD_LAUNCHER_GET_PRIVATE(hd_launcher_get());
	...
	if (!tdata || tdata->cancelled || tdata != priv->current_traversal)
	...
	for (i = 0; i < 5; i++) {
		...
		tile = hd_launcher_tile_new(hd_launcher_item_get_icon_name(item),
		...
		page = g_datalist_get_data(&priv->pages, hd_launcher_item_get_category(item));
		if (!page)
		...
		if (!page) {
			...
			if (hd_launcher_item_get_item_type(item) == HD_CATEGORY_LAUNCHER) {
				g_signal_connect(tile, "clicked",
						 ...
						 g_datalist_get_data(&priv->pages, hd_launcher_item_get_id(item)));
			} else if (hd_launcher_item_get_item_type(item) == HD_APPLICATION_LAUNCHER) {
				g_signal_connect(tile, "clicked",
						 G_CALLBACK(hd_launcher_application_tile_clicked), item);
			}

			g_signal_connect(tile, "long-clicked",
					 G_CALLBACK(hd_launcher_application_tile_long_clicked), item);
		}
...
static void hd_launcher_populate_tree_finished(HdLauncherTree * tree, gpointer data)
{
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(launcher), top_page);
	clutter_actor_hide(top_page);
	priv->active_page = NULL;
	g_datalist_set_data_full(&priv->pages, HD_LAUNCHER_ITEM_TOP_CATEGORY, top_page,
				 (GDestroyNotify) clutter_actor_destroy);
	...
	clutter_threads_add_idle_full(CLUTTER_PRIORITY_REDRAW + 20,
				      hd_launcher_lazy_traverse_tree, tdata, hd_launcher_lazy_traverse_cleanup);
}
...
static gboolean _hd_launcher_transition_clicked(ClutterActor * actor, ClutterEvent * event, gpointer user_data)
{
...
/* handle clicks to the fake launch image. If we've been up this long the
   app may have died and we just want to remove ourselves. */
static gboolean hd_launcher_transition_loading_timeout()
{
	HdLauncherPrivate *priv = HD_LAUNCHER_GET_PRIVATE(hd_launcher_get());
...
void hd_launcher_stop_loading_transition()
{
	HdLauncherPrivate *priv = HD_LAUNCHER_GET_PRIVATE(hd_launcher_get());
...
gboolean hd_launcher_transition_app_start(HdLauncherApp * item)
{
	...
	if (service_name && index(service_name, '/') == NULL && service_name[0] != '.') {
		if (STATE_IS_PORTRAIT(hd_render_manager_get_state()))
			cached_image = g_strdup_printf("%s/.cache/launch/%s_portrait.pvr",
						       getenv("HOME"), service_name);
		else
			cached_image = g_strdup_printf("%s/.cache/launch/%s.pvr", getenv("HOME"), service_name);
	...
	if (loading_image) {
		...
		if (!app_image)
			g_warning("%s: Preload image file '%s' specified for '%s'"
				  " couldn't be loaded", __FUNCTION__, loading_image, hd_launcher_app_get_exec(item));
		else {
			guint w, h;
			...
			region.height = hd_comp_mgr_get_current_screen_height() - HD_COMP_MGR_TOP_MARGIN;
			...
			if (w > region.width || h > region.height) {
				...
				clutter_actor_set_size(CLUTTER_ACTOR(sub), region.width, region.height);
	...
	if (priv->launch_tile && item) {
		...
		ClutterActor *icon;
		clutter_actor_get_positionu(CLUTTER_ACTOR(tile), &priv->launch_position.x, &priv->launch_position.y);
		icon = hd_launcher_tile_get_icon(tile);
		if (icon) {
			ClutterVertex offs, size;
			clutter_actor_get_positionu(icon, &offs.x, &offs.y);
	...
	/* Clean up. */
	if (priv->launch_tile) {
		g_object_remove_weak_pointer(G_OBJECT(priv->launch_tile), &priv->launch_tile);
		priv->launch_tile = NULL;
	}
	/* append scroller movement */
	if (priv->active_page)
		priv->launch_position.y -= hd_launcher_page_get_scroll_y(HD_LAUNCHER_PAGE(priv->active_page));
	/* all because the tidy- stuff breaks clutter's nice 'get absolute position'
	 * code... */

	hd_render_manager_set_loading(priv->launch_image);
	clutter_actor_set_name(priv->launch_image, "HdLauncher:launch_image");
	clutter_actor_set_position(priv->launch_image, 0, HD_COMP_MGR_TOP_MARGIN);
	clutter_actor_set_reactive(priv->launch_image, TRUE);
	g_signal_connect(priv->launch_image, "button-release-event", G_CALLBACK(_hd_launcher_transition_clicked), 0);

	clutter_timeline_set_duration(priv->launch_transition,
				      hd_transition_get_int("launcher_launch", "duration", 200));
	/* Run the first step of the transition so we don't get flicker before
	 * the timeline is called */
	hd_launcher_transition_new_frame(priv->launch_transition, 0, launcher);
	...
	priv->launch_image_timeout = g_timeout_add_seconds(10, hd_launcher_transition_loading_timeout, 0);
...
static void _hd_launcher_transition_stop_foreach(GQuark key_id, gpointer data, gpointer user_data)
{
...
void hd_launcher_transition_stop(void)
{
	...
	g_datalist_foreach(&priv->pages, _hd_launcher_transition_stop_foreach, (gpointer) 0);
}
...
static gboolean hd_launcher_captured_event_cb(HdLauncher * launcher, ClutterEvent * event, gpointer data)
{
	...
	if (event->type == CLUTTER_BUTTON_PRESS) {
		...
		if (priv->active_page)
			hd_launcher_page_set_drag_distance(HD_LAUNCHER_PAGE(priv->active_page), 0);
	}
...
static gboolean hd_launcher_background_clicked(HdLauncher * self, ClutterButtonEvent * event, gpointer * data)
{
	...
	if (!priv->active_page ||
	    (hd_launcher_page_get_drag_distance(HD_LAUNCHER_PAGE(priv->active_page)) < HD_LAUNCHER_TILE_MAX_DRAG))
...
gboolean hd_launcher_is_portrait(void)
{
	HdLauncherPrivate *priv = HD_LAUNCHER_GET_PRIVATE(hd_launcher_get());
