...
static gboolean _hd_launcher_grid_blocker_release_cb(ClutterActor * actor, ClutterButtonEvent * event, gpointer * data);

static gboolean hd_launcher_grid_is_portrait(HdLauncherGrid * self);
#define HD_LAUNCHER_GRID_MAX_COLUMNS_LANDSCAPE (int)(HD_COMP_MGR_LANDSCAPE_WIDTH/160)
#define HD_LAUNCHER_GRID_MAX_COLUMNS_PORTRAIT (int)(HD_COMP_MGR_PORTRAIT_WIDTH/160)
...
G_DEFINE_TYPE_WITH_CODE(HdLauncherGrid, hd_launcher_grid, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(HdLauncherGrid)
			G_IMPLEMENT_INTERFACE(TIDY_TYPE_SCROLLABLE, tidy_scrollable_iface_init));

static inline void hd_launcher_grid_refresh_h_adjustment(HdLauncherGrid * grid)
{
	...
	clutter_actor_get_clipu(CLUTTER_ACTOR(grid), &clip_x, NULL, &clip_width, NULL);

	if (clip_width == 0)
		page_width = CLUTTER_UNITS_TO_FIXED(width);
	else
		page_width = MIN(CLUTTER_UNITS_TO_FIXED(width), CLUTTER_UNITS_TO_FIXED(clip_width - clip_x));

	tidy_adjustment_set_valuesx(priv->h_adjustment,
				    tidy_adjustment_get_valuex(priv->h_adjustment),
				    0, width, CFX_ONE, CFX_ONE * 20, page_width);
}

static inline void hd_launcher_grid_refresh_v_adjustment(HdLauncherGrid * grid)
{
	HdLauncherGridPrivate *priv = grid->priv;
	...
	clutter_actor_get_clipu(CLUTTER_ACTOR(grid), NULL, &clip_y, NULL, &clip_height);
	if (grid_height >= CLUTTER_UNITS_FROM_INT(screen_height)) {
		...
		tidy_adjustment_set_skirtx(priv->v_adjustment, clutter_qdivx(CFX_ONE, CLUTTER_INT_TO_FIXED(4)));
	} else
		tidy_adjustment_set_skirtx(priv->v_adjustment, 0);

	if (clip_height == 0)
		page_height = MIN(CLUTTER_UNITS_TO_FIXED(grid_height),
				  CLUTTER_UNITS_TO_FIXED(CLUTTER_UNITS_FROM_DEVICE(screen_height)));
	else
		page_height = MIN(CLUTTER_UNITS_TO_FIXED(grid_height), CLUTTER_UNITS_TO_FIXED(clip_height - clip_y));

	tidy_adjustment_set_valuesx(priv->v_adjustment,
				    tidy_adjustment_get_valuex(priv->v_adjustment),
				    0, grid_height, CFX_ONE, CFX_ONE * 20, page_height);
}
...
static void adjustment_value_notify(GObject * gobject, GParamSpec * pspec, gpointer user_data)
{
	ClutterActor *grid = user_data;
	HdLauncherGridPrivate *priv = HD_LAUNCHER_GRID(grid)->priv;

	clutter_actor_set_anchor_point(grid, 0, tidy_adjustment_get_value(priv->v_adjustment));
}

static void
hd_launcher_grid_set_adjustments(TidyScrollable * scrollable, TidyAdjustment * h_adj, TidyAdjustment * v_adj)
{
	HdLauncherGridPrivate *priv = HD_LAUNCHER_GRID(scrollable)->priv;

	if (h_adj != priv->h_adjustment) {
		if (priv->h_adjustment) {
			g_signal_handlers_disconnect_by_func(priv->h_adjustment, adjustment_value_notify, scrollable);
		...
		if (h_adj) {
			...
			g_signal_connect(priv->h_adjustment, "notify::value",
					 G_CALLBACK(adjustment_value_notify), scrollable);
		}
	}

	if (v_adj != priv->v_adjustment) {
		if (priv->v_adjustment) {
			g_signal_handlers_disconnect_by_func(priv->v_adjustment, adjustment_value_notify, scrollable);
...
static void
hd_launcher_grid_get_adjustments(TidyScrollable * scrollable, TidyAdjustment ** h_adj, TidyAdjustment ** v_adj)
{
	HdLauncherGridPrivate *priv = HD_LAUNCHER_GRID(scrollable)->priv;
	...
	if (h_adj) {
		if (priv->h_adjustment)
			*h_adj = priv->h_adjustment;
			...
			hd_launcher_grid_set_adjustments(scrollable, adjustment, priv->v_adjustment);
	...
	if (v_adj) {
		if (priv->v_adjustment)
			*v_adj = priv->v_adjustment;
			...
			adjustment = tidy_adjustment_newx(0, 0, 0, 0, 0, 0);
			hd_launcher_grid_set_adjustments(scrollable, priv->h_adjustment, adjustment);
			g_object_unref(adjustment);
...
static void hd_launcher_grid_actor_added(ClutterContainer * container, ClutterActor * actor)
{
...
static void hd_launcher_grid_actor_removed(ClutterContainer * container, ClutterActor * actor)
{
	HdLauncherGridPrivate *priv = HD_LAUNCHER_GRID(container)->priv;
...
static void _hd_launcher_grid_count_children_and_rows(HdLauncherGrid * grid, guint * children, guint * rows)
{
	HdLauncherGridPrivate *priv = HD_LAUNCHER_GRID_GET_PRIVATE(grid);
	GList *l;
 ...
 */
static GList *_hd_launcher_grid_layout_row(HdLauncherGrid * grid,
					   GList * l, guint * remaining, guint cur_y, guint h_spacing)
{
	ClutterActor *child;
	guint allocated;
	guint cur_x;
...
void hd_launcher_grid_layout(HdLauncherGrid * grid)
{
	...
	g_list_foreach(priv->blockers, (GFunc) clutter_actor_destroy, NULL);
	g_list_free(priv->blockers);
	priv->blockers = NULL;

	_hd_launcher_grid_count_children_and_rows(grid, &n_visible_launchers, &n_rows);
	...
	while (l) {
		...
		l = _hd_launcher_grid_layout_row(grid, l, &n_visible_launchers, cur_height, priv->h_spacing);
		if (l) {
			...
			g_signal_connect(blocker, "button-release-event",
					 G_CALLBACK(_hd_launcher_grid_blocker_release_cb), NULL);
	...
	if (hd_launcher_grid_is_portrait(grid))
		clutter_actor_set_size(CLUTTER_ACTOR(grid), HD_LAUNCHER_PAGE_HEIGHT, cur_height);
	else
		clutter_actor_set_size(CLUTTER_ACTOR(grid), HD_LAUNCHER_PAGE_WIDTH, cur_height);
...
static void hd_launcher_grid_dispose(GObject * gobject)
{
	HdLauncherGridPrivate *priv = HD_LAUNCHER_GRID(gobject)->priv;

	g_list_foreach(priv->tiles, (GFunc) clutter_actor_destroy, NULL);
...
static void hd_launcher_grid_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec)
{
	HdLauncherGridPrivate *priv = HD_LAUNCHER_GRID(gobject)->priv;
	...
	case PROP_H_ADJUSTMENT:
		hd_launcher_grid_set_adjustments(TIDY_SCROLLABLE(gobject),
						 g_value_get_object(value), priv->v_adjustment);
		break;

	case PROP_V_ADJUSTMENT:
		hd_launcher_grid_set_adjustments(TIDY_SCROLLABLE(gobject),
						 priv->h_adjustment, g_value_get_object(value));
		break;
...
static void hd_launcher_grid_get_property(GObject * gobject, guint prop_id, GValue * value, GParamSpec * pspec)
{
	switch (prop_id) {
	case PROP_H_ADJUSTMENT:
		{
			...
			hd_launcher_grid_get_adjustments(TIDY_SCROLLABLE(gobject), &adjustment, NULL);
	...
	case PROP_V_ADJUSTMENT:
		{
			...
			hd_launcher_grid_get_adjustments(TIDY_SCROLLABLE(gobject), NULL, &adjustment);
			g_value_set_object(value, adjustment);
		}
		break;
...
static void hd_launcher_grid_class_init(HdLauncherGridClass * klass)
{
...
static gboolean hd_launcher_grid_is_portrait(HdLauncherGrid * grid)
{
 ...
 */
void hd_launcher_grid_set_portrait(HdLauncherGrid * grid, gboolean portraited)
{
	HdLauncherGridPrivate *priv;
...
static void hd_launcher_grid_init(HdLauncherGrid * launcher)
{
	HdLauncherGridPrivate *priv;
	...
	g_signal_connect(launcher, "actor-added", G_CALLBACK(hd_launcher_grid_actor_added), 0);
	g_signal_connect(launcher, "actor-removed", G_CALLBACK(hd_launcher_grid_actor_removed), 0);
}
...
void hd_launcher_grid_transition_begin(HdLauncherGrid * grid, HdLauncherPageTransition trans_type)
{
	HdLauncherGridPrivate *priv = grid->priv;

	priv->transition_depth =
	    hd_transition_get_int(hd_launcher_page_get_transition_string(trans_type),
				  "depth", 100 /* default value */ );
	priv->transition_sequenced = 0;
	if (trans_type == HD_LAUNCHER_PAGE_TRANSITION_IN || trans_type == HD_LAUNCHER_PAGE_TRANSITION_IN_SUB) {
		priv->transition_sequenced =
		    hd_transition_get_int(hd_launcher_page_get_transition_string(trans_type), "sequenced", 0);
		if (priv->transition_sequenced) {
			grid->priv->transition_keyframes =
			    hd_transition_get_keyframes(hd_launcher_page_get_transition_string(trans_type),
							"keyframes", "0,1");
...
hd_launcher_grid_transition(HdLauncherGrid * grid,
			    HdLauncherPage * page, HdLauncherPageTransition trans_type, float amount)
{
	HdLauncherGridPrivate *priv;
	GList *l;
	...
	case HD_LAUNCHER_PAGE_TRANSITION_LAUNCH:
		clutter_actor_get_sizeu(CLUTTER_ACTOR(page), &movement_centre.x, &movement_centre.y);
		break;
	default:
		break;
	}
			...
			case HD_LAUNCHER_PAGE_TRANSITION_IN_SUB:
				{
					...
					if (priv->transition_sequenced) {
						label_amt =
						    hd_key_frame_interpolate(priv->transition_keyframes_label,
									     order_amt);
						icon_amt =
						    hd_key_frame_interpolate(priv->transition_keyframes_icon,
									     order_amt);
						...
						if (icon_amt > 1)
							icon_amt = 1;
						depth = CLUTTER_UNITS_FROM_FLOAT(priv->transition_depth *
										 (1 -
										  hd_key_frame_interpolate
										  (priv->transition_keyframes,
										   order_amt)));
					} else {
