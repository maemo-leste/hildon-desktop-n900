...
G_DEFINE_TYPE_WITH_CODE(HdLauncherTree, hd_launcher_tree, G_TYPE_OBJECT, G_ADD_PRIVATE(HdLauncherTree));

static void hd_launcher_tree_handle_tree_changed(GMenuTree * menu_tree, gpointer user_data);

static void hd_launcher_tree_handle_theme_changed(HdLauncherTree * tree);

static WalkThreadData *walk_thread_data_new(HdLauncherTree * tree)
{
	WalkThreadData *data;
...
static WalkThreadData *walk_thread_data_new_level(WalkThreadData * parent, GMenuTreeDirectory * dir)
{
...
static void walk_thread_data_free(WalkThreadData * data)
{
 ...
 */
static gboolean walk_thread_done_idle(gpointer user_data)
{
	...
	if ((priv->active_walk == data) && !data->cancelled) {
		...
		if (!priv->theme_changed_signal_connected) {
			g_signal_connect_swapped(gtk_icon_theme_get_default(),
						 "changed",
						 G_CALLBACK(hd_launcher_tree_handle_theme_changed), data->tree);
 ...
 */
static gpointer walk_thread_func(gpointer user_data)
{
	WalkThreadData *data = user_data;
	GMenuTreeIter *iter;
	...
	while ((next_type = gmenu_tree_iter_next(iter)) != GMENU_TREE_ITEM_INVALID) {
		...
		case GMENU_TREE_ITEM_DIRECTORY:
			{
				GMenuTreeDirectory *entry_dir = gmenu_tree_iter_get_directory(iter);
				...
				gmenu_tree_item_unref(entry_dir);
		...
		if (stat(key_file_path, &key_file_stat)) {
			g_warning("%s: Unable to stat %s", __FUNCTION__, key_file_path);
		} else {
			key_file = g_key_file_new();
			g_key_file_load_from_file(key_file, key_file_path, 0, &error);
			if (error) {
				g_warning("%s: Unable to parse %s: %s", __FUNCTION__, key_file_path, error->message);
...
static void hd_launcher_tree_finalize(GObject * gobject)
{
...
static void hd_launcher_tree_class_init(HdLauncherTreeClass * klass)
{
...
static void hd_launcher_tree_init(HdLauncherTree * tree)
{
...
HdLauncherTree *hd_launcher_tree_new()
{
	HdLauncherTree *tree;
...
static void hd_launcher_tree_handle_tree_changed(GMenuTree * menu_tree, gpointer user_data)
{
	...
	if (!gmenu_tree_load_sync(menu_tree, &error)) {
		g_printerr("%s: Failed to load tree: %s\n", __FUNCTION__, error->message);
		g_clear_error(&error);
		return;
	}
...
void hd_launcher_tree_populate(HdLauncherTree * tree)
{
	...
	priv->tree = gmenu_tree_new(HD_LAUNCHER_MENU_FILE, GMENU_TREE_FLAGS_SHOW_EMPTY);
	if (!priv->tree) {
		g_warning("%s: Couldn't load menu.", __FUNCTION__);
		return;
	}

	if (!gmenu_tree_load_sync(priv->tree, &error)) {
		g_printerr("%s: Failed to load tree: %s\n", __FUNCTION__, error->message);
		g_clear_error(&error);
		g_object_unref(priv->tree);
		priv->tree = NULL;
...
static gint _compare_item_id(gconstpointer a, gconstpointer b)
{
	return g_strcmp0(hd_launcher_item_get_id(HD_LAUNCHER_ITEM(a)), (const gchar *)b);
}
...
static gint _compare_service(gconstpointer a, gconstpointer b)
{
	if (!HD_IS_LAUNCHER_APP(a))
		return !0;
	return g_strcmp0(hd_launcher_app_get_service(HD_LAUNCHER_APP(a)), (const gchar *)b);
}
...
void hd_launcher_tree_ensure_user_menu(void)
{
	...
	/* The menu directory. */
	menu_dirname = g_build_filename(g_get_user_config_dir(), "menus/hildon", NULL);
	...
	if (!g_file_test(menu_filename, G_FILE_TEST_EXISTS)) {
		...
		g_string_printf(menu, HD_LAUNCHER_MENU_START, g_get_user_data_dir(), g_get_user_data_dir());
	...
	menu_dirname = g_build_filename(g_get_user_data_dir(), "applications/hildon", NULL);
	if (!g_file_test(menu_dirname, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR)) {
		if (g_mkdir_with_parents(menu_dirname, CREATE_MODE)) {
			g_warning("%s: Couldn't create dir %s", __FUNCTION__, menu_dirname);
		}
