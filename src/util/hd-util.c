...
void *hd_util_get_win_prop_data_and_validate(Display * xdpy,
					     Window xwin,
					     Atom prop,
					     Atom type, gint expected_format, gint expected_n_items, gint * n_items_ret)
{
	Atom type_ret;
	int format_ret;
	...
	status = XGetWindowProperty(xdpy,
				    ...
				    prop,
				    0, G_MAXLONG,
				    False, type, &type_ret, &format_ret, &items_ret, &after_ret, &prop_data);
...
char *hd_util_get_x_window_string_property(MBWindowManager * wm, Window xwin, HdAtoms atom_id)
{
	Atom type;
	int format, ret;
	unsigned char *value;
	...
	ret = XGetWindowProperty(wm->xdpy, xwin,
				 hd_comp_mgr_get_atom(HD_COMP_MGR(wm->comp_mgr),
						      atom_id),
				 0, 999, False, XA_STRING, &type, &format, &items, &left, &value);
	if (ret != Success)
		g_warning("%s: XGetWindowProperty(0x%lx, 0x%x): failed (%d)", __FUNCTION__, xwin, atom_id, ret);
	return ret != Success || type == None ? NULL : (char *)value;
}

static void hd_util_modal_blocker_release_handler(XButtonEvent * xev, void *userdata)
{
	...
	if (mb_wm_client_is_map_confirmed(c) && c->cm_client) {
		...
		g_debug("%s: ignoring ButtonRelease because " "window for this blocker is not mapped yet\n", __func__);
}

static void hd_util_modal_blocker_release_handler_for_ping(XButtonEvent * xev, void *userdata)
{
	MBWindowManagerClient *c = userdata;
...
unsigned long hd_util_modal_blocker_realize(MBWindowManagerClient * client, gboolean ping_only)
{
	...
	if (!client->xwin_modal_blocker) {
		...
		attr.event_mask = MBWMChildMask | ButtonPressMask | ButtonReleaseMask | ExposureMask;
		...
		client->xwin_modal_blocker =
		    XCreateWindow(wm->xdpy,
				  ...
				  HD_COMP_MGR_LANDSCAPE_WIDTH,
				  0,
				  CopyFromParent, InputOnly, CopyFromParent, CWOverrideRedirect | CWEventMask, &attr);
...
Bool hd_util_client_has_modal_blocker(MBWindowManagerClient * c)
{
	...
	return
	    ...
	    !mb_wm_client_get_transient_for(c) && mb_wm_get_modality_type(c->wmref) == MBWMModalitySystem;
}

static RRCrtc get_primary_crtc(MBWindowManager * wm, XRRScreenResources * res)
{
	int i;
	...
	rr_connector_type = hd_comp_mgr_wm_get_atom(wm, HD_ATOM_RANDR_CONNECTOR_TYPE);
	rr_connector_panel = hd_comp_mgr_wm_get_atom(wm, HD_ATOM_RANDR_CONNECTOR_TYPE_PANEL);
	...
	for (i = 0; i < res->noutput; i++) {
		...
		if (XRRGetOutputProperty(wm->xdpy, res->outputs[i], rr_connector_type,
					 ...
					 &actual_format, &nitems, &bytes_after, &contype) == Success) {
...
void hd_util_set_screen_size_property(MBWindowManager * wm, gboolean is_portrait)
{
	...
	XChangeProperty(wm->xdpy, wm->root_win->hidden_window,
			...
			XA_CARDINAL, 32, PropModeReplace, (unsigned char *)value, 2);
}

static gboolean randr_supported(MBWindowManager * wm)
{
...
static gboolean hd_util_change_screen_orientation_real(MBWindowManager * wm, gboolean goto_portrait, gboolean do_change)
{
	...
	if (goto_portrait) {
		...
		if ((crtc_info->rotation == RR_Rotate_0 &&
		    ...
		    (crtc_info->rotation == RR_Rotate_270 && crtc_info->width > crtc_info->height)) {
			want = RR_Rotate_0;
	...
	if (initially_rotated == -1) {
		initially_rotated = crtc_info->rotation != RR_Rotate_0 && crtc_info->rotation != RR_Rotate_180;
	}

	if (do_change) {
		if (!(crtc_info->rotations & want)) {
			g_warning("CRTC does not support rotation (0x%.8X vs. 0x%.8X)", crtc_info->rotations, want);
			goto err_crtc_info;
		}
		...
		XRRSetCrtcConfig(wm->xdpy, res, crtc, crtc_info->timestamp, 0, 0, None, RR_Rotate_0, NULL, 0);
		/* Then change the screen size to accommodate our glorious new CRTC. */
		XRRSetScreenSize(wm->xdpy, wm->root_win->xwindow, width, height, width_mm, height_mm);
		/* And now rotate. */
...
gboolean hd_util_change_screen_orientation(MBWindowManager * wm, gboolean goto_portrait)
{
...
void hd_util_root_window_configured(MBWindowManager * wm)
{
	...
	XDeleteProperty(wm->xdpy, wm->root_win->xwindow, wm->atoms[MBWM_ATOM_MAEMO_SUPPRESS_ROOT_RECONFIGURATION]);
	XSync(wm->xdpy, False);
	mb_wm_util_async_untrap_x_errors();
}
...
gboolean hd_util_rotate_geometry(ClutterGeometry * geo, guint scrw, guint scrh)
{
	gint tmp;
	...
	if (scrw > scrh) {	/* Map from from portrait to landscape. */
		tmp = geo->x;
		geo->x = geo->y;
		geo->y = scrh - (tmp + geo->height);
		return TRUE;
	} else {		/* Map from landscape to portrait. */
		tmp = geo->y;
		geo->y = geo->x;
...
static gboolean hd_util_get_actor_bounds(ClutterActor * actor, ClutterGeometry * geo, gboolean * is_visible)
{
	gdouble x, y;
...
void hd_util_partial_redraw_if_possible(ClutterActor * actor, ClutterGeometry * bounds)
{
...
gboolean hd_util_client_obscured(MBWindowManagerClient * client)
{
	...
	for (obscurer = client->stacked_above;
	     obscurer && !gdk_region_empty(region); obscurer = obscurer->stacked_above) {
		GdkRegion *obscure_region;
		if (!obscurer->window)
			continue;	/* be safe */
		obscure_region = gdk_region_rectangle((GdkRectangle *) (void *)&obscurer->window->geometry);
...
guint hd_util_display_width()
{
	static guint width = 0;

	if (width == 0) {
		if ((!display_is_portrait && !initially_rotated) || (display_is_portrait && initially_rotated)) {
			width = WidthOfScreen(ScreenOfDisplay(clutter_x11_get_default_display(), 0));
		} else {
			width = HeightOfScreen(ScreenOfDisplay(clutter_x11_get_default_display(), 0));
		}
	}
...
guint hd_util_display_height()
{
	static guint height = 0;

	if (height == 0) {
		if ((!display_is_portrait && !initially_rotated) || (display_is_portrait && initially_rotated)) {
			height = HeightOfScreen(ScreenOfDisplay(clutter_x11_get_default_display(), 0));
		} else {
			height = WidthOfScreen(ScreenOfDisplay(clutter_x11_get_default_display(), 0));
		}
	}
