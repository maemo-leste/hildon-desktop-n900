...
static void hd_gtk_style_to_clutter_color(ClutterColor * dst, const GdkColor * src)
{
...
hd_gtk_style_get_color_component(HDGtkWidgetSingleton widget_id,
				 HDGtkStyle component, GtkStateType state, ClutterColor * color)
{
	GtkWidget *widget;
	GtkStyle *style;
...
void hd_gtk_style_get_light_color(HDGtkWidgetSingleton widget_id, GtkStateType state, ClutterColor * color)
{
	...
	hd_gtk_style_get_color_component(widget_id, HD_GTK_STYLE_LIGHT, state, color);
}
...
void hd_gtk_style_get_dark_color(HDGtkWidgetSingleton widget_id, GtkStateType state, ClutterColor * color)
{
	...
	hd_gtk_style_get_color_component(widget_id, HD_GTK_STYLE_DARK, state, color);
}
...
void hd_gtk_style_get_mid_color(HDGtkWidgetSingleton widget_id, GtkStateType state, ClutterColor * color)
{
	...
	hd_gtk_style_get_color_component(widget_id, HD_GTK_STYLE_MID, state, color);
}
...
void hd_gtk_style_get_base_color(HDGtkWidgetSingleton widget_id, GtkStateType state, ClutterColor * color)
{
	g_return_if_fail(color != NULL);
...
void hd_gtk_style_resolve_logical_color(ClutterColor * color, const gchar * logical_name)
{
	...
	style = gtk_rc_get_style_by_paths(gtk_settings_get_default(), NULL, NULL, GTK_TYPE_WIDGET);
	if (!style || !gtk_style_lookup_color(style, logical_name, &gtk_color)) {	/* Fall back to white. */
...
gchar *hd_gtk_style_resolve_logical_font(const gchar * logical_name)
{
	...
	style = gtk_rc_get_style_by_paths(gtk_settings_get_default(), logical_name, NULL, G_TYPE_NONE);
	if (!style) {		/* Fall back to system font. */
