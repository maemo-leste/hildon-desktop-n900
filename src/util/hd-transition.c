...
static void hptimer_calc_remaining(HPTimer * hptimer)
{
	guint diff;
	GTimeVal now;
...
static gboolean hptimer_source_prepare(GSource * src, gint * timeout)
{
...
static gboolean hptimer_source_check(GSource * src)
{
	HPTimer *hptimer = (HPTimer *) src;
...
static gboolean hptimer_source_dispatch(GSource * src, GSourceFunc cb, gpointer cbarg)
{
	HPTimer *hptimer = (HPTimer *) src;
 ...
 */
static HPTimer *hptimer_new(unsigned expiry, GSourceFunc cb, gpointer cbarg, GDestroyNotify dtor)
{
...
float hd_transition_overshoot(float x)
{
...
float hd_transition_smooth_ramp(float amt)
{
	if (amt > 0 && amt < 1)
...
float hd_transition_ease_in(float amt)
{
	if (amt > 0 && amt < 1)
...
float hd_transition_ease_out(float amt)
{
	if (amt > 0 && amt < 1)
...
static ClutterTimeline *hd_transition_timeline_new(const gchar * transition,
						   MBWMCompMgrClientEvent event, gint default_length)
{
	const char *key = event == MBWMCompMgrClientEventMap ? "duration_in" : "duration_out";
	return clutter_timeline_new_for_duration(hd_transition_get_int(transition, key, default_length));
}
...
void on_decor_progress_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, ClutterActor * progress_texture)
{
	if (!hd_dbus_display_is_off && TIDY_IS_SUB_TEXTURE(progress_texture) &&
		 ...
		 */
		ClutterGeometry progress_region = { HD_THEME_IMG_PROGRESS_SIZE * frame_num, 0,
			HD_THEME_IMG_PROGRESS_SIZE, HD_THEME_IMG_PROGRESS_SIZE
		};

		tidy_sub_texture_set_region(TIDY_SUB_TEXTURE(progress_texture), &progress_region);
...
static void on_popup_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, HDEffectData * data)
{
	float amt;
	...
	clutter_actor_set_anchor_pointu(actor, 0, CLUTTER_INT_TO_FIXED(geo.y) - CLUTTER_FLOAT_TO_FIXED(status_pos));
	...
	if ((status_pos > status_high && pop_top) || (status_pos < status_high && pop_bottom)) {
		...
		if (pop_top) {
			...
			clutter_actor_set_positionu(filler, CLUTTER_INT_TO_FIXED(0), CLUTTER_INT_TO_FIXED(geo.height));
			clutter_actor_set_sizeu(filler,
						CLUTTER_INT_TO_FIXED(geo.width),
						CLUTTER_FLOAT_TO_FIXED(status_high - status_pos));
		}
...
static void on_fade_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, HDEffectData * data)
{
	float amt, ramt;
	gint alpha;
	ClutterActor *actor;
...
static void on_close_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, HDEffectData * data)
{
	float amt;
	ClutterActor *actor;
	...
	for (i = 0; i < HDCM_UNMAP_PARTICLES; i++)
		if (data->particles[i] && (amtp > 0) && (amtp < 1)) {
			...
			float ang = i * 15 + amtp * 3.141592f / 2;
			...
			clutter_actor_set_opacity(data->particles[i], (int)(255 * opacity));
			clutter_actor_set_scale(data->particles[i], particle_scale, particle_scale);
...
static void on_notification_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, HDEffectData * data)
{
	float now;
	ClutterActor *actor;
	guint width, height;
	...
	if (hd_comp_mgr_is_portrait()
		...
		static struct {
			float x, y;
		} const cpin[] = {	/* Bezier curve control points for the opening part. */
			{185, -88},
			{185, -32},
			{112, 0},
			{-32, 0},
		}, cpout[] = {	/* and for MBWMCompMgrClientEventUnmap. */
			{-32, 0},
			{-176, 0},
			{-478, -32},
			{-478, -88},
		...
		clutter_actor_set_anchor_pointu(actor,
						CLUTTER_FLOAT_TO_FIXED(-bezier(now,
									       curve[0].x, curve[1].x, curve[2].x,
									       curve[3].x)),
						CLUTTER_FLOAT_TO_FIXED(-bezier
								       (now, curve[0].y, curve[1].y, curve[2].y,
									curve[3].y)));
...
static void on_subview_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, HDEffectData * data)
{
	float amt;
	gint n_frames;
...

	{
		...
		if (subview_actor) {
			clutter_actor_set_anchor_pointu(subview_actor,
							CLUTTER_FLOAT_TO_FIXED(-corner_x), CLUTTER_FLOAT_TO_FIXED(0));
...
static void on_rotate_screen_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, HDEffectData * data)
{
...
static void on_screen_size_changed(ClutterActor * stage, GParamSpec * unused, HDEffectData * data)
{
	guint scrw, scrh;
	ClutterActor *actor;
	...
	clutter_actor_set_position(actor, data->geo.x + data->geo.width / 2, data->geo.y + data->geo.height / 2);
}

static void hd_transition_completed(ClutterTimeline * timeline, HDEffectData * data)
{
	gint i;
	...
	if (data->event == MBWMCompMgrClientEventUnmap && data->cclient_actor) {
		...
		if (CLUTTER_IS_CONTAINER(parent))
			clutter_container_remove_actor(CLUTTER_CONTAINER(parent), data->cclient_actor);
	}
...
/*   dump_clutter_tree (CLUTTER_CONTAINER (clutter_stage_get_default()), 0); */
		...
		if (data->particles[i]) {
			...
			if (CLUTTER_IS_CONTAINER(clutter_actor_get_parent(data->particles[i])))
				clutter_container_remove_actor(CLUTTER_CONTAINER
							       (clutter_actor_get_parent(data->particles[i])),
							       data->particles[i]);
			g_object_unref(data->particles[i]);	// unref ourselves
			data->particles[i] = 0;	// for safety, set pointer to 0
		}

	g_signal_handlers_disconnect_by_func(clutter_stage_get_default(), G_CALLBACK(on_screen_size_changed), data);
	...
	if (data->fixup_visibilities && --Transitions_running == 0 && STATE_IS_APP(hd_render_manager_get_state())) {
...
void hd_transition_popup(HdCompMgr * mgr, MBWindowManagerClient * c, MBWMCompMgrClientEvent event)
{
	MBWMCompMgrClutterClient *cclient;
	ClutterActor *actor;
	HDEffectData *data;
	...
	g_signal_connect(data->timeline, "new-frame", G_CALLBACK(on_popup_timeline_new_frame), data);
	g_signal_connect(data->timeline, "completed", G_CALLBACK(hd_transition_completed), data);
	data->geo = geo;
	...
	clutter_rectangle_set_color(CLUTTER_RECTANGLE(data->particles[0]), &col);
...
void hd_transition_fade(HdCompMgr * mgr, MBWindowManagerClient * c, MBWMCompMgrClientEvent event)
{
	MBWMCompMgrClutterClient *cclient;
	HDEffectData *data;
	...
	data->event = event;
	data->cclient = mb_wm_object_ref(MB_WM_OBJECT(cclient));
	data->cclient_actor = g_object_ref(mb_wm_comp_mgr_clutter_client_get_actor(data->cclient));
	data->hmgr = HD_COMP_MGR(mgr);
	...
	if (HD_IS_BANNER_NOTE(c)) {
		...
		data->final_alpha = hd_transition_get_double("fade", "banner_note_alpha", 1.0);
	} else if (HD_IS_INFO_NOTE(c)) {
		clutter_actor_get_geometry(data->cclient_actor, &data->geo);
		data->final_alpha = hd_transition_get_double("fade", "info_note_alpha", 1.0);
	} else
		/* Leave @data->geo 0, we needn't move the actor around. */
		data->final_alpha = 1;

	g_signal_connect(data->timeline, "new-frame", G_CALLBACK(on_fade_timeline_new_frame), data);
	g_signal_connect(data->timeline, "completed", G_CALLBACK(hd_transition_completed), data);
...
void hd_transition_fade_out_loading_screen(ClutterActor * loading_image)
{
	...
	g_signal_connect(data->timeline, "new-frame", G_CALLBACK(on_fade_timeline_new_frame), data);
	g_signal_connect(data->timeline, "completed", G_CALLBACK(hd_transition_completed), data);
	clutter_container_add_actor(hd_render_manager_get_front_group(), loading_image);
	/* first call to stop flicker */
	on_fade_timeline_new_frame(data->timeline, 0, data);
	clutter_timeline_start(data->timeline);
}

void hd_transition_close_app(HdCompMgr * mgr, MBWindowManagerClient * c)
{
	...
	data->cclient_actor = g_object_ref(actor);
	data->hmgr = HD_COMP_MGR(mgr);
	data->timeline = clutter_timeline_new_for_duration(hd_transition_get_int("app_close", "duration", 500));
	g_signal_connect(data->timeline, "new-frame", G_CALLBACK(on_close_timeline_new_frame), data);
	g_signal_connect(clutter_stage_get_default(), "notify::allocation", G_CALLBACK(on_screen_size_changed), data);
	g_signal_connect(data->timeline, "completed", G_CALLBACK(hd_transition_completed), data);
	data->geo = geo;
	...
	for (i = 0; i < HDCM_UNMAP_PARTICLES; ++i) {
		data->particles[i] = hd_clutter_cache_get_texture(HD_THEME_IMG_CLOSING_PARTICLE, TRUE);
		if (data->particles[i]) {
			g_object_ref(data->particles[i]);
			clutter_actor_set_anchor_point_from_gravity(data->particles[i], CLUTTER_GRAVITY_CENTER);
...
void hd_transition_notification(HdCompMgr * mgr, MBWindowManagerClient * c, MBWMCompMgrClientEvent event)
{
	MBWMCompMgrClutterClient *cclient;
	HDEffectData *data;
	...
	g_signal_connect(data->timeline, "new-frame", G_CALLBACK(on_notification_timeline_new_frame), data);
	g_signal_connect(data->timeline, "completed", G_CALLBACK(hd_transition_completed), data);
...
hd_transition_subview(HdCompMgr * mgr,
		      MBWindowManagerClient * subview, MBWindowManagerClient * mainview, MBWMCompMgrClientEvent event)
{
	...
	if (subview == mainview) {	/* This happens sometimes for unknown reason. */
		g_critical("hd_transition_subview: mainview == subview == %p", subview);
		return;
	}
	...
	if (subview_in_trans && mainview_in_trans)
		return;
	if (mainview_in_trans) {	/* Is the mainview we want to leave sliding in? */
		if (event == MBWMCompMgrClientEventMap && (data = HD_COMP_MGR_CLIENT(cclient_mainview)->effect)
		    && data->event == MBWMCompMgrClientEventMap && data->cclient == cclient_mainview) {
			...
			data->cclient_actor = g_object_ref(mb_wm_comp_mgr_clutter_client_get_actor(cclient_subview));
	...
	if (subview_in_trans) {	/* This is almost the same code. */
		...
		if (event == MBWMCompMgrClientEventUnmap && (data = HD_COMP_MGR_CLIENT(cclient_subview)->effect)
		    && data->event == MBWMCompMgrClientEventUnmap && data->cclient2 == cclient_subview) {
			ClutterActor *o;
			...
			data->cclient2_actor = g_object_ref(mb_wm_comp_mgr_clutter_client_get_actor(cclient_mainview));
	...
	data->cclient_actor = g_object_ref(mb_wm_comp_mgr_clutter_client_get_actor(data->cclient));
	data->cclient2 = mb_wm_object_ref(MB_WM_OBJECT(cclient_mainview));
	data->cclient2_actor = g_object_ref(mb_wm_comp_mgr_clutter_client_get_actor(data->cclient2));
	data->hmgr = HD_COMP_MGR(mgr);
	Transitions_running += data->fixup_visibilities = TRUE;
	data->timeline = hd_transition_timeline_new("subview", event, 250);

	g_signal_connect(data->timeline, "new-frame", G_CALLBACK(on_subview_timeline_new_frame), data);
	g_signal_connect(data->timeline, "completed", G_CALLBACK(hd_transition_completed), data);
 ...
 * for subview. */
void hd_transition_stop(HdCompMgr * mgr, MBWindowManagerClient * client)
{
	MBWMCompMgrClutterClient *cclient;
	HDEffectData *data;
	...
	if ((data = HD_COMP_MGR_CLIENT(cclient)->effect)) {
		...
		g_signal_emit_by_name(data->timeline, "new-frame", n_frames, NULL);
...
hd_transition_fade_and_rotate(gboolean first_part,
			      gboolean goto_portrait, GCallback finished_callback, gpointer finished_callback_data)
{
	ClutterColor black = { 0x00, 0x00, 0x00, 0xFF };
	gint use_zaxis = hd_transition_get_int("thp_tweaks", "zaxisrotation", 0);
	HDEffectData *data = g_new0(HDEffectData, 1);
	data->event = first_part ? MBWMCompMgrClientEventMap : MBWMCompMgrClientEventUnmap;
	data->timeline = hd_transition_timeline_new("rotate", data->event, 300);

	g_signal_connect(data->timeline, "new-frame", G_CALLBACK(on_rotate_screen_timeline_new_frame), data);
	g_signal_connect(data->timeline, "completed", G_CALLBACK(hd_transition_completed), data);
	...
	if (!use_zaxis) {
		...
		clutter_actor_set_size(data->particles[0],
				       hd_comp_mgr_get_current_screen_width(), hd_comp_mgr_get_current_screen_height());
		clutter_container_add_actor(CLUTTER_CONTAINER(clutter_stage_get_default()), data->particles[0]);
	...
	if (!goto_portrait && first_part && !use_zaxis) {
		...
		clutter_actor_set_position(data->particles[1], HD_COMP_MGR_LANDSCAPE_HEIGHT, 0);
		...
		clutter_container_add_actor(CLUTTER_CONTAINER(hd_render_manager_get()), data->particles[1]);
...
static void patience(XClientMessageEvent * event, void *unused)
{
	/* Is somebody probing our patience? */
	if (event->message_type != hd_comp_mgr_get_atom(HD_COMP_MGR(Orientation_change.wm->comp_mgr),
							HD_ATOM_MAEMO_ROTATION_PATIENCE))
		return;

	if (event->data.l[0]) {	/* Grant @max:imal patience. */
		gint max;
	...
	} else {		/* Get out of WAIT_FOR_DAMAGES as quickly as possible. */
		if (Orientation_change.patience_requests)
			Orientation_change.patience_requests--;
		if (!Orientation_change.patience_requests && Orientation_change.timeout_id)
			Orientation_change.timeout_id->remaining = 0;
	}
}

static gboolean hd_transition_rotating_fsm(void)
{
	HDRMStateEnum state;
	gboolean change_state;

	g_debug("%s: phase=%d, new_direction=%d, direction=%d", __FUNCTION__,
		Orientation_change.phase, Orientation_change.new_direction, Orientation_change.direction);
	...
	case IDLE:
		...
		tidy_cached_group_set_render_cache(CLUTTER_ACTOR(hd_render_manager_get()), 1);
		tidy_cached_group_set_downsampling_factor(CLUTTER_ACTOR(hd_render_manager_get()), 1);
		...
		hd_util_set_screen_size_property(Orientation_change.wm, Orientation_change.direction == GOTO_PORTRAIT);
	...
	case TRANS_START:
		if (Orientation_change.direction == Orientation_change.new_direction) {
			...
			hd_transition_fade_and_rotate(TRUE, Orientation_change.direction == GOTO_PORTRAIT,
	...
	case FADE_OUT:
		...
		tidy_cached_group_changed(CLUTTER_ACTOR(hd_render_manager_get()));
		tidy_cached_group_set_render_cache(CLUTTER_ACTOR(hd_render_manager_get()), 0);
		tidy_cached_group_set_downsampling_factor(CLUTTER_ACTOR(hd_render_manager_get()), 0);
		...
		if (Orientation_change.direction == Orientation_change.new_direction) {
			...
			clutter_actor_set_allow_redraw(CLUTTER_ACTOR(hd_render_manager_get()), FALSE);
			...
			Orientation_change.root_config_signal_id =
			    g_signal_connect(clutter_stage_get_default(), "notify::width",
					     G_CALLBACK(hd_transition_rotating_fsm), NULL);
			break;
		} else
			Orientation_change.direction = Orientation_change.new_direction;
 ...
 trans_start_error:
		if (Orientation_change.direction != Orientation_change.new_direction) {
			...
			Orientation_change.timeout_id = hptimer_new(Orientation_change.patience_requests
								    ? hd_transition_get_int("rotate",
											    "damage_timeout_max", 1000)
								    : hd_transition_get_int("rotate", "damage_timeout",
											    50),
			...
			Orientation_change.wm->flags &= ~MBWindowManagerFlagLayoutRotated;
			mb_wm_layout_update(Orientation_change.wm->layout);
			if (Orientation_change.phase > TRANS_START) {	/* Fade back in */
				...
				hd_transition_fade_and_rotate(FALSE, Orientation_change.direction == GOTO_PORTRAIT,
							      G_CALLBACK(hd_transition_rotating_fsm), NULL);
	...
	case FADE_IN:
		{
			...
			if (Orientation_change.direction != Orientation_change.new_direction)
				...
				hd_transition_rotating_fsm();
			else {	/* We're finally settled. */
				hd_comp_mgr_reconsider_compositing(Orientation_change.wm->comp_mgr);
...
gboolean hd_transition_rotate_screen(MBWindowManager * wm, gboolean goto_portrait)
{
	...
	if (!cmsg_id)
		cmsg_id = mb_wm_main_context_x_event_handler_add(wm->main_ctx,
								 wm->root_win->xwindow,
								 ClientMessage, (MBWMXEventFunc) patience, NULL);

	Orientation_change.new_direction = goto_portrait ? GOTO_PORTRAIT : GOTO_LANDSCAPE;
	if (Orientation_change.phase == IDLE) {
		if (goto_portrait == hd_comp_mgr_is_portrait()) {
			g_debug("%s: already in %s mode", __FUNCTION__, goto_portrait ? "portrait" : "landscape");
			return FALSE;
		}
 ...
 * is underway.
 */
void hd_transition_rotate_screen_and_change_state(HDRMStateEnum state)
{
...
gboolean hd_transition_rotation_will_change_state(void)
{
	return (Orientation_change.phase == TRANS_START || Orientation_change.phase == FADE_OUT)
	    && Orientation_change.goto_state != HDRM_STATE_UNDEFINED;
}

/* Are we in the middle of a change? */
gboolean hd_transition_is_rotating(void)
{
	return Orientation_change.phase != IDLE;
}

/* Are we about to change to portrait mode? */
gboolean hd_transition_is_rotating_to_portrait(void)
{
	return Orientation_change.phase != IDLE && Orientation_change.direction == GOTO_PORTRAIT;
}
...
gboolean hd_transition_rotate_ignore_damage()
{
	...
	if (Orientation_change.phase == WAIT_FOR_DAMAGES) {
		...
		if (max > 0) {
			...
			remaining = hd_transition_get_int("rotate", "damage_timeout_plus", 50);
 ...
 * are considered. */
gboolean hd_transition_actor_will_go_away(ClutterActor * actor)
{
	HdCompMgrClient *hcmgrc;

	if (!(hcmgrc = g_object_get_data(G_OBJECT(actor), "HD-MBWMCompMgrClutterClient")))
		return FALSE;
	if (!hcmgrc->effect)
		return FALSE;
...
void hd_transition_play_sound(const gchar * fname)
{
	...
	if (millisec > 100)	/* [Bug 105635] */
		g_warning("%s: ca_context_play_full is blocking for %d ms to play %s", __FUNCTION__, millisec, fname);

}

/* We want to call this when the theme changes, as transitions.ini *could*
 * be loaded from the theme. */
void hd_transition_set_file_changed(void)
{
	g_debug("%s: setting transitions.ini modified", __FUNCTION__);
	transitions_ini_is_dirty = 2 * TRUE;
}

static gboolean transitions_ini_changed(GIOChannel * chnl, GIOCondition cond, gpointer unused)
{
...
static GKeyFile *hd_transition_get_keyfile(void)
{
	...
	g_debug("%s: %s %s", __FUNCTION__, transitions_ini_is_dirty ? "reloading" : "loading", fname);

	error = NULL;
	ini = g_key_file_new();
	if (!g_key_file_load_from_file(ini, fname, 0, &error)) {	/* Use the previous @transitions_ini. */
		g_warning("%s: couldn't load %s: %s", __FUNCTION__, fname, error->message);
		if (!transitions_ini)
	...
	if (!transitions_ini_watcher || transitions_ini_is_dirty > TRUE) {
		...
		if (inofd < 0) {
			...
			g_io_add_watch(transitions_ini_watcher, G_IO_IN, transitions_ini_changed, NULL);
		} else if (watch >= 0)
			/* Remove the previous watch. */
			inotify_rm_watch(inofd, watch);

		g_assert(transitions_ini_watcher != NULL);
		watch = inotify_add_watch(inofd, fname, IN_MODIFY | IN_MOVE_SELF | IN_DELETE_SELF);
		if (watch < 0) {
...
gint hd_transition_get_int(const gchar * transition, const char *key, gint default_val)
{
	gint value;
	GError *error;
	GKeyFile *ini;
	...
	if (error) {
		g_debug("couldn't read int %s::%s from transitions.ini: %s", transition, key, error->message);
		g_error_free(error);
		return default_val;
	}
...
gdouble hd_transition_get_double(const gchar * transition, const char *key, gdouble default_val)
{
	gdouble value;
	GError *error;
	GKeyFile *ini;
	...
	if (error) {
		g_debug("couldn't read double %s::%s from transitions.ini: %s", transition, key, error->message);
		g_error_free(error);
		return default_val;
	}
...
gchar *hd_transition_get_string(const gchar * transition, const char *key, gchar * default_val)
{
	gchar *value;
	GError *error;
	GKeyFile *ini;
	...
	if (error) {
		g_debug("couldn't read string %s::%s from transitions.ini: %s", transition, key, error->message);
		g_error_free(error);
...
HdKeyFrameList *hd_transition_get_keyframes(const gchar * transition, const char *key, gchar * default_val)
{
...
void hd_transition_play_tactile(gboolean is_map, MBWMClientType c_type)
{
