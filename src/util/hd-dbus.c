...
static DBusHandlerResult hd_dbus_signal_handler(DBusConnection * conn, DBusMessage * msg, void *data)
{
	HdCompMgr *hmgr = data;

	if (dbus_message_is_signal(msg, APPKILLER_SIGNAL_INTERFACE, APPKILLER_SIGNAL_NAME)) {
	...
	} else if (dbus_message_is_signal(msg, TASKNAV_SIGNAL_INTERFACE, TASKNAV_SIGNAL_NAME)) {
		if (STATE_IS_APP(hd_render_manager_get_state()))
			hd_render_manager_set_state(HDRM_STATE_TASK_NAV);
		return DBUS_HANDLER_RESULT_HANDLED;
	} else if (dbus_message_is_signal(msg, TASKNAV_SIGNAL_INTERFACE, "set_state")) {
		DBusMessageIter args;
		int sigvalue;

		if ((dbus_message_iter_init(msg, &args)) && (DBUS_TYPE_INT32 == dbus_message_iter_get_arg_type(&args))) {
	...
	} else if (dbus_message_is_signal(msg, TASKNAV_SIGNAL_INTERFACE, "activate_window")) {
		DBusMessageIter args;
		int sigvalue;
	...
	} else if (dbus_message_is_signal(msg, TASKNAV_SIGNAL_INTERFACE, "close_window")) {
		DBusMessageIter args;
		int sigvalue;
	...
	} else if (dbus_message_is_signal(msg, TASKNAV_SIGNAL_INTERFACE, "activate_window_time")) {
		DBusMessageIter args;
		int sigvalue;
	...
	} else if (dbus_message_is_signal(msg, TASKNAV_SIGNAL_INTERFACE, "close_window_time")) {
		DBusMessageIter args;
		int sigvalue;
...
static DBusHandlerResult hd_dbus_system_bus_signal_handler(DBusConnection * conn, DBusMessage * msg, void *data)
{
	HdCompMgr *hmgr = data;
	extern MBWindowManager *hd_mb_wm;
	static gboolean call_active;

	if (dbus_message_is_signal(msg, DSME_SIGNAL_INTERFACE, DSME_SHUTDOWN_SIGNAL_NAME)) {
		Window overlay;
		...
		hd_comp_mgr_kill_all_apps(hmgr);
		overlay = mb_wm_comp_mgr_clutter_get_overlay_window(MB_WM_COMP_MGR_CLUTTER(hmgr));
		if (overlay != None) {
		...
		if (dbus_message_get_args(msg, NULL, DBUS_TYPE_STRING, &mode, DBUS_TYPE_INVALID)) {
			if (strcmp(mode, MCE_TK_LOCKED)) {
				...
				hd_dbus_cunt = call_active && (hd_render_manager_get_state()
		...
		if (dbus_message_iter_init(msg, &iter)) {
			...
			if (str) {
				if (strcmp(str, "on") == 0) {
					...
					if (hd_task_navigator_has_notifications()) {	/* (Re)start pulsating if we have notifs. */
		...
		dbus_message_get_args(msg, NULL, DBUS_TYPE_STRING, &state, DBUS_TYPE_INVALID);
		call_active = !strcmp(state, "active");
		hd_dbus_cunt = call_active && hd_dbus_tklock_on && (hd_render_manager_get_state()
								    & (HDRM_STATE_HOME | HDRM_STATE_HOME_PORTRAIT));
	}
...
static void hd_dbus_prevent_display_blanking(void)
{
	DBusMessage *msg;
	dbus_bool_t b;
...
static gboolean display_timeout_f(gpointer unused)
{
...
void hd_dbus_send_desktop_orientation_changed(gboolean to_portrait)
{
	...
	msg =
	    dbus_message_new_signal("/com/nokia/hildon_desktop", "com.nokia.hildon_desktop",
				    "DesktopOrientationChanged");
	if (msg == NULL) {
		g_warning("%s: could not create message", __func__);
		return;
	}
...
DBusConnection *hd_dbus_init(HdCompMgr * hmgr)
{
	...
	if (!connection || !sysbus_conn) {
		...
		/* session bus */
		dbus_bus_add_match(connection, "type='signal', interface='" APPKILLER_SIGNAL_INTERFACE "'", NULL);

		dbus_bus_add_match(connection, "type='signal', interface='" TASKNAV_SIGNAL_INTERFACE "'", NULL);

		dbus_connection_add_filter(connection, hd_dbus_signal_handler, hmgr, NULL);

		/* system bus */
		dbus_bus_add_match(sysbus_conn, "type='signal', interface='" DSME_SIGNAL_INTERFACE "'", NULL);
		dbus_bus_add_match(sysbus_conn,
				   "type='signal',path='" MCE_SIGNAL_PATH "',"
				   "interface='" MCE_SIGNAL_IF "'," "member='tklock_mode_ind'", NULL);
		dbus_bus_add_match(sysbus_conn,
				   "type='signal',path='" MCE_SIGNAL_PATH "',"
				   "interface='" MCE_SIGNAL_IF "'," "member='" MCE_DISPLAY_SIG "'", NULL);
		dbus_bus_add_match(sysbus_conn,
				   "type='signal',path='" MCE_SIGNAL_PATH "',"
				   "interface='" MCE_SIGNAL_IF "'," "member='" MCE_CALL_STATE_SIG "'", NULL);

		dbus_connection_add_filter(sysbus_conn, hd_dbus_system_bus_signal_handler, hmgr, NULL);
	}
