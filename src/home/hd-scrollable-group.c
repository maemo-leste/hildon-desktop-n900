...
G_DEFINE_TYPE_WITH_CODE(HdScrollableGroup, hd_scrollable_group, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(HdScrollableGroup)
			G_IMPLEMENT_INTERFACE(TIDY_TYPE_SCROLLABLE, hd_scrollable_group_iface_init));

/* #GObject overrides {{{ */
static void hd_scrollable_group_get_property(GObject * obj, guint prop_id, GValue * value, GParamSpec * pspec)
{
...
static void hd_scrollable_group_set_property(GObject * obj, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static void
hd_scrollable_group_get_adjustments(TidyScrollable * scrable, TidyAdjustment ** hadjp, TidyAdjustment ** vadjp)
{
...
static void hd_scrollable_group_adjval_changed(HdScrollableGroup * self, GParamSpec * pspec, TidyAdjustment * adj)
{
...
static void hd_scrollable_group_parent_changed(ClutterActor * actor, ClutterActor * unused)
{
	...
	if (parent)
		clutter_actor_get_size(parent, (guint *) & width, (guint *) & height);
	width = CLUTTER_INT_TO_FIXED(width);
	height = CLUTTER_INT_TO_FIXED(height);
	tidy_adjustment_set_valuesx(priv->horizontal.adjustment, 0, 0, width, 1, 1, width);
	tidy_adjustment_set_valuesx(priv->vertical.adjustment, 0, 0, height, 1, 1, height);
}

/* #HdScrollableGroup's "captured-event" handler. */
static gboolean hd_scrollable_group_touched(ClutterActor * actor, ClutterEvent * event)
{
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(actor);
	...
	if (event->type == CLUTTER_BUTTON_PRESS) {
		priv->horizontal.last_position = hd_scrollable_group_get_viewport_x(HD_SCROLLABLE_GROUP(actor));
		priv->vertical.last_position = hd_scrollable_group_get_viewport_y(HD_SCROLLABLE_GROUP(actor));
	}
...
static gboolean
hd_scrollable_group_tick(ClutterTimeline * timeline, guint current, const HdScrollableGroupDirectionInfo * dir)
{
	guint max;
	...
	if (current < max) {
		...
		diff = CLUTTER_FIXED_TO_INT(dir->manual_scroll_to - dir->manual_scroll_from);
		diff *= (1 - cos(t * M_PI)) / 2.0;
		tidy_adjustment_set_valuex(dir->adjustment, dir->manual_scroll_from + CLUTTER_FLOAT_TO_FIXED(diff));
	} else {		/* Make sure we land at @manual_scroll_to at the end. */
		tidy_adjustment_set_valuex(dir->adjustment, dir->manual_scroll_to);
		if (dir->on_manual_scroll_complete)
			dir->on_manual_scroll_complete(CLUTTER_ACTOR(dir->self), dir->on_manual_scroll_complete_param);
	}
...
{
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(self);
	return CLUTTER_FIXED_TO_INT(tidy_adjustment_get_valuex(priv->horizontal.adjustment));
}
...
{
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(self);
	return CLUTTER_FIXED_TO_INT(tidy_adjustment_get_valuex(priv->vertical.adjustment));
}
...
{				/* tidy_adjustment_set_valuex() takes care of proper clamping. */
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(self);
	tidy_adjustment_set_valuex(priv->horizontal.adjustment, CLUTTER_INT_TO_FIXED(x));
}
...
{
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(self);
	tidy_adjustment_set_valuex(priv->vertical.adjustment, CLUTTER_INT_TO_FIXED(y));
}
...
hd_scrollable_group_scroll_viewport(HdScrollableGroup * self,
				    ...
				    gboolean is_relative, gint diff, ClutterEffectCompleteFunc fun, gpointer funparam)
{
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(self);
	...
	dir = which == HD_SCROLLABLE_GROUP_HORIZONTAL ? &priv->horizontal : &priv->vertical;
	...
	tidy_adjustment_get_valuesx(dir->adjustment, &dir->manual_scroll_from, NULL, &upper, NULL, NULL, &page);
	dir->manual_scroll_to = is_relative ? dir->manual_scroll_from + CLUTTER_INT_TO_FIXED(diff)
...
static void hd_scrollable_group_set_new_upper(HdScrollableGroup * self, HdScrollableGroupDirectionInfo * dir)
{
	ClutterFixed current, lower, stepinc, pageinc, pagesize;

	tidy_adjustment_get_valuesx(dir->adjustment, &current, &lower, NULL, &stepinc, &pageinc, &pagesize);
	tidy_adjustment_set_valuesx(dir->adjustment, current, lower, dir->new_upper, stepinc, pageinc, pagesize);
	dir->can_scroll = dir->new_upper > pagesize;
}

/* Tells us just how large the scrollable area is in a direction. */
void hd_scrollable_group_set_real_estate(HdScrollableGroup * self, HdScrollableGroupDirection which, guint upper)
{
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(self);
	HdScrollableGroupDirectionInfo *dir;
	ClutterFixed current, lower, stepinc, pageinc, pagesize;
	...
	if (upper < pagesize) {	/* Invariant #1 violated, align with the top. */
		dir->new_upper = pagesize;
		hd_scrollable_group_scroll_viewport(self, which, FALSE, 0, (ClutterEffectCompleteFunc)
						    hd_scrollable_group_set_new_upper, dir);
		return;
	} else if (upper < current + pagesize) {	/* Invariant #2 violated, align with the bottom. */
		dir->new_upper = upper;
		hd_scrollable_group_scroll_viewport(self, which, FALSE,
						    CLUTTER_FIXED_TO_INT(upper - pagesize), (ClutterEffectCompleteFunc)
						    hd_scrollable_group_set_new_upper, dir);
		return;
	}

	tidy_adjustment_set_valuesx(dir->adjustment, current, lower, upper, stepinc, pageinc, pagesize);
	dir->can_scroll = upper > pagesize;
}

/* ClutterActor::hide signal of one of the scrollbars
 * for tidy_scroll_view_show_scrollbar(). */
static gboolean scrollbar_shown(ClutterActor * sbar, gpointer unused)
...
void tidy_scroll_view_show_scrollbar(TidyScrollView * self, HdScrollableGroupDirection which, gboolean enable)
{
	...
	sbar = which == HD_SCROLLABLE_GROUP_HORIZONTAL ? tidy_scroll_view_get_hscroll_bar(self)
	...
	if (!enable) {		/* Override clutter_actor_show() requests from the signal handler. */
		clutter_actor_hide(sbar);
...
static void hd_scrollable_group_class_init(HdScrollableGroupClass * klass)
{
	...
	g_object_class_override_property(G_OBJECT_CLASS(klass), HD_SCROLLABLE_GROUP_HORIZONTAL, "hadjustment");
	g_object_class_override_property(G_OBJECT_CLASS(klass), HD_SCROLLABLE_GROUP_VERTICAL, "vadjustment");
}
...
static void setup_direction(HdScrollableGroup * self, HdScrollableGroupDirectionInfo * dir)
{
	dir->self = self;
	...
	g_signal_connect_swapped(dir->adjustment, "notify::value",
				 G_CALLBACK(hd_scrollable_group_adjval_changed), self);

	dir->manual_scroll_timeline = clutter_timeline_new(1, clutter_get_default_frame_rate());
	g_signal_connect(dir->manual_scroll_timeline, "new-frame", G_CALLBACK(hd_scrollable_group_tick), dir);
}

static void hd_scrollable_group_init(HdScrollableGroup * self)
{
	HdScrollableGroupPrivate *priv = HD_SCROLLABLE_GROUP_GET_PRIVATE(self);
