...
static void hd_clutter_cache_init(HdClutterCache * cache)
{
	ClutterStage *stage;
	...
	if (stage) {
		clutter_container_add_actor(CLUTTER_CONTAINER(stage), CLUTTER_ACTOR(cache));
	}
}

static void hd_clutter_cache_dispose(GObject * obj)
{
	G_OBJECT_CLASS(hd_clutter_cache_parent_class)->dispose(obj);
}

static void hd_clutter_cache_class_init(HdClutterCacheClass * klass)
{
...
static HdClutterCache *hd_get_clutter_cache()
{
	if (!the_clutter_cache)
		the_clutter_cache = HD_CLUTTER_CACHE(g_object_ref(g_object_new(HD_TYPE_CLUTTER_CACHE, NULL)));
	return the_clutter_cache;
}

static ClutterActor *hd_clutter_cache_get_real_texture(const char *filename, gboolean from_theme)
{
	...
	if (from_theme) {
		...
		theme_path = mb_wm_theme_is_broken()?
		    HD_CLUTTER_CACHE_FALLBACK_THEME_PATH : HD_CLUTTER_CACHE_THEME_PATH;
	...
	if (!texture) {
		...
		filename_alloc = g_malloc(strlen(HD_CLUTTER_CACHE_FALLBACK_THEME_PATH) + strlen(filename) + 1);
		if (!filename_alloc)
			return 0;
...
static ClutterActor *hd_clutter_cache_get_broken_texture()
{
...
ClutterActor *hd_clutter_cache_get_texture(const char *filename, gboolean from_theme)
{
...
ClutterActor *hd_clutter_cache_get_sub_texture(const char *filename, gboolean from_theme, ClutterGeometry * geo)
{
...
ClutterActor *hd_clutter_cache_get_texture_for_area(const char *filename, gboolean from_theme, ClutterGeometry * area)
{
	ClutterGeometry geo = { 0, 0, 0, 0 };
	return hd_clutter_cache_get_sub_texture_for_area(filename, from_theme, &geo, area);
}

/* like hd_clutter_cache_get_sub_texture, but divides up the texture
 * and repositions it so it extends to fill the given area */
ClutterActor *hd_clutter_cache_get_sub_texture_for_area(const char *filename,
							gboolean from_theme,
							ClutterGeometry * geo_, ClutterGeometry * area)
{
	...
	texture = CLUTTER_TEXTURE(hd_clutter_cache_get_real_texture(filename, from_theme));
	if (!texture) {
	...
	if (!extend_x && !extend_y) {
		...
		ClutterActor *actor = hd_clutter_cache_get_sub_texture(filename, from_theme, &geo);
		clutter_actor_set_position(actor, area->x, area->y);
		return actor;
	}
	...
	for (y = 0; y < 3; y++)
		for (x = 0; x < 3; x++) {
			...
			if (geot.width > 0 && geot.height > 0) {
				...
				clutter_container_add_actor(CLUTTER_CONTAINER(group), CLUTTER_ACTOR(tex));
			}
		}
...
static void reload_texture_cb(ClutterActor * child, gpointer data)
{
	gchar *filename;
...
void hd_clutter_cache_theme_changed(void)
{
	...
	clutter_container_foreach(CLUTTER_CONTAINER(the_clutter_cache), reload_texture_cb, 0);
}
