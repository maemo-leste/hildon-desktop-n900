...
G_DEFINE_TYPE_WITH_CODE(HdHomeViewLayout, hd_home_view_layout, G_TYPE_OBJECT, G_ADD_PRIVATE(HdHomeViewLayout));

static rect_t *rect_new(int x1, int y1, int x2, int y2)
{
...
static void rect_free(rect_t * r)
{
	if (r)
		g_slice_free(rect_t, r);
}

static gint rect_cmp(gconstpointer a, gconstpointer b)
{
	const rect_t *r1, *r2;
...
static gboolean rect_subtract(rect_t * r1, rect_t * r2, GList ** l)
{
	if (r1->x1 < r2->x2 && r2->x1 < r1->x2 && r1->y1 < r2->y2 && r2->y1 < r1->y2) {
		...
		if ((r2->y1 - r1->y1) >= MIN_SIZE)
			*l = g_list_insert_sorted(*l, rect_new(r1->x1, r1->y1, r1->x2, r2->y1), rect_cmp);
		/* new south rectangle */
		if ((r1->y2 - r2->y2) >= MIN_SIZE)
			*l = g_list_insert_sorted(*l, rect_new(r1->x1, r2->y2, r1->x2, r1->y2), rect_cmp);
		/* new west rectangle */
		if ((r2->x1 - r1->x1) >= MIN_SIZE)
			*l = g_list_insert_sorted(*l, rect_new(r1->x1, r1->y1, r2->x1, r1->y2), rect_cmp);
		/* new east rectangle */
		if ((r1->x2 - r2->x2) >= MIN_SIZE)
			*l = g_list_insert_sorted(*l, rect_new(r2->x2, r1->y1, r1->x2, r1->y2), rect_cmp);
...
static GList *list_subtract(GList * list, rect_t * r)
{
	GList *new_list = NULL;
	...
	while (list) {
		...
		if (rect_subtract(old, r, &new_list))
			...
			new_list = g_list_insert_sorted(new_list, old, rect_cmp);
	}

	return new_list;
}

static rect_t *list_find(GList * list, rect_t * r)
{
	GList *l;
	...
	for (l = list; l; l = l->next) {
		...
		if ((d->x2 - d->x1) >= (r->x2 - r->x1) && (d->y2 - d->y1) >= (r->y2 - r->y1))
			return d;
	}
...
static layer_t *layer_new(GSList * applets)
{
...
static void hd_home_view_layout_init(HdHomeViewLayout * layout)
{
	layout->priv = hd_home_view_layout_get_instance_private(layout);
}

static void hd_home_view_layout_dispose(GObject * object)
{
...
static void hd_home_view_layout_class_init(HdHomeViewLayoutClass * klass)
{
...
void hd_home_view_layout_arrange_applet(HdHomeViewLayout * layout, GSList * applets, ClutterActor * new_applet)
{
	HdHomeViewLayoutPrivate *priv = layout->priv;
	guint width, height;
	rect_t r;
	...
	for (l = priv->layer; l; l = l->child) {
		...
		if (f) {
			...
			for (k = priv->layer; k; k = k->child) {
				k->rectangles = list_subtract(k->rectangles, &r);
