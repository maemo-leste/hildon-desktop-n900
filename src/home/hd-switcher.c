...
static void hd_switcher_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);

static void hd_switcher_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static void hd_switcher_item_closed(HdSwitcher * switcher, ClutterActor * actor);

static void hd_switcher_group_background_clicked(HdSwitcher * switcher, ClutterActor * actor);

static gboolean hd_switcher_notification_clicked(HdSwitcher * switcher, HdNote * note);
static gboolean hd_switcher_notification_closed(HdSwitcher * switcher, HdNote * note);

static void hd_switcher_launcher_cat_launched(HdLauncher * launcher, HdSwitcher * switcher);
static void hd_switcher_launcher_cat_hidden(HdLauncher * launcher, HdSwitcher * switcher);
static void hd_switcher_launch_app(HdSwitcher * switcher, HdLauncherApp * app, gpointer data);
static void hd_switcher_relaunch_app(HdSwitcher * switcher, HdLauncherApp * app, gpointer data);
static void hd_switcher_loading_fail(HdSwitcher * switcher, HdLauncherApp * app, gpointer data);
static void hd_switcher_app_crashed(HdSwitcher * switcher, HdLauncherApp * app, gpointer data);
static void hd_switcher_insufficient_memory(HdSwitcher * switcher, gboolean waking_up, gpointer data);
static void hd_switcher_zoom_in_complete(ClutterActor * actor, HdSwitcher * switcher);
static MBWindowManagerClient *actor_to_client_window(ClutterActor * clutter_window);

G_DEFINE_TYPE_WITH_CODE(HdSwitcher, hd_switcher, G_TYPE_OBJECT, G_ADD_PRIVATE(HdSwitcher));
...
static void hd_switcher_class_init(HdSwitcherClass * klass)
{
	...
	pspec = g_param_spec_pointer("task-nav",
				     "Task Navigator", "HdTaskNavigator Object", G_PARAM_READWRITE | G_PARAM_CONSTRUCT);

	g_object_class_install_property(object_class, PROP_TASK_NAV, pspec);
}

static void launcher_back_button_clicked(HdLauncher * launcher, gpointer * data)
{
...
static void hd_switcher_constructed(GObject * object)
{
	...
	g_signal_connect_swapped(hd_render_manager_get_title_bar(),
				 "clicked-top-left", G_CALLBACK(hd_switcher_clicked), object);
	g_signal_connect_swapped(hd_render_manager_get_title_bar(),
				 "press-top-left", G_CALLBACK(hd_switcher_press), object);
	g_signal_connect_swapped(hd_render_manager_get_title_bar(),
				 "leave-top-left", G_CALLBACK(hd_switcher_leave), object);
	/* Task Launcher events */
	priv->launcher = hd_launcher_get();
	g_signal_connect(priv->launcher, "launcher-hidden", G_CALLBACK(launcher_back_button_clicked), object);
	g_signal_connect(priv->launcher, "category-launched", G_CALLBACK(hd_switcher_launcher_cat_launched), object);
	g_signal_connect(priv->launcher, "category-hidden", G_CALLBACK(hd_switcher_launcher_cat_hidden), object);
	/* App manager events. */
	g_signal_connect_swapped(hd_app_mgr_get(), "application-launched", G_CALLBACK(hd_switcher_launch_app), object);
	g_signal_connect_swapped(hd_app_mgr_get(), "application-relaunched",
				 G_CALLBACK(hd_switcher_relaunch_app), object);
	g_signal_connect_swapped(hd_app_mgr_get(), "application-loading-fail",
				 G_CALLBACK(hd_switcher_loading_fail), object);
	g_signal_connect_swapped(hd_app_mgr_get(), "application-crashed", G_CALLBACK(hd_switcher_app_crashed), object);
	g_signal_connect_swapped(hd_app_mgr_get(), "not-enough-memory",
				 G_CALLBACK(hd_switcher_insufficient_memory), object);

	/* Task navigator events */
	g_signal_connect_swapped(priv->task_nav, "thumbnail-clicked", G_CALLBACK(hd_switcher_item_selected), object);
	g_signal_connect_swapped(priv->task_nav, "thumbnail-closed", G_CALLBACK(hd_switcher_item_closed), object);
	g_signal_connect_swapped(priv->task_nav, "notification-clicked",
				 G_CALLBACK(hd_switcher_notification_clicked), object);
	g_signal_connect_swapped(priv->task_nav, "notification-closed",
				 G_CALLBACK(hd_switcher_notification_closed), object);
	g_signal_connect_swapped(priv->task_nav, "background-clicked",
				 G_CALLBACK(hd_switcher_group_background_clicked), object);
	...
	if (G_UNLIKELY(error)) {
		...
		priv->hildon_home_proxy = dbus_g_proxy_new_for_name(priv->connection,
								    ...
								    "/com/nokia/HildonHome", "com.nokia.HildonHome");
	}
}
...
static gboolean hd_switcher_press(HdSwitcher * switcher)
{
	...
	if (STATE_IS_APP(hd_render_manager_get_state())) {
		priv->press_timeout = g_timeout_add_seconds(LONG_PRESS_DUR, press_timeout_cb, switcher);
		...
		if (priv->hildon_home_proxy)
			dbus_g_proxy_call_no_reply(priv->hildon_home_proxy, "ShowEditMenu",
						   G_TYPE_UINT, hd_home_get_current_view_id(home), G_TYPE_INVALID);
...
static void hd_switcher_relaunched_app_callback(ClutterActor * actor, HdSwitcherRelaunchAppData * data)
{
...
static void hd_switcher_relaunch_app_callback(HdSwitcherRelaunchAppData * data)
{
	...
	g_signal_handlers_disconnect_by_func(hd_render_manager_get(),
					     G_CALLBACK(hd_switcher_relaunch_app_callback), data);
	...
	if (STATE_IS_TASK_NAV(hd_render_manager_get_state()) && !should_rotate) {
		hd_task_navigator_zoom_in(priv->task_nav, data->actor,
					  (ClutterEffectCompleteFunc) hd_switcher_relaunched_app_callback, data);
	} else
...
static void hd_switcher_relaunch_app(HdSwitcher * switcher, HdLauncherApp * app, gpointer data)
{
	...
	actor = hd_task_navigator_find_app_actor(priv->task_nav, hd_launcher_item_get_id(HD_LAUNCHER_ITEM(app)));
	if (!actor) {
	...
	if (!STATE_IS_TASK_NAV(hd_render_manager_get_state())) {
		g_signal_connect_swapped(hd_render_manager_get(), "transition-complete",
					 G_CALLBACK(hd_switcher_relaunch_app_callback), cb_data);
		...
		hd_task_navigator_zoom_in(priv->task_nav, actor,
					  (ClutterEffectCompleteFunc) hd_switcher_relaunched_app_callback, cb_data);
}
 ...
 * responding.
 */
static void hd_switcher_waking_fail(HdSwitcher * switcher)
{
	GtkWidget *banner;

	banner = hildon_banner_show_information(NULL, NULL, _("ckct_ib_application_loading_failed"));
	hildon_banner_set_timeout(HILDON_BANNER(banner), 6000);
 ...
 */
static gboolean hd_switcher_wakeup_timeout(gpointer data)
{
	HdSwitcher *switcher = HD_SWITCHER(data);
	HdSwitcherPrivate *priv = HD_SWITCHER(switcher)->priv;
 ...
 */
static void hd_switcher_render_manager_notify_state(GObject * gobject, GParamSpec * pspec, gpointer data)
{
	HdSwitcher *switcher = HD_SWITCHER(data);
	HdSwitcherPrivate *priv = HD_SWITCHER(switcher)->priv;

	g_signal_handlers_disconnect_by_func(priv->task_nav, G_CALLBACK(hd_switcher_render_manager_notify_state), data);
...
static void hd_switcher_zoom_in_complete(ClutterActor * actor, HdSwitcher * switcher)
{
	...
	hclient = g_object_get_data(G_OBJECT(actor), "HD-MBWMCompMgrClutterClient");
	if (!hclient) {
	...
	if (!hd_comp_mgr_client_is_hibernating(hclient)) {
		...
		gchar *text = g_strdup_printf(dgettext("maemo-af-desktop",
		 ...
		 */
		priv->wakeup_timeout = g_timeout_add(6000, hd_switcher_wakeup_timeout, switcher);
...
void hd_switcher_item_selected(HdSwitcher * switcher, ClutterActor * actor)
{
	...
	hd_task_navigator_zoom_in(priv->task_nav, actor,
				  (ClutterEffectCompleteFunc) hd_switcher_zoom_in_complete, switcher);
}
				...
				ClutterActor * dialog, MBWindowManagerClient * transfor)
{
	...
	parent = mb_wm_comp_mgr_clutter_client_get_actor(MB_WM_COMP_MGR_CLUTTER_CLIENT(transfor->cm_client));
...
static void hd_switcher_something_removed(void)
{
	if (STATE_IS_TASK_NAV(hd_render_manager_get_state())
...
static void hd_switcher_window_removed(ClutterActor * unused, MBWMCompMgrClutterClient * cmgrcc)
{
	mb_wm_object_unref(MB_WM_OBJECT(cmgrcc));
	hd_switcher_something_removed();
}

void hd_switcher_remove_window_actor(HdSwitcher * switcher, ClutterActor * actor, MBWMCompMgrClutterClient * cmgrcc)
{
	HdSwitcherPrivate *priv = HD_SWITCHER(switcher)->priv;
	...
	hd_task_navigator_remove_window(priv->task_nav, actor,
					(ClutterEffectCompleteFunc) hd_switcher_window_removed, cmgrcc);
}
...
static MBWindowManagerClient *actor_to_client_window(ClutterActor * clutter_window)
{
	...
	cmgrc = g_object_get_data(G_OBJECT(clutter_window), "HD-MBWMCompMgrClutterClient");
