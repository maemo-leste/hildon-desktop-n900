...
static void hd_home_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);

static void hd_home_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static void do_applet_release(HdHome * home, ClutterActor * applet, ClutterButtonEvent * event);
static gboolean do_home_applet_motion(HdHome * home, ClutterActor * applet, int x, int y);
static void update_edge_indication_visibility(HdHome * home, guint8 left_opacity, guint8 right_opacity);
static void update_dbus_interfaces_from_gconf(HdHome * home, HdHomeGconfUpdateMode what);

G_DEFINE_TYPE_WITH_CODE(HdHome, hd_home, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(HdHome));
...
static void hd_home_class_init(HdHomeClass * klass)
{
	...
	pspec = g_param_spec_pointer("hdrm", "HdRenderManager", "--" "--", G_PARAM_WRITABLE);
	g_object_class_install_property(object_class, PROP_HDRM, pspec);
}

static gboolean hd_home_edit_button_clicked(ClutterActor * button, ClutterEvent * event, HdHome * home)
{
...
static void hd_home_live_bg_emit_button1_event(HdHome * home, Window mywindow, int x, int y, int event_type)
{
	...
	while (mywindow) {
		...
		XTranslateCoordinates(wm->xdpy,
				      xev.root, xev.window, xev.x_root, xev.y_root, &xev.x, &xev.y, &mywindow);
	}
	XSendEvent(wm->xdpy, xev.window, True, 0, (XEvent *) & xev);
}

static void hd_home_desktop_do_motion(HdHome * home, int x, int y)
{
	HdHomePrivate *priv = home->priv;
	...
	live_bg = hd_home_view_container_get_live_bg(HD_HOME_VIEW_CONTAINER(priv->view_container));
	if (!live_bg)
		live_bg = hd_home_view_get_live_bg(HD_HOME_VIEW(hd_home_get_current_view(home)));
	if (live_bg)
		hd_home_live_bg_emit_button1_event(home, live_bg->window->xwindow, x, y, MotionNotify);
	...
	if (!STATE_IS_PORTRAIT(hd_render_manager_get_state())
	    || !priv->vertical_scrolling) {	/* Scrolling in landscape mode */
		drag_item->x = x - priv->last_x;
		priv->cumulative_x += drag_item->x;

		DRAG_DEBUG("drag motion %dms, %dx%d -> %d", (int)(drag_item->period * 1000), x, y, drag_item->x);
		...
		if (!priv->moved_over_threshold && ABS(priv->cumulative_x) > HDH_PAN_THRESHOLD) {
		...
		priv->last_x = x;
	} else {		/* Scrolling in portrait mode */
		...
		if (!priv->moved_over_threshold && ABS(priv->cumulative_y) > HDH_PAN_THRESHOLD) {
			priv->moved_over_threshold = TRUE;
			if (priv->press_timeout)
				priv->press_timeout = (g_source_remove(priv->press_timeout), 0);
...
static void hd_home_desktop_motion(XButtonEvent * xev, void *userdata)
{
...
static void hd_home_desktop_do_release(HdHome * home)
{
	HdHomePrivate *priv = home->priv;
	...
	if (priv->desktop_motion_cb)
		mb_wm_main_context_x_event_handler_remove(wm->main_ctx, MotionNotify, priv->desktop_motion_cb);
	...
	if (!priv->long_press && priv->moved_over_threshold) {
		if (ABS(cumulative_value) >= PAN_NEXT_PREVIOUS_PERCENTAGE * HD_COMP_MGR_LANDSCAPE_WIDTH) {	/* */
		   ...
		   priv->initial_x == -1 && priv->initial_y == -1) {
		/*
...
static gboolean press_timeout_cb(gpointer data)
{
	HdHome *home = data;
	HdHomePrivate *priv = home->priv;
	...
	if (priv->pressed_applet)
		do_applet_release(home, priv->pressed_applet, NULL);
	else
...
static Bool hd_home_desktop_do_press(HdHome * home, int x, int y)
{
	HdHomePrivate *priv = home->priv;
	MBWindowManager *wm = MB_WM_COMP_MGR(priv->comp_mgr)->wm;
	GSList *applets, *a;
	...
	if (!applet_hit) {
		...
		if (!live_bg)
			live_bg = hd_home_view_get_live_bg(HD_HOME_VIEW(hd_home_get_current_view(home)));
		if (live_bg)
			hd_home_live_bg_emit_button1_event(home, live_bg->window->xwindow, x, y, ButtonPress);
	}

	priv->long_press = FALSE;
	if (priv->press_timeout)
		priv->press_timeout = (g_source_remove(priv->press_timeout), 0);
	priv->press_timeout = g_timeout_add_seconds(LONG_PRESS_DUR, press_timeout_cb, home);
	...
	priv->desktop_motion_cb =
	    mb_wm_main_context_x_event_handler_add(wm->main_ctx, priv->desktop, MotionNotify, (MBWMXEventFunc)
						   hd_home_desktop_motion, home);
	return True;
}

static Bool hd_home_desktop_release(XButtonEvent * xev, void *userdata)
{
	HdHome *home = userdata;
	HdHomePrivate *priv = home->priv;
	MBWindowManagerClient *live_bg;
	...
	if (!live_bg)
		live_bg = hd_home_view_get_live_bg(HD_HOME_VIEW(hd_home_get_current_view(home)));
	if (live_bg)
		hd_home_live_bg_emit_button1_event(home, live_bg->window->xwindow, xev->x, xev->y, ButtonRelease);

	return True;
}

static Bool hd_home_desktop_press(XButtonEvent * xev, void *userdata)
{
	HdHome *home = userdata;
...
static gboolean is_russian_cyrillic_keyboard(void)
{
	...
	kb_layout = gconf_client_get_string(client, "/apps/osso/inputmethod/int_kb_layout", NULL);
	kb_level_shifted = gconf_client_get_bool(client, "/apps/osso/inputmethod/int_kb_level_shifted", NULL);
...
static guint get_keyval_for_russian_cyrillic_keyboard(GdkKeymap * keymap, guint keycode, GdkModifierType state)
{
	GdkKeymapKey keymap_key;

	gdk_keymap_translate_keyboard_state(keymap,
					    keycode, state, 0, NULL, &keymap_key.group, &keymap_key.level, NULL);

	keymap_key.keycode = keycode;
	keymap_key.level += 4;

	return gdk_keymap_lookup_key(keymap, &keymap_key);
}

static void hd_home_desktop_key_press(XKeyEvent * xev, void *userdata)
{
	...
	if (hd_render_manager_get_state() == HDRM_STATE_LAUNCHER) {
		...
		if (delta && conf_enable_launcher_navigator_accel) {
			...
			char s[16];
			gdk_keymap_translate_keyboard_state(keymap, keycode, xev->state, 0, &keyval, NULL, NULL, NULL);
	...
	if (STATE_IS_TASK_NAV(hd_render_manager_get_state())) {
		...
		if (delta && conf_enable_launcher_navigator_accel) {
			...
			if (xev->state & (FN_MODIFIER | ShiftMask) ||
			    (difftime(now, priv->last_fn_time) < 4 && (priv->fn_state || priv->shift_state))) {
	...
	if (STATE_IS_HOME(hd_render_manager_get_state())) {
		gdk_keymap_translate_keyboard_state(keymap, xev->keycode, xev->state, 0, &keyval, NULL, NULL, NULL);
		...
		if (conf_enable_home_contacts_phone) {
			...
			if (priv->key_sent == KEY_SENT_NONE) {
				if (g_unichar_isdigit(unicode) || unicode == '#' || unicode == '*' || unicode == '+') {
			...
			char s[16];
			gdk_keymap_translate_keyboard_state(keymap,
							    xev->keycode, xev->state, 0, &keyval, NULL, NULL, NULL);
...
static void hd_home_desktop_key_release(XKeyEvent * xev, void *userdata)
{
	HdHome *home = userdata;
	HdHomePrivate *priv = home->priv;
 ...
 */
static gboolean take_screenshot(MBWindowManager * wm, Window xwin, gboolean take)
{
	...
	if (!launcher_app) {
		g_warning("Window 0x%lx did not have an application associated" " with it", client->window->xwindow);
		return FALSE;
	}

	service_name = hd_launcher_app_get_service(launcher_app);
	if (!service_name || strchr(service_name, '/') || service_name[0] == '.') {
		g_warning("Window 0x%lx has no sane service name", client->window->xwindow);
	...
	if (STATE_IS_PORTRAIT(hd_render_manager_get_state()))
		filename = g_strdup_printf("%s/.cache/launch/%s_portrait.pvr", getenv("HOME"), service_name);
	else
		filename = g_strdup_printf("%s/.cache/launch/%s.pvr", getenv("HOME"), service_name);
	...
	if (take) {
		...
		if (g_file_test(filename, G_FILE_TEST_EXISTS)) {
			g_debug("%s: not creating '%s', already exists", __func__, filename);
			g_free(filename);
			return FALSE;
		}

		actor = mb_wm_comp_mgr_clutter_client_get_actor(MB_WM_COMP_MGR_CLUTTER_CLIENT(client->cm_client));
		texture = clutter_group_get_nth_child(CLUTTER_GROUP(actor), 0);
		g_object_get(texture,
			     "pixmap", &pixmap,
			     "pixmap-depth", &depth, "pixmap-width", &width, "pixmap-height", &height, NULL);
...
{
	HdHomePrivate *priv = home->priv;
	hd_home_view_container_set_live_bg(HD_HOME_VIEW_CONTAINER(priv->view_container), client);
}

/* Called when a client message is sent to the root window. */
static void root_window_client_message(XClientMessageEvent * event, HdHome * home)
{
	HdHomePrivate *priv = home->priv;
	MBWindowManager *wm = MB_WM_COMP_MGR(priv->comp_mgr)->wm;
	...
	if (event->message_type == wm->atoms[MBWM_ATOM_NET_CURRENT_DESKTOP]) {
		hd_home_view_container_set_current_view(HD_HOME_VIEW_CONTAINER(priv->view_container), event->data.l[0]);
		return;
	}
#endif
	...
	if (event->message_type == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_LOADING_SCREENSHOT)) {
		...
		reply.xclient.message_type = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_LOADING_SCREENSHOT);
		reply.xclient.format = 32;
		reply.xclient.data.l[0] = event->serial;
		reply.xclient.data.l[1] = take_screenshot(wm, event->data.l[1], event->data.l[0] != 1);

		mb_wm_util_async_trap_x_errors(wm->xdpy);
		XSendEvent(wm->xdpy, reply.xclient.window, False, NoEventMask, &reply);
		XFlush(wm->xdpy);
		mb_wm_util_async_untrap_x_errors();
	}
}

static ClutterActor *hd_home_create_edit_button(void)
{
	ClutterActor *edit_button;
	...
	clutter_container_add(CLUTTER_CONTAINER(edit_button), bg_left, bg_center, bg_right, icon, NULL);
	...
	clutter_actor_set_position(icon, (clutter_actor_get_width(edit_button) - clutter_actor_get_width(icon)) / 2, 0);

	return edit_button;
}

static ClutterActor *create_edge_indicator(HdHome * home, ClutterContainer * edit_group, const gchar * name, gint x)
{
	...
	clutter_actor_set_size(edge_indicator, HD_EDGE_INDICATION_WIDTH, HD_COMP_MGR_LANDSCAPE_HEIGHT);
	clutter_actor_set_position(edge_indicator, x, 0);

	clutter_container_add_actor(CLUTTER_CONTAINER(edit_group), edge_indicator);
...
static void hd_home_constructed(GObject * object)
{
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(object), CLUTTER_ACTOR(priv->front));
	...
	priv->view_container = hd_home_view_container_new(HD_COMP_MGR(priv->comp_mgr), CLUTTER_ACTOR(object));
	...
	if (!priv->vertical_scrolling)
		g_signal_connect_swapped(clutter_stage_get_default(), "notify::allocation",
					 G_CALLBACK(hd_home_resize_view_container), priv->view_container);
	...
	g_signal_connect(priv->edit_button, "button-release-event", G_CALLBACK(hd_home_edit_button_clicked), object);
	...
	priv->edge_indication_left = create_edge_indicator(HD_HOME(object),
							   CLUTTER_CONTAINER(edit_group), "HdHome:left_switch", 0);
	...
	priv->desktop = XCreateWindow(wm->xdpy,
				      ...
				      HD_COMP_MGR_LANDSCAPE_WIDTH,
				      0, CopyFromParent, InputOnly, CopyFromParent, CWEventMask, &attr);
	XSetWMHints(wm->xdpy, priv->desktop, &wmhints);
	mb_wm_rename_window(wm, priv->desktop, "desktop");

	XChangeProperty(wm->xdpy, priv->desktop,
			wm->atoms[MBWM_ATOM_NET_WM_WINDOW_TYPE], XA_ATOM, 32, PropModeReplace, (unsigned char *)
			&wm->atoms[MBWM_ATOM_NET_WM_WINDOW_TYPE_DESKTOP], 1);

	XChangeProperty(wm->xdpy, priv->desktop,
			wm->atoms[MBWM_ATOM_NET_WM_PID],
			XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&our_pid, 1);
	...
	XChangeProperty(wm->xdpy, priv->desktop,
			...
			XA_STRING, 8, PropModeReplace, (unsigned char *)buf, strlen(buf) + 1);

	XMapWindow(wm->xdpy, priv->desktop);

	mb_wm_main_context_x_event_handler_add(wm->main_ctx, priv->desktop, ButtonPress, (MBWMXEventFunc)
					       hd_home_desktop_press, object);

	mb_wm_main_context_x_event_handler_add(wm->main_ctx, priv->desktop, ButtonRelease, (MBWMXEventFunc)
					       hd_home_desktop_release, object);

	mb_wm_main_context_x_event_handler_add(wm->main_ctx, priv->desktop, KeyPress, (MBWMXEventFunc)
					       hd_home_desktop_key_press, object);

	mb_wm_main_context_x_event_handler_add(wm->main_ctx, priv->desktop, KeyRelease, (MBWMXEventFunc)
					       hd_home_desktop_key_release, object);

	mb_wm_main_context_x_event_handler_add(wm->main_ctx, wm->root_win->xwindow, ClientMessage, (MBWMXEventFunc)
					       root_window_client_message, object);
}
...
static void update_dbus_interfaces_from_gconf(HdHome * home, HdHomeGconfUpdateMode what)
{
	...
	if (what == HD_HOME_GCONF_UPDATE_BOTH || what == HD_HOME_GCONF_UPDATE_ABOOK) {
		l_start = l = gconf_client_get_list(gconf_client, ADDRESSBOOK_GCONF_KEY, GCONF_VALUE_STRING, NULL);
		...
		priv->addressbook_proxy = dbus_g_proxy_new_for_name(connection,
								    ...
								    priv->abook_interface[1], priv->abook_interface[0]);
	}

	if (what == HD_HOME_GCONF_UPDATE_BOTH || what == HD_HOME_GCONF_UPDATE_CALLUI) {
		l_start = l = gconf_client_get_list(gconf_client, CALL_UI_GCONF_KEY, GCONF_VALUE_STRING, NULL);
		for (i = 0; l && i < 3; ++i, l = l->next) {
		...
		priv->call_ui_proxy = dbus_g_proxy_new_for_name(connection,
								...
								priv->callui_interface[1], priv->callui_interface[0]);
	}
}

static void dbus_interface_notify_func(GConfClient * client, guint cnxn_id, GConfEntry * entry, gpointer user_data)
{
...
static void hd_home_init(HdHome * self)
{
	...
	priv->show_edit_button_template =
	    clutter_effect_template_new_for_duration(HDH_EDIT_BUTTON_DURATION, CLUTTER_ALPHA_SINE_INC);
	priv->hide_edit_button_template =
	    clutter_effect_template_new_for_duration(HDH_EDIT_BUTTON_DURATION, CLUTTER_ALPHA_SINE_INC);

	/* Listen to gconf notifications */
	gconf_client = gconf_client_get_default();
	gconf_client_add_dir(gconf_client, "/apps/osso/hildon-desktop", GCONF_CLIENT_PRELOAD_NONE, NULL);
	...
	if (!org_freedesktop_DBus_request_name(bus_proxy,
					       HD_HOME_DBUS_NAME, DBUS_NAME_FLAG_DO_NOT_QUEUE, &result, &error)) {
	...
	dbus_g_object_type_install_info(HD_TYPE_HOME, &dbus_glib_hd_home_object_info);

	dbus_g_connection_register_g_object(connection, HD_HOME_DBUS_PATH, G_OBJECT(self));

	g_debug("%s registered to session bus at %s", HD_HOME_DBUS_NAME, HD_HOME_DBUS_PATH);

	gconf_client_notify_add(gconf_client, ADDRESSBOOK_GCONF_KEY, dbus_interface_notify_func, self, NULL, NULL);
	gconf_client_notify_add(gconf_client, CALL_UI_GCONF_KEY, dbus_interface_notify_func, self, NULL, NULL);
...
static void hd_home_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
	...
	case PROP_HDRM:
		g_signal_connect_swapped(g_value_get_pointer(value),
					 "notify::state", G_CALLBACK(hd_home_reset_fn_state), object);
		break;
	default:
...
/* FOR HDRM_STATE_HOME */
static void _hd_home_do_normal_layout(HdHome * home)
{
	HdHomePrivate *priv = home->priv;
...
static void _hd_home_do_edit_layout(HdHome * home)
{
	HdHomePrivate *priv = home->priv;
	...
	update_edge_indication_visibility(home, EDGE_INDICATION_OPACITY_INVISIBLE, EDGE_INDICATION_OPACITY_INVISIBLE);
}
...
void hd_home_update_layout(HdHome * home)
{
	...
	case HDRM_STATE_HOME_EDIT_DLG_PORTRAIT:
		...
		break;
	default:
		g_warning("%s: should only be called for HDRM_STATE_HOME.*", __FUNCTION__);
	}

	for (i = 0; i < MAX_VIEWS; i++) {
		ClutterActor *view;

		view = hd_home_view_container_get_view(HD_HOME_VIEW_CONTAINER(priv->view_container), i);
...
static void hd_home_applet_emit_button_release_event(HdHome * home, ClutterActor * applet, int x, int y)
{
	HdHomePrivate *priv = home->priv;
	MBWindowManager *wm;
	...
	XTranslateCoordinates(wm->xdpy, xev.root, xev.window, xev.x_root, xev.y_root, &xev.x, &xev.y, &mywindow);
	while (mywindow) {
		xev.window = mywindow;
		XTranslateCoordinates(wm->xdpy,
				      xev.root, xev.window, xev.x_root, xev.y_root, &xev.x, &xev.y, &mywindow);
...
static void hd_home_applet_emit_button_press_event(HdHome * home, ClutterActor * applet, int x, int y)
{
	HdHomePrivate *priv = home->priv;
	MBWindowManager *wm;
	MBWMCompMgrClient *cclient;
...
static void hd_home_applet_emit_leave_event(HdHome * home, ClutterActor * applet, int x, int y)
{
	HdHomePrivate *priv = home->priv;
	MBWindowManager *wm;
	MBWMCompMgrClient *cclient;
...
static void hd_home_applet_emit_enter_event(HdHome * home, ClutterActor * applet, int x, int y)
{
	HdHomePrivate *priv = home->priv;
	MBWindowManager *wm;
	MBWMCompMgrClient *cclient;
...
static gboolean hd_home_client_owns_or_child_xwindow(MBWindowManagerClient * client, Window xwindow)
{
	...
	s = XQueryTree(client->wmref->xdpy, client->window->xwindow, &root, &parent, &children, &nchildren);
	mb_wm_util_async_untrap_x_errors();
...
static gboolean hd_home_applet_press(ClutterActor * applet, ClutterButtonEvent * event, HdHome * home)
{
	HdHomePrivate *priv = home->priv;
	MBWMCompMgrClient *cclient;
	MBWindowManagerClient *client;
	...
	cclient = g_object_get_data(G_OBJECT(applet), "HD-MBWMCompMgrClutterClient");
	if (cclient && (client = cclient->wm_client) && mb_wm_client_want_focus(client)) {
		Window focused = None;
		int revert_to;
		MBWindowManager *wm;
		wm = MB_WM_COMP_MGR(priv->comp_mgr)->wm;
		XGetInputFocus(wm->xdpy, &focused, &revert_to);
		if (focused == None || !hd_home_client_owns_or_child_xwindow(client, focused))
...
static void do_applet_release(HdHome * home, ClutterActor * applet, ClutterButtonEvent * event)
{
	...
	if (priv->initial_x > -1 && priv->initial_y > -1) {
		...
		if (event && event->source == applet) {
			...
			hd_home_applet_emit_leave_event(home, applet, priv->initial_x, priv->initial_y);
...
static gboolean hd_home_applet_release(ClutterActor * applet, ClutterButtonEvent * event, HdHome * home)
{
	/*
	 * If we are in edit mode the HdHomeView will have to deal with this event.
	 */
...
static gboolean do_home_applet_motion(HdHome * home, ClutterActor * applet, int x, int y)
{
	 ...
	 * -1;
	 */
	if (priv->initial_x == -1 && priv->initial_x == -1)
		return TRUE;

	moved_over_threshold =
	    ABS(priv->initial_x - x) > HDH_CLICK_THRESHOLD || ABS(priv->initial_y - y) > HDH_CLICK_THRESHOLD;
...
static gboolean hd_home_applet_motion(ClutterActor * applet, ClutterMotionEvent * event, HdHome * home)
{
	DRAG_DEBUG("%s. (x, y) = (%d, %d)", __FUNCTION__, event->x, event->y);

	if (!(event->modifier_state & (CLUTTER_BUTTON1_MASK | CLUTTER_BUTTON2_MASK | CLUTTER_BUTTON2_MASK)))
		return FALSE;
...
void hd_home_add_applet(HdHome * home, ClutterActor * applet)
{
	...
	view_id = gconf_client_get_int(client, view_key, &error);
	view_id--;

	if (error) {
		g_warning("%s. Could not get view for applet %s. %d. %s.",
			  __FUNCTION__, wm_applet->applet_id, error->code, error->message);
		g_clear_error(&error);
	}
	 ...
	 */
	g_signal_connect(applet, "button-press-event", G_CALLBACK(hd_home_applet_press), home);
	g_signal_connect(applet, "button-release-event", G_CALLBACK(hd_home_applet_release), home);
	g_signal_connect(applet, "motion-event", G_CALLBACK(hd_home_applet_motion), home);

	if (view_id < 0 || view_id >= MAX_VIEWS) {
		GError *error = NULL;

		g_debug("%s. Fix View %d for widget %s.", __FUNCTION__, view_id, wm_applet->applet_id);
	...
	} else if (!hd_home_view_container_get_active(HD_HOME_VIEW_CONTAINER(priv->view_container), view_id)) {
		view_id = hd_home_get_current_view_id(home);
	}

	wm_applet->view_id = view_id;

	g_object_set_data(G_OBJECT(applet), "HD-view-id", GINT_TO_POINTER(view_id));
	...
	if (view_id >= 0) {
		...
		view = hd_home_view_container_get_view(HD_HOME_VIEW_CONTAINER(priv->view_container), view_id);
...
static gboolean hd_home_edit_button_timeout(gpointer data)
{
	HdHome *home = data;
...
static void hd_home_show_edit_button(HdHome * home)
{
	...
	timeline = clutter_effect_move(priv->show_edit_button_template,
				       CLUTTER_ACTOR(priv->edit_button), x, 0, NULL, NULL);

	priv->edit_button_cb = g_timeout_add(HDH_EDIT_BUTTON_TIMEOUT, hd_home_edit_button_timeout, home);

	clutter_timeline_start(timeline);
}

static void hd_home_edit_button_move_completed(ClutterActor * actor, gpointer data)
{
...
void hd_home_hide_edit_button(HdHome * home)
{
	...
	timeline = clutter_effect_move(priv->hide_edit_button_template,
				       ...
				       x, -button_height,
				       (ClutterEffectCompleteFunc) hd_home_edit_button_move_completed, home);
...
void hd_home_set_operator_applet(HdHome * home, ClutterActor * applet)
{
	HdHomePrivate *priv = home->priv;
...
static void update_edge_indication_visibility(HdHome * home, guint8 left_opacity, guint8 right_opacity)
{
	...
	if (left_opacity && hd_home_view_container_get_previous_view(HD_HOME_VIEW_CONTAINER(priv->view_container))) {
	...
	if (right_opacity && hd_home_view_container_get_next_view(HD_HOME_VIEW_CONTAINER(priv->view_container))) {
	...
	if (STATE_IS_PORTRAIT(hd_render_manager_get_state())) {
		if (priv->vertical_scrolling) {
			...
			clutter_actor_set_position(priv->edge_indication_right,
						   HD_COMP_MGR_PORTRAIT_WIDTH - HD_EDGE_INDICATION_WIDTH, 0);
		}
	} else {
		clutter_actor_set_size(priv->edge_indication_left, HD_EDGE_INDICATION_WIDTH,
...
void hd_home_highlight_edge_indication(HdHome * home, gboolean left, gboolean right)
{
	update_edge_indication_visibility(home,
					  left ? EDGE_INDICATION_OPACITY_WIDGET_OVER :
					  EDGE_INDICATION_OPACITY_WIDGET_MOVING,
					  right ? EDGE_INDICATION_OPACITY_WIDGET_OVER :
					  EDGE_INDICATION_OPACITY_WIDGET_MOVING);
}

static gboolean is_status_menu_dialog(MBWindowManagerClient * c)
{
	XClassHint xwinhint;
	...
	if (XGetClassHint(c->window->wm->xdpy, c->window->xwindow, &xwinhint)) {
		if (xwinhint.res_name) {
			status_menu_dialog = !strcmp(xwinhint.res_name, "hildon-status-menu");
...
{
	if (HD_WM_CLIENT_CLIENT_TYPE(c) == HdWmClientTypeAppMenu && !strcmp(c->window->name, "hildon-home"))
		return TRUE;
...
void hd_home_theme_changed(HdHome * home)
{
	...
	clutter_rectangle_set_color(CLUTTER_RECTANGLE(priv->edge_indication_left), &col);
	clutter_rectangle_set_color(CLUTTER_RECTANGLE(priv->edge_indication_right), &col);
}

void hd_home_unregister_applet(HdHome * home, ClutterActor * applet)
{
	HdHomePrivate *priv = home->priv;
	HdHomeView *view;
	...
	view = g_object_get_data(G_OBJECT(applet), "HD-HomeView");
