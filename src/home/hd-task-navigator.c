	...
	union {
		...
		struct {
			...
			struct {
				...
				union {
					struct {	/* north: west, middle, east; middle; south */
...
typedef struct {
	...
	union {
		...
		struct {	/* The final dimensions of @actor. */
			guint width, height;
		};
...
static void clutter_actor_set_rotation_z(ClutterActor * actor, gfloat angle, gfloat z);
...
static gboolean hd_task_navigator_app_portrait_capable(Thumbnail * thumb);
...
static ClutterActor *pixbuf2texture(GdkPixbuf * pixbuf)
{
	GError *err;
	gboolean isok;
	...
	if (gdk_pixbuf_get_colorspace(pixbuf) != GDK_COLORSPACE_RGB
	    ...
	    || gdk_pixbuf_get_n_channels(pixbuf) != (gdk_pixbuf_get_has_alpha(pixbuf) ? 4 : 3)) {
	...
	isok = clutter_texture_set_from_rgb_data(CLUTTER_TEXTURE(texture),
						 ...
						 gdk_pixbuf_get_n_channels(pixbuf), 0, &err);
	if (!isok) {
...
static ClutterActor *load_image(char const *fname, guint aw, guint ah)
{
	GError *err;
	GdkPixbuf *pixbuf;
	gint dx, dy;
	...
	if (scale < 1) {
		...
		tmp = gdk_pixbuf_new(GDK_COLORSPACE_RGB,
				     gdk_pixbuf_get_has_alpha(pixbuf), gdk_pixbuf_get_bits_per_sample(pixbuf), dw, dh);
		gdk_pixbuf_scale(pixbuf, tmp, 0, 0, dw, dh, dx, dy, scale, scale, GDK_INTERP_BILINEAR);
		g_object_unref(pixbuf);
		pixbuf = tmp;
	}
	...
	if (dw < vw || dh < vh) {
		...
		clutter_container_add(CLUTTER_CONTAINER(final), bg, texture, NULL);
	} else
		final = texture;
...
static ClutterActor *load_icon(const gchar * iname, guint isize)
{
	...
	anyad[0] = iname;
	if (!(icinf = gtk_icon_theme_choose_icon(gtk_icon_theme_get_default(), anyad, isize, 0)))
		return NULL;

	icon = (iname = gtk_icon_info_get_filename(icinf)) != NULL ? clutter_texture_new_from_file(iname, NULL) : NULL;
...
static ClutterActor *get_icon(const gchar * iname, guint isize)
{
	...
	if (cache && (icon = g_hash_table_lookup(cache, ikey)) != NULL) {	/* Yeah */
		g_free(ikey);
	} else if ((icon = load_icon(iname, isize)) != NULL) {	/* No, but we could load it. */
		if (!cache)
	...
	clutter_actor_move_anchor_point(icon, (gint) (w - isize) / 2, (gint) (h - isize) / 2);
	return icon;
...
static inline gboolean animation_in_progress(ClutterTimeline * timeline)
{
	return clutter_timeline_is_playing(timeline);
}

/* Stop activities on @timeline as if it were completed normally. */
static void stop_animation(ClutterTimeline * timeline)
{
	guint nframes;
...
static gboolean need_to_animate(ClutterActor * actor)
{
 ...
 * a new. */
static EffectClosure *has_effect(ClutterActor * actor, gconstpointer effectid)
{
	guint i;
...
static EffectClosure *new_effect(ClutterTimeline * timeline, ClutterActor * actor,
				 ...
				 void(*complete_fun)(ClutterTimeline *, EffectClosure *))
{
	...
	if (frame_fun)
		closure->new_frame_cb_id = g_signal_connect(timeline, "new-frame", G_CALLBACK(frame_fun), closure);
	else
		closure->new_frame_cb_id = 0;
	closure->timeline_complete_cb_id = g_signal_connect(timeline, "completed", G_CALLBACK(complete_fun), closure);
 ...
 * without further ado. */
static void free_effect(ClutterTimeline * timeline, EffectClosure * closure)
{
 ...
 */
static EffectClosure *linear_effect(ClutterTimeline * timeline, ClutterActor * actor,
				    void (*frame_fun)(ClutterTimeline *, gint, EffectClosure *),
				    void(*complete_fun)(ClutterTimeline *, EffectClosure *), ...)
{
	guint i;
...
static void call_effect_closure(ClutterTimeline * timeline, EffectCompleteClosure * closure)
{
...
static void
add_effect_closure(ClutterTimeline * timeline, ClutterEffectCompleteFunc fun, ClutterActor * actor, gpointer funparam)
{
	...
	closure->handler_id = g_signal_connect(timeline, "completed", G_CALLBACK(call_effect_closure), closure);
}
...
DEFINE_RMS_EFFECT(move, gint, clutter_actor_get_position, clutter_actor_set_position);
static void check_and_move(ClutterActor * actor, gint xpos_new, gint ypos_new)
{
	EffectClosure *closure;
...
#ifdef __i386__
DEFINE_RMS_EFFECT(resize, guint, clutter_actor_get_size, clutter_actor_set_size);
static void check_and_resize(ClutterActor * actor, gint width_new, gint height_new)
{
	EffectClosure *closure;
...
#else				/* __armel__ */
static void resize_effect_complete(ClutterTimeline * timeline, EffectClosure * closure)
{
	clutter_actor_set_size(closure->actor, closure->width, closure->height);
	free_effect(timeline, closure);
}

static void resize_effect(ClutterTimeline * timeline, ClutterActor * actor, guint wfinal, guint hfinal)
{
	guint width, height;
	EffectClosure *closure;
...
static void resize(ClutterActor * actor, guint width, guint height)
{
	if (need_to_animate(actor))
...
DEFINE_RMS_EFFECT(scale, gdouble, clutter_actor_get_scale, clutter_actor_set_scale);
static void check_and_scale(ClutterActor * actor, gdouble sx_new, gdouble sy_new)
{
	EffectClosure *closure;
...
DEFINE_RMS_EFFECT(rotate_z, gfloat, clutter_actor_get_rotation_z, clutter_actor_set_rotation_z)
static void check_and_rotate_z(ClutterActor * actor, gfloat angle_new, gfloat z_new)
{
	EffectClosure *closure;
	gfloat angle_now;
	gint z_now;
...
static void check_and_clip(ClutterActor * actor, gint appwgw, gint appwgh)
{
	EffectClosure *closure;
...
static void set_clip(ClutterActor * actor, gint appwgw, gint appwgh)
{
	clutter_actor_set_clip(actor, App_window_geometry_x, IS_PORTRAIT ? 0 : App_window_geometry_y, appwgw, appwgh);
}
...
/* Fading effect {{{ */
/* frame_fun of fade() */
static void fade_frame(ClutterTimeline * timeline, gint frame, EffectClosure * closure)
{
...
static void fade_complete(ClutterTimeline * timeline, EffectClosure * closure)
{
	if (closure->finally == FINALLY_HIDE)
		clutter_actor_hide(closure->another_actor);
	else if (closure->finally == FINALLY_REMOVE)
		clutter_container_remove_actor(CLUTTER_CONTAINER(closure->another_actor), closure->actor);
 ...
 * action.
 */
static EffectClosure *fade(ClutterTimeline * timeline, ClutterActor * actor, guint opacity,
			   enum final_fade_action_t finally, ClutterActor * another_actor)
{
	...
	closure = linear_effect(timeline, actor, fade_frame, fade_complete,
				(gdouble) clutter_actor_get_opacity(actor), (gdouble) opacity, NAN);
...
static EffectClosure *fade_for_duration(guint msecs, ClutterActor * actor, guint opacity,
					enum final_fade_action_t finally, ClutterActor * another_actor)
{
	EffectClosure *closure;
	ClutterTimeline *timeline;
...
static void reset_opacity(ClutterActor * actor, guint opacity, gboolean be_shown)
{
	EffectClosure *closure;
...
gdouble turnoff_fun(gdouble x0, gdouble y0, gdouble x1, gdouble y1, gdouble t)
{
	return ((y1 - y0) * cos(t) + (y0 * cos(x1) - y1 * cos(x0))) / (cos(x1) - cos(x0));
}

/* ClutterTimeline::new-frame callback for turnoff_effect(). */
static void turnoff_effect_frame(ClutterTimeline * timeline, gint frame, EffectClosure * closure)
{
	gdouble now;
	...
	if (0.5 <= now) {
		...
		for (i = 0; i < G_N_ELEMENTS(closure->particles); i++) {
			...
			clutter_actor_set_position(closure->particles[i].particle, cos(ang) * rad, sin(ang) * rad);
			clutter_actor_set_scale(closure->particles[i].particle, 1.5 - now, 1.5 - now);
		}
...
static void turnoff_effect_complete(ClutterTimeline * timeline, EffectClosure * closure)
{
	clutter_container_remove_actor(CLUTTER_CONTAINER(Navigator), closure->all_particles);
	free_effect(timeline, closure);
}
 ...
 */
static void turnoff_effect(ClutterTimeline * timeline, ClutterActor * thwin)
{
	guint i;
	gint centerx, centery;
	EffectClosure *closure;

	closure = new_effect(timeline, thwin, turnoff_effect_frame, turnoff_effect_complete);

	/* Scale @thwin in the middle. */
	clutter_actor_move_anchor_point_from_gravity(thwin, CLUTTER_GRAVITY_CENTER);
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(Navigator), closure->all_particles);
	...
	for (i = 0; i < G_N_ELEMENTS(closure->particles); i++) {
		...
		particle = hd_clutter_cache_get_texture(HD_THEME_IMG_CLOSING_PARTICLE, TRUE);
		clutter_actor_set_anchor_point_from_gravity(particle, CLUTTER_GRAVITY_CENTER);
		clutter_container_add_actor(CLUTTER_CONTAINER(closure->all_particles), particle);
...
static ClutterActor *set_label_text_and_color(ClutterActor * label, const char *newtext, const ClutterColor * color)
{
	const gchar *text;
 ...
 * linespace. */
static gint get_real_label_height(ClutterActor * label)
{
	gint height;

	pango_layout_get_pixel_size(clutter_label_get_layout(CLUTTER_LABEL(label)), NULL, &height);
	return height;
}

/* Returns the number of \n-s in @label. */
static guint get_nlines_of_label(ClutterActor * label)
{
	guint i;
	const gchar *text;
...
static void preserve_linespacing(ClutterActor * actor, gpointer unused, gpointer spc)
{
 ...
 * of @actor. */
static void reanchor_on_resize(ClutterActor * actor, gpointer unused, gpointer grv)
{
...
static void hide_when_complete(ClutterActor * actor, ClutterActor * other)
{
...
static void show_when_complete(ClutterActor * actor)
{
	clutter_actor_hide(actor);
	add_effect_closure(Fly_effect_timeline, (ClutterEffectCompleteFunc) clutter_actor_show, actor, NULL);
}

/* Can be called after show_when_complete() is done to fade in $actor,
 * probably a thwin of a nothumb. */
static void fade_in_when_complete(ClutterActor * actor, gpointer msecs)
{
...
gboolean hd_task_navigator_has_window(HdTaskNavigator * self, ClutterActor * win)
{
	const GList *li;
...
ClutterActor *hd_task_navigator_find_app_actor(HdTaskNavigator * self, const gchar * id)
{
	const GList *li;
	const Thumbnail *thumb;
...
static void set_navigator_height(guint hnavigator)
{
	hd_scrollable_group_set_real_estate(Grid, HD_SCROLLABLE_GROUP_VERTICAL, hnavigator);
}
...
static void calc_layout(Layout * lout)
{
	 ...
	 */
	lout->xpos = layout_fun(DESKTOP_WIDTH, lout->thumbsize->width, GRID_HORIZONTAL_GAP, lout->cells_per_row);
	...
	if (NThumbnails <= (IS_PORTRAIT ? 20 : 12))
		lout->ypos = GRID_TOP_MARGIN + layout_fun(DESKTOP_HEIGHT - GRID_TOP_MARGIN,
							  lout->thumbsize->height, GRID_VERTICAL_GAP, nrows_per_page);
	else
...
static void layout_thumb_frame(const Thumbnail * thumb, const Flyops * ops, gboolean landscape)
{
	guint wt, ht, wb, hb;
	...
	ops->scale(thumb->frame.nm, (gdouble) (Thumbsize->width - 2 * wt)
		   / clutter_actor_get_width(thumb->frame.nm), 1);
	ops->scale(thumb->frame.sm, (gdouble) (Thumbsize->width - 2 * wb)
		   / clutter_actor_get_width(thumb->frame.sm), 1);
	ops->scale(thumb->frame.mw, 1, (gdouble) (Thumbsize->height - (ht + hb))
		   / clutter_actor_get_height(thumb->frame.mw));
	ops->scale(thumb->frame.me, 1, (gdouble) (Thumbsize->height - (ht + hb))
		   / clutter_actor_get_height(thumb->frame.me));
	if (landscape) {
		ops->scale(thumb->frame.mep, (gdouble)
...
static void layout_notwin(Thumbnail * thumb, const GtkRequisition * oldthsize, const Flyops * ops)
{
	TNote *tnote;
	...
	reload_icon = !tnote->icon
	    || oldthsize == &Thumbsizes[IS_PORTRAIT].large || Thumbsize == &Thumbsizes[IS_PORTRAIT].large;
	if (reload_icon && tnote->icon) {	/* Kill the icon and reload a different size. */
		clutter_actor_get_position(tnote->icon, &x, &y);
		clutter_container_remove_actor(CLUTTER_CONTAINER(tnote->notwin), tnote->icon);
		tnote->icon = NULL;
	}
	if (!tnote->icon) {
		tnote->icon = get_icon(hd_note_get_icon(tnote->hdnote), isize);
		clutter_container_add_actor(CLUTTER_CONTAINER(tnote->notwin), tnote->icon);
		if (reload_icon)
	...
	clutter_label_set_font_name(CLUTTER_LABEL(tnote->count), THUMBSIZE_IS_LARGE ? LargeSystemFont : SystemFont);
	...
	clutter_label_set_font_name(CLUTTER_LABEL(tnote->time), THUMBSIZE_IS_LARGE ? SystemFont : SmallSystemFont);
	...
	if (hleft < htime) {	/* We must have lost linespacing or we have a 3-line-ling text. */
	...
	if (width > wmax || height > hleft || get_nlines_of_label(tnote->message) > maxmsg) {
		PangoLayout *lout;
		...
		if (pango_layout_get_line_count(lout) > maxmsg) {
			...
			for (iter = pango_layout_get_iter(lout); maxmsg > 1; pango_layout_iter_next_line(iter))
				maxmsg--;
	...
	if (!THUMBSIZE_IS(small))
		hleft -= height;
	if (FLY(ops, smoothly)) {	/* Don't fly if it's (will be shortly) hidden. */
	...
	ops->move(tnote->count, NOTE_MARGINS + xicon + NOTE_ICON_GAP + isize, y + ycount);
	y += isize;
	...
	ops->scale(tnote->background, (gdouble) Thumbsize->width / width, (gdouble) Thumbsize->height / height);
	ops->move(tnote->separator, NOTE_SEPARATOR_PADDING, 0);
	ops->scale(tnote->separator, (gdouble) (Thumbsize->width - 2 * NOTE_SEPARATOR_PADDING)
 ...
 */
static guint layout_thumbs(ClutterActor * newborn)
{
	Layout lout;
	guint maxwtitle;
	const GList *li;
	...
	maxwtitle = Thumbsize->width - (TITLE_LEFT_MARGIN + TITLE_RIGHT_MARGIN + CLOSE_ICON_SIZE);
	...
	for (li = Thumbnails, i = 0; li && (thumb = li->data); li = li->next, i++) {
		...
		if (!(i % lout.cells_per_row)) {
			...
			xthumb = i + lout.cells_per_row <= NThumbnails ? lout.xpos : lout.last_row_xpos;
		}
		...
		ops->resize(thumb->title, maxwtitle, clutter_actor_get_height(thumb->title));
		...
		if (thumb_is_application(thumb)) {
			...
			ops->clip(thumb->prison,
				  appwgw, appwgh + app_geom_fix + (IS_PORTRAIT ? HD_COMP_MGR_TOP_MARGIN : 0));
...
static void layout(ClutterActor * newborn, gboolean newborn_is_notification)
{
...
/* %Thumbnail:s {{{ */
static MBWMClientWindow *actor_to_client_window(ClutterActor * win, const HdCompMgrClient ** hcmgrcp);
static Bool win_title_changed(MBWMClientWindow * win, int unused1, Thumbnail * thumb);
...
static void reset_thumb_title(Thumbnail * thumb)
{
	...
	if (thumb_has_notification(thumb)) {	/* To the notification summary. */
		new_title = hd_note_get_summary(thumb->tnote->hdnote);
		use_markup = FALSE;
	} else if (thumb->win) {	/* Normal case. */
		new_title = thumb->win->name;
		use_markup = thumb->win->name_has_markup;
	} else {		/* Client must be having its sweet dreams in hibernation. */
		new_title = thumb->saved_title;
		use_markup = thumb->title_had_markup;
	}

	g_assert(thumb->title != NULL);
	set_label_text_and_color(thumb->title, new_title, thumb_has_notification(thumb)
...
static void create_thwin(Thumbnail * thumb, ClutterActor * prison)
{
	/* .title */
	...
	clutter_actor_set_position(thumb->title, TITLE_LEFT_MARGIN, TITLE_HEIGHT / 2);
	...
	thumb->close_app_icon = hd_clutter_cache_get_texture("TaskSwitcherThumbnailTitleCloseIcon.png", TRUE);
	clutter_actor_set_anchor_point(thumb->close_app_icon,
				       CLOSE_ICON_SIZE / 2 - CLOSE_AREA_SIZE / 2,
				       CLOSE_ICON_SIZE / 2 - CLOSE_AREA_SIZE / 2);
	thumb->close_notif_icon = hd_clutter_cache_get_texture("TaskSwitcherNotificationThumbnailCloseIcon.png", TRUE);
	clutter_actor_set_anchor_point(thumb->close_notif_icon,
				       CLOSE_ICON_SIZE / 2 - CLOSE_AREA_SIZE / 2,
				       CLOSE_ICON_SIZE / 2 - CLOSE_AREA_SIZE / 2);
	clutter_container_add(CLUTTER_CONTAINER(thumb->close), thumb->close_app_icon, thumb->close_notif_icon, NULL);
	if (thumb_has_notification(thumb))
	...
	clutter_container_add(CLUTTER_CONTAINER(thumb->plate), thumb->title, thumb->close, NULL);
	...
	clutter_container_add(CLUTTER_CONTAINER(thumb->thwin), prison, thumb->plate, NULL);
...
static void free_thumb(Thumbnail * thumb, gboolean animate)
{
	/* This will kill the entire actor hierarchy. */
	if (animate && CLUTTER_ACTOR_IS_VISIBLE(thumb->thwin)) {	/* We may be adding it, no point of animation then. */
		fade_for_duration(NOTIFADE_OUT_DURATION, thumb->thwin, 0, FINALLY_REMOVE, CLUTTER_ACTOR(Grid));

		/* @thumb will be invalid very soon so prevent any signal handlers
		 * doing harm while we're fading out. */
		g_signal_handlers_disconnect_matched(thumb->thwin, G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, thumb);
		g_signal_handlers_disconnect_matched(thumb->close, G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, thumb);
	} else
	...
	if (thumb_is_application(thumb)) {
		...
		if (thumb->cemetery) {
			...
			for (i = 0; i < thumb->cemetery->len; i++)
				g_object_weak_unref(thumb->cemetery->pdata[i],
						    (GWeakNotify) g_ptr_array_remove_fast, thumb->cemetery);
		...
		if (thumb->win)
			mb_wm_object_signal_disconnect(MB_WM_OBJECT(thumb->win), thumb->win_changed_cb_id);
...
static gboolean need_to_load_video(Thumbnail * apthumb)
{
	...
	if (apthumb->video) {
		...
		if (clear_video) {
			clutter_container_remove_actor(CLUTTER_CONTAINER(apthumb->prison), apthumb->video);
			apthumb->video = NULL;
		}
...
static void claim_win(Thumbnail * apthumb)
{
	...
	if (apthumb->cemetery) {
		...
		for (i = 0; i < apthumb->cemetery->len; i++) {
			clutter_actor_reparent(apthumb->cemetery->pdata[i], apthumb->windows);
			clutter_actor_hide(apthumb->cemetery->pdata[i]);
		}
	}

	if (apthumb->dialogs)
		g_ptr_array_foreach(apthumb->dialogs, (GFunc) clutter_actor_reparent, apthumb->windows);
	...
	if (need_to_load_video(apthumb)) {
		...
		if (apthumb->video) {
			...
			clutter_actor_set_position(apthumb->video, App_window_geometry_x, App_window_geometry_y);
			clutter_container_add_actor(CLUTTER_CONTAINER(apthumb->prison), apthumb->video);
		}
	}
...
static void release_win(const Thumbnail * apthumb)
{
	hd_render_manager_return_app(apthumb->apwin);
	if (apthumb->cemetery)
		g_ptr_array_foreach(apthumb->cemetery, (GFunc) hd_render_manager_return_app, NULL);
	if (apthumb->dialogs)
		g_ptr_array_foreach(apthumb->dialogs, (GFunc) hd_render_manager_return_dialog, NULL);
}

/* Hide our .prison behind @tnote->notwin and reset_thumb_title(). */
static void adopt_notification(Thumbnail * apthumb, TNote * tnote)
{
	g_assert(!apthumb->tnote);
	apthumb->tnote = tnote;
	clutter_container_add_actor(CLUTTER_CONTAINER(apthumb->thwin), tnote->notwin);
	clutter_container_raise_child(CLUTTER_CONTAINER(apthumb->thwin), tnote->notwin, apthumb->prison);
	...
	if (hd_task_navigator_is_active()) {
		...
		if (CLUTTER_ACTOR_IS_VISIBLE(apthumb->close_app_icon))
			fade(timeline, apthumb->close_app_icon, 0, FINALLY_HIDE, apthumb->close_app_icon);
		else		/* Reset its opacity to normal. */
			clutter_actor_set_opacity(apthumb->close_app_icon, 0);
		g_object_unref(timeline);
	} else {		/* Make sure all opacities are reset to the normal values. */
...
/* Restore .prison. */
static TNote *orphan_notification(Thumbnail * apthumb, gboolean animate)
{
	TNote *tnote;
	...
	if (!animate || !hd_task_navigator_is_active())
		clutter_container_remove_actor(CLUTTER_CONTAINER(apthumb->thwin), tnote->notwin);
	...
	if (hd_task_navigator_is_active()) {
		...
		fade(timeline, apthumb->close_notif_icon, 0, FINALLY_HIDE, apthumb->close_notif_icon);
		...
		g_object_unref(timeline);
	} else {		/* Reset opacities to normal values. */
		...
		reset_opacity(apthumb->close_app_icon, 255, !apthumb_has_dialogs(apthumb));
	}
...
static Thumbnail *find_by_apwin(ClutterActor * apwin)
{
	GList *li;
	Thumbnail *thumb;
 ...
 * window too.
 */
static Thumbnail *find_dialog(guint * idxp, ClutterActor * dialog, gboolean for_removal)
{
	GList *li;
	Thumbnail *thumb;
	...
	for_each_appthumb(li, thumb) {
		...
		if (!for_removal && thumb->apwin == dialog) {	/* The dialog will be transient for .apwin. */
			if (idxp)
				*idxp = 0;
			return thumb;
		}
 ...
 */
static void zoom_fun(gint * xposp, gint * yposp, gdouble * xscalep, gdouble * yscalep)
{
...
static void zoom_in(const Thumbnail * apthumb)
{
	gint xpos, ypos;
...
static void rezoom(ClutterActor * actor, GParamSpec * unused, const Thumbnail * apthumb)
{
	zoom_in(apthumb);
}
...
static void zoom_in_complete(ClutterActor * navigator, const Thumbnail * apthumb)
{
	g_signal_handlers_disconnect_by_func(apthumb->prison, rezoom, (Thumbnail *) apthumb);
	g_signal_handlers_disconnect_by_func(apthumb->thwin, rezoom, (Thumbnail *) apthumb);
 ...
 */
void
hd_task_navigator_zoom_in(HdTaskNavigator * self, ClutterActor * win, ClutterEffectCompleteFunc fun, gpointer funparam)
{
	...
	if (thumb_has_notification(apthumb)) {	/* fade() robustly to be friendly with ongoing adopt/
						 * orphan_notification() */
		clutter_actor_show(apthumb->prison);
		fade(Zoom_effect_timeline, apthumb->tnote->notwin, 0, FINALLY_REST, NULL);
	}
	 ...
	 */
	g_signal_connect(apthumb->thwin, "notify::allocation", G_CALLBACK(rezoom), (Thumbnail *) apthumb);
	g_signal_connect(apthumb->prison, "notify::scale-x", G_CALLBACK(rezoom), (Thumbnail *) apthumb);
	g_signal_connect(apthumb->prison, "notify::scale-y", G_CALLBACK(rezoom), (Thumbnail *) apthumb);

	/* Clean up, exit and call @fun when then animation is finished. */
	add_effect_closure(Zoom_effect_timeline,
			   (ClutterEffectCompleteFunc) zoom_in_complete, CLUTTER_ACTOR(self), (Thumbnail *) apthumb);
	add_effect_closure(Zoom_effect_timeline, (ClutterEffectCompleteFunc) fun, win, funparam);
	return;
 ...
 * effect completes. */
void
hd_task_navigator_zoom_out(HdTaskNavigator * self, ClutterActor * win, ClutterEffectCompleteFunc fun, gpointer funparam)
{
	g_debug(__FUNCTION__);
	const Thumbnail *apthumb;
	gdouble xscale, yscale;
	...
	clutter_effect_fade(Zoom_effect, apthumb->titlebar, 0,
			    (ClutterEffectCompleteFunc) hide_when_complete, apthumb->titlebar);
	...
	if (thumb_has_notification(apthumb)) {	/* Use robust fade()ing for the same reason as in *_zoom_in(). */
		clutter_actor_show(apthumb->prison);
		clutter_actor_set_opacity(apthumb->tnote->notwin, 0);
		fade(Zoom_effect_timeline, apthumb->tnote->notwin, 255, FINALLY_HIDE, apthumb->prison);
	}
...
static gboolean tnote_matches_thumb(const TNote * tnote, const Thumbnail * thumb);
...
static MBWMClientWindow *actor_to_client_window(ClutterActor * win, const HdCompMgrClient ** hcmgrcp)
{
	...
	cmgrc = g_object_get_data(G_OBJECT(win), "HD-MBWMCompMgrClutterClient");
...
static void appthumb_set_desaturation(const Thumbnail * apthumb, gboolean desaturate)
{
	if (desaturate)
...
static gboolean appthumb_clicked(Thumbnail * apthumb)
{
	...
	if (thumb_has_notification(apthumb))
		g_signal_emit_by_name(Navigator, "notification-clicked", apthumb->tnote->hdnote);
	else
...
static gboolean appthumb_close_clicked(const Thumbnail * apthumb)
{
	if (!STATE_IS_TASK_NAV(hd_render_manager_get_state()))
	...
	if (thumb_has_notification(apthumb))
		g_signal_emit_by_name(Navigator, "notification-closed", apthumb->tnote->hdnote);
	else {
		if (!apthumb_has_dialogs(apthumb)
		...
		g_signal_emit_by_name(Navigator, apthumb_has_dialogs(apthumb)
				      ? "thumbnail-clicked" : "thumbnail-closed", apthumb->apwin);
	}
	return TRUE;
}

/* Called when the %MBWM_WINDOW_PROP_NAME of @win has changed. */
static Bool win_title_changed(MBWMClientWindow * win, int unused1, Thumbnail * apthumb)
{
	if (!thumb_has_notification(apthumb))
		reset_thumb_title(apthumb);
	return True;
 ...
 * with frame graphics. */
static void create_apthumb_frame(Thumbnail * apthumb)
{
	static struct {
		const gchar *fname;
	...
	for (i = 0; i < G_N_ELEMENTS(frames); i++) {
		apthumb->frame.pieces[i] = hd_clutter_cache_get_texture(frames[i].fname, TRUE);
		clutter_actor_set_anchor_point_from_gravity(apthumb->frame.pieces[i], frames[i].gravity);
		clutter_container_add_actor(CLUTTER_CONTAINER(apthumb->frame.all), apthumb->frame.pieces[i]);
	}
...
static Thumbnail *create_appthumb(ClutterActor * apwin)
{
	GList *li;
	...
	apthumb->win_changed_cb_id = mb_wm_object_signal_connect(MB_WM_OBJECT(apthumb->win), MBWM_WINDOW_PROP_NAME,
	...
	apthumb->nodest = hd_util_get_x_window_string_property(apthumb->win->wm, apthumb->win->xwindow,
							       HD_ATOM_NOTIFICATION_THREAD);
	if (!apthumb->nodest) {
		XClassHint xwinhint;

		if (XGetClassHint(apthumb->win->wm->xdpy, apthumb->win->xwindow, &xwinhint)) {
	...
	clutter_actor_set_anchor_point(apthumb->prison, App_window_geometry_x, App_window_geometry_y);
	clutter_actor_set_position(apthumb->prison, PRISON_XPOS, PRISON_YPOS);
	clutter_container_add(CLUTTER_CONTAINER(apthumb->prison), apthumb->titlebar, apthumb->windows, NULL);

	/* Have .thwin created. */
	create_thwin(apthumb, apthumb->prison);
	g_signal_connect_swapped(apthumb->thwin, "button-release-event", G_CALLBACK(appthumb_clicked), apthumb);
	g_signal_connect_swapped(apthumb->close, "button-release-event", G_CALLBACK(appthumb_close_clicked), apthumb);

	/* Add our .frame. */
	create_apthumb_frame(apthumb);
	clutter_container_add_actor(CLUTTER_CONTAINER(apthumb->plate), apthumb->frame.all);
	clutter_actor_lower_bottom(apthumb->frame.all);

	/* Do we have a notification for @apwin? */
	for_each_notification(li, nothumb) {
		if (tnote_matches_thumb(nothumb->tnote, apthumb)) {	/* Yes, steal it from the the @Grid. */
...
static void appthumb_turned_off_1(ClutterActor * unused, Thumbnail * apthumb)
{				/* Byebye @apthumb! */
	release_win(apthumb);
...
static void appthumb_turned_off_2(ClutterActor * unused, MBWMCompMgrClutterClient * cmgrcc)
{
...
static void remove_window_later(ClutterActor * win, EffectCompleteClosure * closure)
{
	hd_task_navigator_remove_window(HD_TASK_NAVIGATOR(Navigator), win, closure->fun, closure->funparam);
	g_slice_free(EffectCompleteClosure, closure);
}
...
hd_task_navigator_remove_window(HdTaskNavigator * self,
				ClutterActor * win, ClutterEffectCompleteFunc fun, gpointer funparam)
{
	g_debug(__FUNCTION__);
	GList *li;
	Thumbnail *apthumb;
	...
	if (animation_in_progress(Zoom_effect_timeline)) {
		...
		closure->fun = fun;
		closure->funparam = funparam;
		add_effect_closure(Zoom_effect_timeline, (ClutterEffectCompleteFunc) remove_window_later, win, closure);
		return;
	}
	...
	for_each_appthumb(li, apthumb)
		...
		break;
	if (!apthumb) {		/* Code bloat is your enemy, right? */
		g_critical("%s: window actor %p not found.  This is most likely "
			   "not a switcher bug, but indicates a stacking problem. " "See? Good.", __FUNCTION__, win);
		return;
	}
	...
	newborn = thumb_has_notification(apthumb)
	    ? add_nothumb(orphan_notification(apthumb, FALSE))->thwin : NULL;
	...
	if (hd_task_navigator_is_active()) {
		...
		if (!(cmgrcc = g_object_get_data(G_OBJECT(apthumb->apwin), "HD-MBWMCompMgrClutterClient"))) {	/* No point in trying to animate, the actor is destroyed. */
		...
		add_effect_closure(Fly_effect_timeline,
				   (ClutterEffectCompleteFunc) appthumb_turned_off_1, apthumb->thwin, apthumb);
		add_effect_closure(Fly_effect_timeline,
				   (ClutterEffectCompleteFunc) appthumb_turned_off_2, apthumb->thwin, cmgrcc);
	} else {
 damage_control:
		release_win(apthumb);
		free_thumb(apthumb, FALSE);
	...
	if (animation_in_progress(Fly_effect_timeline))
		add_effect_closure(Fly_effect_timeline, fun, CLUTTER_ACTOR(self), funparam);
	else if (fun)
 ...
 */
void hd_task_navigator_add_window(HdTaskNavigator * self, ClutterActor * win)
{
	g_debug(__FUNCTION__);
	Thumbnail *apthumb;
	...
	Thumbnails = Notifications ? g_list_insert_before(Thumbnails, Notifications, apthumb)
...
void hd_task_navigator_remove_dialog(HdTaskNavigator * self, ClutterActor * dialog)
{
	g_debug(__FUNCTION__);
	guint i;
	Thumbnail *apthumb;
 ...
 */
void hd_task_navigator_add_dialog(HdTaskNavigator * self, ClutterActor * parent, ClutterActor * dialog)
{
	g_debug(__FUNCTION__);
	guint i;
	Thumbnail *apthumb;
	...
	if (find_dialog(NULL, dialog, FALSE)) {
		g_critical("%s: dialog actor %s added twice", __FUNCTION__, clutter_actor_get_name(dialog));
		return;
	}
...
void hd_task_navigator_hibernate_window(HdTaskNavigator * self, ClutterActor * win)
{
	Thumbnail *apthumb;
	...
	/* Release .win. */
	mb_wm_object_signal_disconnect(MB_WM_OBJECT(apthumb->win), apthumb->win_changed_cb_id);
	apthumb->win = NULL;
}
...
void hd_task_navigator_replace_window(HdTaskNavigator * self, ClutterActor * old_win, ClutterActor * new_win)
{
	g_debug(__FUNCTION__);
	Thumbnail *apthumb;
	gboolean showing;
	...
	if (apthumb->cemetery) {
		...
		if (g_ptr_array_remove_fast(apthumb->cemetery, new_win))
			g_object_weak_unref(G_OBJECT(new_win),
					    (GWeakNotify) g_ptr_array_remove_fast, apthumb->cemetery);
	} else
	...
	g_object_weak_ref(G_OBJECT(old_win), (GWeakNotify) g_ptr_array_remove_fast, apthumb->cemetery);
	...
	if (apthumb->win) {
		...
		apthumb->win_changed_cb_id =
		    mb_wm_object_signal_connect(MB_WM_OBJECT(apthumb->win), MBWM_WINDOW_PROP_NAME,
						(MBWMObjectCallbackFunc) win_title_changed, apthumb);
 ...
 */
void hd_task_navigator_notification_thread_changed(HdTaskNavigator * self, ClutterActor * win, char *nothread)
{
	GList *li;
	gboolean relayout;
	ClutterActor *newborn;
	...
	for_each_thumbnail(li, thumb)
		...
		if (thumb_is_application(thumb)) {	/* It's another application's, take it away. */
			adopt_notification(apthumb, orphan_notification(thumb, FALSE));
			if (thumb->nodest) {	/* Make sure @thumb won't receive our notifications. */
				XFree(thumb->nodest);
				thumb->nodest = NULL;
			}
		} else {	/* Individual notification thumbnail, kidnap it. */
			adopt_notification(apthumb, remove_nothumb(li, FALSE));
			relayout = TRUE;
		}
		break;
...
static gint numstrcmp(const gchar * a, const gchar * b)
{
	gint decision;
...
static Bool tnote_changed(HdNote * hdnote, int unused1, TNote * tnote)
{
	g_debug(__FUNCTION__);
	GList *li;
	Thumbnail *thumb;
	...
	is_more = numstrcmp(clutter_label_get_text(CLUTTER_LABEL(tnote->count)), hd_note_get_count(tnote->hdnote)) < 0;
	set_label_text_and_color(tnote->time, hd_note_get_time(tnote->hdnote), NULL);
	set_label_text_and_color(tnote->count, hd_note_get_count(tnote->hdnote), NULL);
	set_label_text_and_color(tnote->message, hd_note_get_message(tnote->hdnote), NULL);

	if ((iname = hd_note_get_icon(tnote->hdnote)) != NULL) {	/* Replace icon? */
		if (!(oname = clutter_actor_get_name(tnote->icon))
		    || strcmp(iname, oname)) {	/* Ignore further errors. */
			clutter_container_remove_actor(CLUTTER_CONTAINER(tnote->notwin), tnote->icon);
			tnote->icon = get_icon(iname, THUMBSIZE_IS_LARGE ? ICON_FINGER : ICON_STYLUS);
			clutter_container_add_actor(CLUTTER_CONTAINER(tnote->notwin), tnote->icon);
		}
	}
	...
	if (is_more)
		hd_title_bar_set_switcher_pulse(HD_TITLE_BAR(hd_render_manager_get_title_bar()), TRUE);

	return False;
}

/* Returns a %TNote prepared for @hdnote. */
static TNote *create_tnote(HdNote * hdnote)
{
	TNote *tnote;
	...
	tnote->hdnote_changed_cb_id = mb_wm_object_signal_connect(MB_WM_OBJECT(hdnote), HdNoteSignalChanged,
								  (MBWMObjectCallbackFunc) tnote_changed, tnote);

	/* Decoration */
	/* .background */
	tnote->background = hd_clutter_cache_get_texture("TaskSwitcherNotificationThumbnail.png", TRUE);
	tnote->separator = hd_clutter_cache_get_texture("TaskSwitcherNotificationThumbnailSeparator.png", TRUE);

	/* Inners */
	/* .count */
	tnote->count = set_label_text_and_color(clutter_label_new(),
						hd_note_get_count(tnote->hdnote), &NotificationTextColor);

	/* .time */
	tnote->time = set_label_text_and_color(clutter_label_new(),
					       hd_note_get_time(tnote->hdnote), &NotificationSecondaryTextColor);
	clutter_label_set_line_wrap(CLUTTER_LABEL(tnote->time), TRUE);
	clutter_label_set_alignment(CLUTTER_LABEL(tnote->time), PANGO_ALIGN_CENTER);
	set_label_line_spacing(tnote->time, NOTE_TIME_LINESPACING);
	g_signal_connect(tnote->time, "notify::allocation",
			 G_CALLBACK(reanchor_on_resize), GINT_TO_POINTER(CLUTTER_GRAVITY_NORTH));
	if (NOTE_TIME_LINESPACING != 0)
		g_signal_connect(tnote->time, "notify::allocation",
				 G_CALLBACK(preserve_linespacing), GINT_TO_POINTER(NOTE_TIME_LINESPACING));

	/* .message */
	tnote->message = set_label_text_and_color(clutter_label_new(),
						  hd_note_get_message(tnote->hdnote), &NotificationTextColor);
	clutter_label_set_font_name(CLUTTER_LABEL(tnote->message), SmallSystemFont);
	clutter_label_set_line_wrap(CLUTTER_LABEL(tnote->message), TRUE);
	clutter_label_set_alignment(CLUTTER_LABEL(tnote->message), PANGO_ALIGN_CENTER);
	g_signal_connect(tnote->message, "notify::allocation", G_CALLBACK(clip_on_resize), NULL);
	...
	clutter_container_add(CLUTTER_CONTAINER(tnote->notwin),
			      tnote->background, tnote->separator, tnote->count, tnote->time, tnote->message, NULL);
...
static void free_tnote(TNote * tnote)
{
	mb_wm_object_signal_disconnect(MB_WM_OBJECT(tnote->hdnote), tnote->hdnote_changed_cb_id);
...
static gboolean nothumb_clicked(Thumbnail * nothumb)
{
	if (!STATE_IS_TASK_NAV(hd_render_manager_get_state()))
		/* Be consistent with appthumb_clicked(). */
		return TRUE;

	g_signal_emit_by_name(Navigator, "notification-clicked", nothumb->tnote->hdnote);
	return TRUE;
}

/* Called thwn a notification %Thumbnail's close buttin is clicked. */
static gboolean nothumb_close_clicked(Thumbnail * nothumb)
{
	g_assert(thumb_has_notification(nothumb));
	if (!STATE_IS_TASK_NAV(hd_render_manager_get_state()))
		/* Be consistent with appthumb_clicked(). */
		return TRUE;
	g_signal_emit_by_name(Navigator, "notification-closed", nothumb->tnote->hdnote);
	return TRUE;
}
...
static Thumbnail *add_nothumb(TNote * tnote)
{
	...
	g_signal_connect_swapped(nothumb->thwin, "button-release-event", G_CALLBACK(nothumb_clicked), nothumb);
	g_signal_connect_swapped(nothumb->close, "button-release-event", G_CALLBACK(nothumb_close_clicked), nothumb);
...
static TNote *remove_nothumb(GList * li, gboolean destroy_tnote)
{
	...
	if (destroy_tnote) {
		...
		clutter_container_remove_actor(CLUTTER_CONTAINER(nothumb->thwin), nothumb->tnote->notwin);
		tnote = nothumb->tnote;
	}
	nothumb->tnote = NULL;
...
void hd_task_navigator_add_notification(HdTaskNavigator * self, HdNote * hdnote)
{
	g_debug(__FUNCTION__);
	GList *li;
	TNote *tnote;
	...
	for_each_appthumb(li, apthumb) {
		if (tnote_matches_thumb(tnote, apthumb)) {
			if (thumb_has_notification(apthumb)) {
				 ...
				 * @Grid. */
				g_critical("%s: attempt to add more than one notification", __FUNCTION__);
				break;
			}
...
void hd_task_navigator_remove_notification(HdTaskNavigator * self, HdNote * hdnote)
{
	g_debug(__FUNCTION__);
	GList *li;
	Thumbnail *thumb;
	...
	if (thumb_is_notification(thumb)) {	/* @hdinfo is displayed in a thumbnail on its own. */
	...
	if (!hd_task_navigator_has_notifications())
		hd_title_bar_set_switcher_pulse(HD_TITLE_BAR(hd_render_manager_get_title_bar()), FALSE);
}
 ...
 */
static gboolean within_grid(const ClutterButtonEvent * event)
{
	Layout lout;
	gint x, y, n, m;
	...
	if (event->y <= y) {	/* Clicked somewhere in the complete rows. */
		x = lout.xpos;
		n = lout.cells_per_row;
	} else if (m && event->y <= y + lout.vspace) {	/* Clicked somewhere in the incomplete row. */
...
static ClutterActor *clicked_widget(const ClutterButtonEvent * event)
{
...
static void navigator_shown(ClutterActor * navigator, gpointer unused)
{
	g_debug(__FUNCTION__);
	GList *li;
	Thumbnail *thumb;
...
static void navigator_hidden(ClutterActor * navigator, gpointer unused)
{
	g_debug(__FUNCTION__);
	GList *li;
	Thumbnail *thumb;
 ...
 */
static gboolean navigator_touched(ClutterActor * navigator, ClutterEvent * event)
{
	...
	if (event->type == CLUTTER_BUTTON_PRESS) {
		...
		if (now.tv_usec > last_press_time.tv_usec)
			dt = now.tv_usec - last_press_time.tv_usec + (now.tv_sec - last_press_time.tv_sec) * 1000000;
 ...
 * by grid_clicked(). */
static gboolean navigator_clicked(ClutterActor * navigator, ClutterButtonEvent * event)
...
static gboolean scroller_touched(ClutterActor * scroller, const ClutterEvent * event)
...
static gboolean grid_touched(ClutterActor * grid, ClutterEvent * event)
{				/* Don't propagate non-clicks to things within_grid(). */
	return event->type == CLUTTER_BUTTON_RELEASE && within_grid(&event->button)
	    && !hd_scrollable_group_is_clicked(HD_SCROLLABLE_GROUP(grid));
}

/* @Grid::button-release-event handler */
static gboolean grid_clicked(ClutterActor * grid, ClutterButtonEvent * event)
...
static gboolean unclip(ClutterActor * actor, GParamSpec * prop, gpointer unused)
{
...
static ClutterEffectTemplate *new_animation(ClutterTimeline ** timelinep, guint duration)
{
	...
	*timelinep = clutter_timeline_new_for_duration(duration);
	effect = clutter_effect_template_new(*timelinep, CLUTTER_ALPHA_SMOOTHSTEP_INC);
...
static void hd_task_navigator_init(HdTaskNavigator * self)
{
	...
	g_signal_connect(Navigator, "captured-event", G_CALLBACK(navigator_touched), NULL);
	g_signal_connect(Navigator, "button-release-event", G_CALLBACK(navigator_clicked), NULL);
	...
	g_signal_connect(Scroller, "captured-event", G_CALLBACK(scroller_touched), NULL);
	...
	g_signal_connect_after(Grid, "captured-event", G_CALLBACK(grid_touched), NULL);
	g_signal_connect(Grid, "button-release-event", G_CALLBACK(grid_clicked), NULL);
	...
	hd_gtk_style_resolve_logical_color(&NotificationTextColor, "NotificationTextColor");
	hd_gtk_style_resolve_logical_color(&NotificationSecondaryTextColor, "NotificationSecondaryTextColor");

	/* We don't have anything to show yet, so let's hide. */
	clutter_actor_hide(Navigator);
}

static void hd_task_navigator_class_init(HdTaskNavigatorClass * klass)
{
	...
	g_signal_new("thumbnail-clicked", G_TYPE_FROM_CLASS(klass),
		     G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
		     0, NULL, NULL, g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, CLUTTER_TYPE_ACTOR);
...
void hd_task_navigator_rotate_thumbs(void)
{
	...
	while (t && !thumb_has_notification((Thumbnail *) (t->data)) && !thumb_is_notification((Thumbnail *) (t->data)))
		t = g_list_next(t);
...
static gboolean hd_task_navigator_app_portrait_capable(Thumbnail * thumb)
{
	MBWindowManagerClient *c = mb_wm_managed_client_from_xwindow(thumb->win->wm,
								     thumb->win->xwindow);
...
void hd_task_navigator_update_win_orientation(Window xwindow, gboolean portrait)
{
	...
	if (thumb) {
		...
		if (portrait > 0) {
			...
			ops->rotate_z(thumb->windows, 0.0f, 0);
			ops->move(thumb->windows, 0, 0);
			ops->scale(thumb->prison, (gdouble) wprison / appwgw, (gdouble) hprison / appwgh);
		} else {
			...
			ops->scale(thumb->prison,
				   (gdouble) wprison / (appwgw - adj), (gdouble) hprison / (appwgh + adj));
		}

		layout_thumbs(thumb->thwin);
		mb_wm_client_geometry_mark_dirty(mb_wm_managed_client_from_xwindow(thumb->win->wm,
										   thumb->win->xwindow));
	}
}
...
gboolean hd_task_navigator_get_disable_portrait(MBWindowManagerClient * c)
{
	...
	result = XGetWindowProperty(dpy,
				    ...
				    0L, (~0L), False,
				    XA_WINDOW, &actual_type, &actual_format, &num_items, &bytes_left, &leader_data_ptr);
	...
	result = XGetWindowProperty(dpy,
				    *(Window *) leader_data_ptr,
				    ...
				    XA_CARDINAL, &actual_type, &actual_format, &num_items, &bytes_left, &ret_data_ptr);
...
static void hd_task_navigator_set_disable_portrait(Thumbnail * thumb, gboolean disable)
{
	...
	int status = XGetWindowProperty(dpy, thumb->win->xwindow,
					...
					AnyPropertyType, &actual_type, &actual_format, &nitems, &bytes,
					&leader_data_ptr);
	mb_wm_util_async_untrap_x_errors();
