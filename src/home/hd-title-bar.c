...
G_DEFINE_TYPE_WITH_CODE(HdTitleBar, hd_title_bar, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(HdTitleBar));

#define HD_TITLE_BAR_GET_PRIVATE(obj) \
                (hd_title_bar_get_instance_private (obj))

static void hd_title_bar_stage_allocation_changed(HdTitleBar * bar, GParamSpec * unused, ClutterActor * stage);
static void hd_title_bar_add_left_signals(HdTitleBar * bar, ClutterActor * actor);
static void hd_title_bar_add_right_signals(HdTitleBar * bar, ClutterActor * actor);
static void on_switcher_timeline_new_frame(ClutterTimeline * timeline, gint frame_num, HdTitleBar * bar);
static void hd_title_bar_set_full_width(HdTitleBar * bar, gboolean full_size);
static void hd_title_bar_set_button_positions(HdTitleBar * bar);
static void hd_title_bar_set_title(HdTitleBar * bar, const char *title, gboolean has_markup, gboolean waiting);
/* ------------------------------------------------------------------------- */
...
static void hd_title_bar_init(HdTitleBar * bar)
{
	...
	clutter_actor_set_size(actor, hd_comp_mgr_get_current_screen_width(), HD_COMP_MGR_TOP_MARGIN);
	...
	clutter_actor_set_name(CLUTTER_ACTOR(priv->foreground), "HdTitleBar::foreground");
	clutter_container_add_actor(CLUTTER_CONTAINER(bar), CLUTTER_ACTOR(priv->foreground));

	/* Title background */
	priv->title_bg = hd_clutter_cache_get_sub_texture(HD_THEME_IMG_TITLE_BAR, TRUE, &title_bg_size);
	...
	for (i = 0; i < BTN_COUNT; i++) {
		if (BTN_FILENAMES[i]) {
			...
			label = clutter_label_new_full(font_name,
			...
			clutter_actor_set_size(CLUTTER_ACTOR(priv->buttons[i]), w, HD_COMP_MGR_TOP_MARGIN);
			clutter_actor_set_position(label, HD_TITLE_BAR_TEXT_MARGIN, (HD_COMP_MGR_TOP_MARGIN - h) / 2);
		}

		/* Explicitly enable maemo-specific visibility detection to cut down
		 * spurious paints */
		clutter_actor_set_visibility_detect(CLUTTER_ACTOR(priv->buttons[i]), TRUE);
		...
		if (BTN_FLAGS[i] & BTN_FLAG_FOREGROUND)
			clutter_container_add_actor(CLUTTER_CONTAINER(priv->foreground), priv->buttons[i]);
		else
		...
		if (BTN_FLAGS[i] & BTN_FLAG_SET_SIZE) {
			if (!(BTN_FLAGS[i] & BTN_FLAG_ALIGN_RIGHT))
				clutter_actor_set_size(priv->buttons[i],
						       HD_COMP_MGR_TOP_LEFT_BTN_WIDTH, HD_COMP_MGR_TOP_LEFT_BTN_HEIGHT);
			else
				clutter_actor_set_size(priv->buttons[i],
	...
	g_signal_connect(priv->switcher_timeline, "new-frame", G_CALLBACK(on_switcher_timeline_new_frame), bar);

	/* Create progress indicator */
	{
		ClutterGeometry progress_geo = { 0, 0, HD_THEME_IMG_PROGRESS_SIZE, HD_THEME_IMG_PROGRESS_SIZE };
		priv->progress_texture = hd_clutter_cache_get_sub_texture(HD_THEME_IMG_PROGRESS, TRUE, &progress_geo);
		clutter_container_add_actor(CLUTTER_CONTAINER(bar), priv->progress_texture);
		clutter_actor_set_visibility_detect(CLUTTER_ACTOR(priv->progress_texture), TRUE);
		clutter_actor_set_size(priv->progress_texture, HD_THEME_IMG_PROGRESS_SIZE, HD_THEME_IMG_PROGRESS_SIZE);
		clutter_actor_hide(priv->progress_texture);
		/* Create the timeline for animation */
		priv->progress_timeline = g_object_ref(clutter_timeline_new(HD_THEME_IMG_PROGRESS_FRAMES,
		...
		g_signal_connect(priv->progress_timeline, "new-frame",
				 G_CALLBACK(on_decor_progress_timeline_new_frame), priv->progress_texture);
	}

	g_signal_connect_swapped(clutter_stage_get_default(), "notify::allocation",
				 G_CALLBACK(hd_title_bar_stage_allocation_changed), bar);
 ...
 */
static gboolean status_area_is_visible(void)
{
	...
	return status_area && sa_client &&
	    ...
	    sa_client->frame_geometry.x >= 0 && sa_client->frame_geometry.y >= 0;
	/*
...
gint hd_title_bar_get_button_width(HdTitleBar * bar)
{
...
static void hd_title_bar_dispose(GObject * obj)
{
...
static void hd_title_bar_class_init(HdTitleBarClass * klass)
{
...
static void hd_title_bar_set_state_real(HdTitleBar * bar, HdTitleBarVisEnum button)
{
	...
	if (button & HDTB_VIS_FOREGROUND) {
		...
		if (clutter_actor_get_parent(CLUTTER_ACTOR(priv->foreground)) != front) {
			clutter_actor_reparent(CLUTTER_ACTOR(priv->foreground), CLUTTER_ACTOR(front));
			clutter_actor_raise_top(CLUTTER_ACTOR(priv->foreground));
		}
	} else {
		if (clutter_actor_get_parent(CLUTTER_ACTOR(priv->foreground)) != CLUTTER_ACTOR(bar)) {
			clutter_actor_reparent(CLUTTER_ACTOR(priv->foreground), CLUTTER_ACTOR(bar));
			clutter_actor_raise_top(CLUTTER_ACTOR(priv->foreground));
...
void hd_title_bar_set_state(HdTitleBar * bar, HdTitleBarVisEnum button)
{
	if (!HD_IS_TITLE_BAR(bar))
		return;
...
void hd_title_bar_left_pressed(HdTitleBar * bar, gboolean pressed)
{
	HdTitleBarPrivate *priv;
	if (!HD_IS_TITLE_BAR(bar))
		return;
...
static void hd_title_bar_set_full_width(HdTitleBar * bar, gboolean full_size)
{
	...
	if (full_size) {
		...
		clutter_actor_set_width(priv->title_bg, hd_comp_mgr_get_current_screen_width());
	...
	if ((!full_size) || (priv->state & HDTB_VIS_FOREGROUND)) {
		...
		if (status_area_is_visible() && !(priv->state & HDTB_VIS_FOREGROUND)) {
			...
			clutter_actor_show(priv->buttons[BTN_SEPARATOR_LEFT]);
			clutter_actor_set_x(priv->buttons[BTN_SEPARATOR_LEFT], hd_title_bar_get_button_width(bar));
		} else
			clutter_actor_hide(priv->buttons[BTN_SEPARATOR_LEFT]);
		...
		clutter_actor_set_x(priv->buttons[BTN_BG_LEFT_END], left_width - HD_COMP_MGR_TOP_LEFT_BTN_WIDTH);
	 ...
	 * will work */
	clutter_actor_set_size(CLUTTER_ACTOR(bar), hd_comp_mgr_get_current_screen_width(), HD_COMP_MGR_TOP_MARGIN);
}

void hd_title_bar_right_pressed(HdTitleBar * bar, gboolean pressed)
{
	HdTitleBarPrivate *priv;
	if (!HD_IS_TITLE_BAR(bar))
		return;
...
static void hd_title_bar_set_for_edit_mode(HdTitleBar * bar)
{
	HdTitleBarPrivate *priv;
...
static void hd_title_bar_set_for_loading(HdTitleBar * bar)
{
	HdTitleBarPrivate *priv;
...
static void hd_title_bar_set_window(HdTitleBar * bar, MBWindowManagerClient * client)
{
	...
	if (client != NULL) {
		...
		is_waiting = hd_decor_window_is_waiting(client->wmref, client->window->xwindow);
		priv->has_menu_indicator = hd_decor_window_has_menu_indicator(client->wmref, client->window->xwindow);
	...
	if (d->show_title && title && strlen(title))
		hd_title_bar_set_title(bar, title, client->window->name_has_markup, is_waiting);
	else
	...
	for (l = MB_WM_DECOR(decor)->buttons; l; l = l->next) {
		...
		if (HD_THEME_BUTTON_TYPE(button->type) == HdHomeThemeButtonBack)
...
static gint hd_title_bar_get_end_of_title(HdTitleBar * bar, int width, gboolean allow_title_overlap)
{
	HdTitleBarPrivate *priv = bar->priv;
	gint x = 0;
	gint max_x = hd_comp_mgr_get_current_screen_width() - (width + hd_title_bar_get_button_width(bar));
	...
	x = clutter_actor_get_x(CLUTTER_ACTOR(priv->title)) +
	    (int)pango_units_to_double(logical_rect.width) + HD_TITLE_BAR_PROGRESS_MARGIN;
...
static void hd_title_bar_set_title(HdTitleBar * bar, const char *title, gboolean has_markup, gboolean waiting)
{
	...
	if (title) {
		...
		clutter_actor_set_position(CLUTTER_ACTOR(priv->title),
					   x_start + title_margin, (HD_COMP_MGR_TOP_MARGIN - h) / 2);
	...
	if (priv->has_menu_indicator && title && !waiting) {
		...
		if (x >= 0) {
			...
			clutter_actor_set_position(priv->buttons[BTN_MENU_INDICATOR],
						   x, (HD_COMP_MGR_TOP_MARGIN - HD_THEME_IMG_MENU_INDICATOR_SIZE) / 2);
		} else
			clutter_actor_hide(priv->buttons[BTN_MENU_INDICATOR]);
	} else
		clutter_actor_hide(priv->buttons[BTN_MENU_INDICATOR]);
...
void hd_title_bar_set_loading_title(HdTitleBar * bar, const char *title)
{
	HdTitleBarPrivate *priv;
	if (!HD_IS_TITLE_BAR(bar))
		return;
...
static gboolean hd_title_bar_update_idle(HdTitleBar * bar)
{
	HdTitleBarPrivate *priv = bar->priv;
...
void hd_title_bar_update(HdTitleBar * bar)
{
	...
	if (!priv->update_title_bar)
		...
		priv->update_title_bar = g_idle_add_full(G_PRIORITY_DEFAULT + 20,
							 (GSourceFunc) hd_title_bar_update_idle, bar, NULL);
}
...
gboolean hd_title_bar_is_title_bar_decor(HdTitleBar * bar, MBWMDecor * decor)
{
	return (decor->type == MBWMDecorTypeNorth) &&
	    decor->parent_client && (MB_WM_CLIENT_CLIENT_TYPE(decor->parent_client) == MBWMClientTypeApp);
}

ClutterGroup *hd_title_bar_get_foreground_group(HdTitleBar * bar)
{
	if (!HD_IS_TITLE_BAR(bar))
		return 0;
...
void hd_title_bar_set_switcher_pulse(HdTitleBar * bar, gboolean pulse)
{
	HdTitleBarPrivate *priv;
	...
	if (!pulse) {		/* Stop animation and unhilight the tasks button. */
		clutter_timeline_stop(priv->switcher_timeline);
		clutter_actor_set_opacity(priv->buttons[BTN_SWITCHER_HIGHLIGHT], 0);
		priv->state &= ~HDTB_VIS_BTN_SWITCHER_HIGHLIGHT;
	} else if (!clutter_timeline_is_playing(priv->switcher_timeline)) {	/* Be sure not to start overlapping animations. */
...
static void hd_title_bar_set_button_positions(HdTitleBar * bar)
{
	...
	gint offset = HD_COMP_MGR_TOP_LEFT_BTN_WIDTH - hd_title_bar_get_button_width(bar);
	gint i, wscr;
	...
	for (i = 0; i < BTN_COUNT; i++) {
		...
		if (BTN_FLAGS[i] & BTN_FLAG_ALIGN_LEFT)
			clutter_actor_set_x(priv->buttons[i], -button_offset);
	...
	clutter_actor_set_x(priv->buttons[BTN_BG_LEFT_ATTACHED_PRESSED], -offset);
	clutter_actor_set_x(priv->buttons[BTN_BG_ATTACHED], 0);
...
static void hd_title_bar_top_left_clicked(HdTitleBar * bar)
{
	g_signal_emit(bar, signals[CLICKED_TOP_LEFT], 0);
}

static void hd_title_bar_top_left_press(HdTitleBar * bar)
{
...
static void hd_title_bar_top_left_leave(HdTitleBar * bar)
{
...
static void hd_title_bar_top_right_clicked(HdTitleBar * bar)
{
...
static void hd_title_bar_top_right_press(HdTitleBar * bar)
{
...
static void hd_title_bar_top_right_leave(HdTitleBar * bar)
{
...
static void hd_title_bar_add_left_signals(HdTitleBar * bar, ClutterActor * actor)
{
	...
	g_signal_connect_swapped(actor, "button-release-event", G_CALLBACK(hd_title_bar_top_left_clicked), bar);
	g_signal_connect_swapped(actor, "button-press-event", G_CALLBACK(hd_title_bar_top_left_press), bar);
	g_signal_connect_swapped(actor, "leave-event", G_CALLBACK(hd_title_bar_top_left_leave), bar);
}

static void hd_title_bar_add_right_signals(HdTitleBar * bar, ClutterActor * actor)
{
	clutter_actor_set_reactive(actor, TRUE);
	g_signal_connect_swapped(actor, "button-release-event", G_CALLBACK(hd_title_bar_top_right_clicked), bar);
	g_signal_connect_swapped(actor, "button-press-event", G_CALLBACK(hd_title_bar_top_right_press), bar);
	g_signal_connect_swapped(actor, "leave-event", G_CALLBACK(hd_title_bar_top_right_leave), bar);
}

/* Create a fake version of the title bar that can be used in the switcher */
ClutterActor *hd_title_bar_create_fake(gint width)
{
	ClutterActor *group = clutter_group_new();
	clutter_actor_set_name(group, "hd_title_bar_create_fake");
	ClutterActor *title_bar = hd_clutter_cache_get_texture(HD_THEME_IMG_TITLE_BAR, TRUE);
...
void hd_title_bar_get_xy(HdTitleBar * bar, int *x, int *y)
{
