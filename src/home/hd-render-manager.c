...
GType hd_render_manager_state_get_type(void)
{
	...
	if (G_UNLIKELY(gtype == 0)) {
		static GEnumValue values[] = {
			...
			{HDRM_STATE_HOME_EDIT_DLG_PORTRAIT, "HDRM_STATE_HOME_EDIT_DLG_PORTRAIT",
			 "Home edit dialog in portrait mode"},
			{0, NULL, NULL}
		};
...
G_DEFINE_TYPE_WITH_CODE(HdRenderManager, hd_render_manager, TIDY_TYPE_CACHED_GROUP, G_ADD_PRIVATE(HdRenderManager));
...
/* ------------------------------------------------------------------------- */
static void stage_allocation_changed(ClutterActor * actor, GParamSpec * unused, ClutterActor * stage);
static gboolean hd_render_manager_captured_event_cb(ClutterActor * actor, ClutterEvent * event, gpointer * data);

static void on_timeline_blur_new_frame(ClutterTimeline * timeline, gint frame_num, gpointer data);
static void on_timeline_blur_completed(ClutterTimeline * timeline, gpointer data);

static void hd_render_manager_sync_clutter_before(void);
static void hd_render_manager_sync_clutter_after(void);

static const char *hd_render_manager_state_str(HDRMStateEnum state);

static void hd_render_manager_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec);
static void hd_render_manager_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec);
...
static void press_effect_new_frame(ClutterTimeline * timeline, gint n_frame, HdRenderManagerPrivate * priv)
{
...
#define PRESS_ANCHOR 0.0025
	...
	if (time <= 0.15) {
		...
		clutter_actor_set_anchor_point(stage, ax - CLUTTER_FIXED_TO_INT(w), ay - CLUTTER_FIXED_TO_INT(h));
	} else if (sx < 1) {
		clutter_actor_set_scale(stage, sx + PRESS_SCALE, sy + PRESS_SCALE);
					  ...
					  HdLauncher * launcher, HdHome * home, HdTaskNavigator * task_nav)
{
	...
	render_manager = HD_RENDER_MANAGER(g_object_ref(g_object_new(HD_TYPE_RENDER_MANAGER, NULL)));
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(render_manager), CLUTTER_ACTOR(priv->task_nav));
	clutter_actor_move_anchor_point_from_gravity(CLUTTER_ACTOR(priv->task_nav), CLUTTER_GRAVITY_CENTER);

	/* Add the launcher widget. */
	clutter_container_add_actor(CLUTTER_CONTAINER(render_manager), CLUTTER_ACTOR(launcher));
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(priv->home_blur), CLUTTER_ACTOR(priv->home));

	/* Edit button */
	clutter_container_add_actor(CLUTTER_CONTAINER(priv->blur_front), hd_home_get_edit_button(priv->home));
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(priv->blur_front), CLUTTER_ACTOR(priv->title_bar));
...
static void hd_render_manager_finalize(GObject * gobject)
{
...
static void hd_render_manager_dispose(GObject * gobject)
{
	HdRenderManagerPrivate *priv = HD_RENDER_MANAGER_GET_PRIVATE(gobject);
...
static void hd_render_manager_class_init(HdRenderManagerClass * klass)
{
	...
	pspec = g_param_spec_enum("state",
				  ...
				  G_PARAM_READABLE |
				  G_PARAM_WRITABLE | G_PARAM_STATIC_NICK | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
	g_object_class_install_property(gobject_class, PROP_STATE, pspec);
}

static void hd_render_manager_init(HdRenderManager * self)
{
	ClutterActor *stage;
	HdRenderManagerPrivate *priv;
	...
	g_signal_connect_swapped(stage, "notify::allocation", G_CALLBACK(stage_allocation_changed), self);
	...
	g_signal_connect(clutter_stage_get_default(),
			 "captured-event", G_CALLBACK(hd_render_manager_captured_event_cb), self);
	...
	clutter_actor_set_name(CLUTTER_ACTOR(priv->home_blur), "HdRenderManager:home_blur");
	...
	g_signal_connect_swapped(stage, "notify::allocation", G_CALLBACK(stage_allocation_changed), priv->home_blur);
	clutter_container_add_actor(CLUTTER_CONTAINER(self), CLUTTER_ACTOR(priv->home_blur));

	priv->app_top = CLUTTER_GROUP(clutter_group_new());
	clutter_actor_set_name(CLUTTER_ACTOR(priv->app_top), "HdRenderManager:app_top");
	g_signal_connect_swapped(stage, "notify::allocation", G_CALLBACK(stage_allocation_changed), priv->app_top);
	clutter_actor_set_visibility_detect(CLUTTER_ACTOR(priv->app_top), FALSE);
	clutter_container_add_actor(CLUTTER_CONTAINER(self), CLUTTER_ACTOR(priv->app_top));

	priv->front = CLUTTER_GROUP(clutter_group_new());
	clutter_actor_set_name(CLUTTER_ACTOR(priv->front), "HdRenderManager:front");
	g_signal_connect_swapped(stage, "notify::allocation", G_CALLBACK(stage_allocation_changed), priv->front);
	clutter_actor_set_visibility_detect(CLUTTER_ACTOR(priv->front), FALSE);
	clutter_container_add_actor(CLUTTER_CONTAINER(self), CLUTTER_ACTOR(priv->front));

	priv->blur_front = CLUTTER_GROUP(clutter_group_new());
	clutter_actor_set_name(CLUTTER_ACTOR(priv->blur_front), "HdRenderManager:blur_front");
	g_signal_connect_swapped(stage, "notify::allocation", G_CALLBACK(stage_allocation_changed), priv->blur_front);
	...
	clutter_container_add_actor(CLUTTER_CONTAINER(priv->home_blur), CLUTTER_ACTOR(priv->blur_front));
	...
	g_signal_connect(priv->timeline_press, "new-frame", G_CALLBACK(press_effect_new_frame), priv);
	priv->press_effect = FALSE;

	priv->timeline_blur = clutter_timeline_new_for_duration(250);
	g_signal_connect(priv->timeline_blur, "new-frame", G_CALLBACK(on_timeline_blur_new_frame), self);
	g_signal_connect(priv->timeline_blur, "completed", G_CALLBACK(on_timeline_blur_completed), self);
...
static void on_timeline_blur_new_frame(ClutterTimeline * timeline, gint frame_num, gpointer data)
{
	...
	tidy_blur_group_set_blur(CLUTTER_ACTOR(priv->home_blur), priv->home_radius.current);
	tidy_blur_group_set_saturation(CLUTTER_ACTOR(priv->home_blur), priv->home_saturation.current);
	tidy_blur_group_set_brightness(CLUTTER_ACTOR(priv->home_blur), priv->home_brightness.current);
	tidy_blur_group_set_zoom(CLUTTER_ACTOR(priv->home_blur), priv->home_zoom.current);
	...
	if (task_opacity == 0) {
		...
		if (priv->loading_image) {
			clutter_actor_reparent(priv->loading_image, CLUTTER_ACTOR(priv->blur_front));
	...
	clutter_actor_set_scale(CLUTTER_ACTOR(priv->task_nav),
				priv->task_nav_zoom.current, priv->task_nav_zoom.current);
	...
	clutter_actor_set_scale(CLUTTER_ACTOR(home_front), priv->applets_zoom.current, priv->applets_zoom.current);
...
void hd_render_manager_set_blur(HDRMBlurEnum blur)
{
	...
	if (priv->home_radius.b == 0 && priv->home_radius.current != 0)
		tidy_blur_group_set_blur(CLUTTER_ACTOR(render_manager->priv->home_blur), 0);
	...
	if (blur_home) {
		priv->home_saturation.b = hd_transition_get_double("home", "saturation", 1);
		priv->home_brightness.b = hd_transition_get_double("home", "brightness", 1);
		priv->home_radius.b = hd_transition_get_double("home", (zoom_home) ? "radius_more" : "radius", 8);
	}

	if (zoom_home) {
		float zoom = hd_transition_get_double("home", "zoom", 1);
	...
	if (blur & HDRM_ZOOM_FOR_HOME)
		priv->task_nav_zoom.b = hd_transition_get_double("task_nav", "zoom_for_home", 1);
	...
	if (range_equal(&priv->home_radius) &&
	    ...
	    range_equal(&priv->task_nav_zoom) && range_equal(&priv->applets_opacity)) {
		hd_render_manager_sync_clutter_after();
		return;
	}
	...
	clutter_timeline_set_duration(priv->timeline_blur, hd_transition_get_int("blur", "duration", 250));
...
void hd_render_manager_unzoom_background()
{
	HdRenderManagerPrivate *priv = render_manager->priv;
...
static gboolean hd_render_manager_should_status_area_be_visible()
{
	HdRenderManagerPrivate *priv = render_manager->priv;
...
static void hd_render_manager_update_status_area(gboolean has_fullscreen)
{
	...
	if (has_fullscreen || !STATE_SHOW_STATUS_AREA(priv->state)) {
		...
		if (c && c->frame_geometry.y >= 0) {	/* Move SA out of the way. */
		...
		if (c && c->frame_geometry.y < 0) {	/* Restore the position of SA. */
...
void hd_render_manager_sync_clutter_before()
{
	...
	HdTitleBarVisEnum btn_state = hd_title_bar_get_state(priv->title_bar) &
	    ~(HDTB_VIS_BTN_LEFT_MASK | HDTB_VIS_BTN_RIGHT_MASK | HDTB_VIS_FOREGROUND | HDTB_VIS_SMALL_BUTTONS);
	...
	case HDRM_STATE_AFTER_TKLOCK:
		g_error("%s: NEVER supposed to be in HDRM_STATE_UNDEFINED" "or HDRM_STATE_AFTER_TKLOCK", __func__);
		return;
	...
	case HDRM_STATE_TASK_NAV_PORTRAIT:
		btn_state |= HDTB_VIS_BTN_LAUNCHER;
		clutter_actor_show(CLUTTER_ACTOR(priv->home));
		blur |= HDRM_BLUR_HOME | HDRM_ZOOM_FOR_TASK_NAV | HDRM_SHOW_TASK_NAV;
		break;
	...
	case HDRM_STATE_LAUNCHER_PORTRAIT:
		...
		blur |=
		    HDRM_BLUR_HOME |
		    HDRM_ZOOM_FOR_LAUNCHER | (STATE_IS_TASK_NAV(priv->previous_state) ? HDRM_ZOOM_FOR_TASK_NAV : 0);
		break;
	 ...
	 * when needed */
	if (STATE_SHOW_OPERATOR(priv->state) && (priv->previous_state != HDRM_STATE_LAUNCHER_PORTRAIT))
	...
	if (!STATE_BLUR_BUTTONS(priv->state) &&
	    clutter_actor_get_parent(CLUTTER_ACTOR(priv->blur_front)) != CLUTTER_ACTOR(render_manager)) {
		/* raise the blur_front out of the blur group so we can still
		 * see it unblurred */
		clutter_actor_reparent(CLUTTER_ACTOR(priv->blur_front), CLUTTER_ACTOR(render_manager));
		/* lower this below task_nav (see the ordering comments at the top) */
		clutter_actor_lower(CLUTTER_ACTOR(priv->blur_front), CLUTTER_ACTOR(priv->task_nav));
	...
	/* Move the applets out to the front if required */
	{
		...
		if (STATE_HOME_FRONT(priv->state)) {
			if (clutter_actor_get_parent(home_front) != CLUTTER_ACTOR(priv->blur_front)) {
				clutter_actor_reparent(home_front, CLUTTER_ACTOR(priv->blur_front));
				blurred_changed = TRUE;
			}
			clutter_actor_lower_bottom(home_front);
		} else if (clutter_actor_get_parent(home_front) != CLUTTER_ACTOR(priv->home)) {
	...
	if (STATE_IS_APP(priv->state) != STATE_IS_APP(priv->previous_state) || STATE_IS_NON_COMP(priv->previous_state))
...
void hd_render_manager_sync_clutter_after()
{
	...
	if (STATE_BLUR_BUTTONS(priv->state) &&
	    clutter_actor_get_parent(CLUTTER_ACTOR(priv->blur_front)) != CLUTTER_ACTOR(priv->home_blur)) {
		/* raise the blur_front to the top of the home_blur group so
		 * we still see the apps */
		clutter_actor_reparent(CLUTTER_ACTOR(priv->blur_front), CLUTTER_ACTOR(priv->home_blur));
		hd_render_manager_blurred_changed();
	}
}
...
static gboolean hd_render_manager_status_area_clicked(ClutterActor * actor, ClutterEvent * event, gpointer data)
{
	...
	for (c = wm->stack_top; c; c = c->stacked_below) {
		...
		if (c->xwin_modal_blocker) {
			...
			XSendEvent(wm->xdpy, c->xwin_modal_blocker, False, ButtonReleaseMask, (XEvent *) & ev);
...
void hd_render_manager_set_status_area(ClutterActor * item)
{
	...
	if (item) {
		...
		clutter_actor_reparent(priv->status_area, CLUTTER_ACTOR(priv->title_bar));
		clutter_actor_set_reactive(priv->status_area, TRUE);
		g_signal_connect(item, "notify::width", G_CALLBACK(hd_render_manager_place_titlebar_elements), NULL);
		g_signal_connect(item, "button-release-event", G_CALLBACK(hd_render_manager_status_area_clicked), NULL);
	} else {
...
void hd_render_manager_set_loading(ClutterActor * item)
{
	...
	if (priv->loading_image) {
		...
		if (priv->loading_image_parent) {
			clutter_actor_reparent(priv->loading_image, priv->loading_image_parent);
			...
			clutter_container_remove_actor(CLUTTER_CONTAINER(clutter_actor_get_parent(priv->loading_image)),
						       priv->loading_image);
			...
			if (item == NULL && STATE_IS_APP(priv->state) && !CLUTTER_IS_RECTANGLE(priv->loading_image))
...
static void zoom_out_completed(ClutterActor * actor, MBWMCompMgrClutterClient * cmgrcc)
{
...
static MBWindowManagerClient *hd_render_manager_get_visible_client_for_tasknav(MBWindowManager * wm)
{
	MBWindowManagerClient *c;
...
void hd_render_manager_set_state(HDRMStateEnum state)
{
	...
	if (hd_debug_mode_set)
		g_warning("%s -> %s", hd_render_manager_state_str(priv->state), hd_render_manager_state_str(state));
	else
		g_debug("%s: STATE %s -> STATE %s\n", __FUNCTION__,
			hd_render_manager_state_str(priv->state), hd_render_manager_state_str(state));
	...
	if (priv->in_set_state) {
		g_warning("%s: State change ignored as already in " "hd_render_manager_set_state", __FUNCTION__);
		return;
	}
	...
	if (state != priv->state) {
		...
		if ((oldstate == HDRM_STATE_NON_COMPOSITED &&
		    ...
		    (oldstate == HDRM_STATE_NON_COMP_PORT && state != HDRM_STATE_NON_COMPOSITED)) {
		...
		if (STATE_IS_TASK_NAV(state)) {
			if (hd_task_navigator_is_empty() || hd_wm_has_modal_blockers(wm)) {
				...
				g_debug("you must have meant STATE %s -> STATE %s",
					hd_render_manager_state_str(oldstate), hd_render_manager_state_str(state));
				if (state == oldstate)
					goto out;
			} else
				/* unfocus any applet */
		...
		if (STATE_NEED_TASK_NAV(state)) {
			...
			if (STATE_IS_APP(oldstate)) {
				...
				if (mbwmc && HD_IS_APP(mbwmc)) {
					...
					if (actor && hd_task_navigator_has_window(priv->task_nav, actor)) {
				...
				if (actor &&
					...
					hd_task_navigator_zoom_out(priv->task_nav, actor,
								   ...
								   cmgrcc);
				} else if ((oldstate == HDRM_STATE_NON_COMPOSITED
					    && state == HDRM_STATE_TASK_NAV_PORTRAIT)
		...
		if (STATE_IS_LOADING(state) && priv->loading_image) {
			ClutterActor *parent = clutter_actor_get_parent(priv->loading_image);
			/* Keep the loading screen parent to return to it later. */
			priv->loading_image_parent = parent ? CLUTTER_ACTOR(g_object_ref(parent)) : NULL;
			clutter_actor_reparent(priv->loading_image, CLUTTER_ACTOR(priv->blur_front));
			clutter_actor_set_size(priv->loading_image,
					       hd_comp_mgr_get_current_screen_width(),
					       hd_comp_mgr_get_current_screen_height()
					       - HD_COMP_MGR_TOP_MARGIN);
			clutter_actor_set_position(priv->loading_image, 0, HD_COMP_MGR_TOP_MARGIN);
			clutter_actor_show(priv->loading_image);
		}
		...
		if (((oldstate != HDRM_STATE_APP_PORTRAIT && state == HDRM_STATE_APP)
			...
			hd_render_manager_set_state(state == HDRM_STATE_APP
						    ? HDRM_STATE_APP_PORTRAIT : HDRM_STATE_HOME_PORTRAIT);
			return;
		}
		...
		if (oldstate != HDRM_STATE_UNDEFINED)
			hd_transition_rotate_screen(wm, STATE_IS_PORTRAIT(state)
						    && !hd_comp_mgr_is_blacklisted(wm, wm->stack_top));
		...
		if ((state == HDRM_STATE_APP || state == HDRM_STATE_APP_PORTRAIT
		     ...
		     || state == HDRM_STATE_HOME_EDIT_DLG || state == HDRM_STATE_HOME_EDIT_DLG_PORTRAIT)
...
static const char *hd_render_manager_state_str(HDRMStateEnum state)
{
	...
	GEnumValue *state_value = g_enum_get_value(G_ENUM_CLASS(state_class),
						   state);
...
{
	HdRenderManagerPrivate *priv = HD_RENDER_MANAGER_GET_PRIVATE(render_manager);
	...
	if (cc && (mb_wm_comp_mgr_clutter_client_get_flags(cc) & MBWMCompMgrClutterClientEffectRunning))
		return;
	...
	if (clutter_actor_get_parent(actor)) {
		clutter_actor_reparent(actor, CLUTTER_ACTOR(render_manager->priv->home_blur));
...
{
	clutter_actor_reparent(actor, CLUTTER_ACTOR(render_manager->priv->app_top));
	clutter_actor_hide(actor);
}
 ...
 * you can ignore it.
 */
static gboolean hd_render_manager_clip_geo(ClutterGeometry * geo)
{
	guint t;
...
static gboolean hd_render_manager_geo_is_suitable_for_screen(const ClutterGeometry * geo, guint scrw, guint scrh)
{
 ...
 */
static void hd_render_manager_get_geo_for_current_screen(ClutterActor * actor, ClutterGeometry * geo)
{
	guint scrw, scrh;
...
void hd_render_manager_restack()
{
	...
	for (i = 0,
	     ...
	     child; child = clutter_group_get_nth_child(CLUTTER_GROUP(priv->home_blur), ++i))
	...
	for (c = wm->stack_bottom; c; c = c->stacked_above) {
		...
		if (c->cm_client && c->desktop >= 0) {	/* FIXME: should check against
			 ...
			 */
			if (hd_render_manager_should_ignore_cm_client(MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client))
			    || c->window->live_background)
				continue;

			ClutterActor *desktop =
			    mb_wm_comp_mgr_clutter_get_nth_desktop(MB_WM_COMP_MGR_CLUTTER(priv->comp_mgr), c->desktop);
			actor = mb_wm_comp_mgr_clutter_client_get_actor(MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client));
			...
			if (mb_wm_client_is_unmap_confirmed(c)) {
				...
				g_debug("%s: skip unmapped client '%s' (actor '%s')\n",
					__func__, mb_wm_client_get_name(c), actor ? clutter_actor_get_name(actor) : "");
				continue;
			}

			{
				...
				if (!((x >= 0 && x < screenw && y >= 0 && y < screenh) || (x + w > 0 && y + h > 0)))
					continue;
			}
				...
				if (past_desktop) {
					...
					if (parent) {
						if (parent == CLUTTER_ACTOR(desktop) ||
							...
							clutter_actor_reparent(actor, CLUTTER_ACTOR(priv->home_blur));
						}
#if STACKING_DEBUG
						else
						...
						if (live_bg_actor && c->desktop == curr_view &&
						    HD_WM_CLIENT_CLIENT_TYPE(c) == HdWmClientTypeHomeApplet) {
	 ...
	 * any that filled the screen then add the window that does. */
	{
		...
		for (i = n_elements - 1; i >= 0; i--) {
			ClutterActor *child = clutter_group_get_nth_child(CLUTTER_GROUP(priv->home_blur), i);

			/* If the client decides its own visibility, skip it */
			if (hd_render_manager_should_ignore_actor(child))
				continue;

			if (child != CLUTTER_ACTOR(priv->home) && child != CLUTTER_ACTOR(priv->blur_front)) {
				union {
		 ...
		 * the status menu.
		 */
		if (clutter_actor_get_parent(CLUTTER_ACTOR(priv->blur_front)) == CLUTTER_ACTOR(priv->home_blur)) {
			...
			if (highest_maximized)
				clutter_actor_raise(CLUTTER_ACTOR(priv->blur_front), highest_maximized);
			else
	...
	for (i = 0, it = g_list_last(previous_home_blur),
	     child = clutter_group_get_nth_child(CLUTTER_GROUP(priv->home_blur), 0);
	     child && it; child = clutter_group_get_nth_child(CLUTTER_GROUP(priv->home_blur), ++i), it = it->prev) {
	...
	if (blur_changed) {
		...
		for (i = 0; i < n_elements; i++) {
			ClutterActor *actor = clutter_group_get_nth_child(CLUTTER_GROUP(priv->home_blur), i);
			if (CLUTTER_ACTOR_IS_VISIBLE(actor)) {
				if (clutter_actor_get_name(actor))
	...
	for (i = 0; i < clutter_group_get_n_children(CLUTTER_GROUP(priv->home_blur)); i++) {
		...
		g_debug("STACK[%d] %s %s", i, name ? name : "?", CLUTTER_ACTOR_IS_VISIBLE(child) ? "" : "(invisible)");
	}
	for (i = 0; i < clutter_group_get_n_children(CLUTTER_GROUP(priv->app_top)); i++) {
		ClutterActor *child = clutter_group_get_nth_child(CLUTTER_GROUP(priv->app_top), i);
		const char *name = clutter_actor_get_name(child);
		g_debug("TOP[%d] %s %s", i, name ? name : "?", CLUTTER_ACTOR_IS_VISIBLE(child) ? "" : "(invisible)");
	}
...
void hd_render_manager_update_blur_state()
{
	...
	for (c = wm->stack_top; c; c = c->stacked_below) {
		...
		if (blur && c->cm_client) {
			if (hd_comp_mgr_client_has_video_overlay(HD_COMP_MGR_CLIENT(c->cm_client)))
		...
		if (c_type == MBWMClientTypeApp) {
			...
			if (c->window && hd_comp_mgr_client_is_maximized(c->window->geometry))
				blur_buttons = TRUE;
			break;
		}
		...
		if (c_type == MBWMClientTypeDialog ||
		    ...
		    c_type == HdWmClientTypeStatusMenu || HD_IS_CONFIRMATION_NOTE(c) || HD_IS_INFO_NOTE(c)) {
	...
	tidy_blur_group_set_chequer(CLUTTER_ACTOR(priv->home_blur), blur && has_video_overlay);
}

void hd_render_manager_pause_blur_animation()
{
	tidy_blur_group_stop_progressing(CLUTTER_ACTOR(render_manager->priv->home_blur));
}
...
void hd_render_manager_set_launcher_subview(gboolean subview)
{
	...
	if (subview)
		hd_render_manager_set_blur(priv->current_blur | HDRM_ZOOM_FOR_LAUNCHER_SUBMENU);
	else
		hd_render_manager_set_blur(priv->current_blur & ~HDRM_ZOOM_FOR_LAUNCHER_SUBMENU);
}
 ...
 * rect in blockers */
static gboolean hd_render_manager_is_visible(GList * blockers, ClutterGeometry rect)
{
	HdRenderManagerPrivate *priv = render_manager->priv;
	...
	for (; blockers; blockers = blockers->next) {
		...
		VISIBILITY("RECT %dx%d%+d%+d BLOCKER %dx%d%+d%+d", MBWM_GEOMETRY(&rect), MBWM_GEOMETRY(&blocker));
		 ...
		 * unsigneds.
		 */
		if (!(blocker.x <= rect.x && rect.x + (gint) rect.width <= blocker.x + (gint) blocker.width))
			continue;
		...
		if (rect.y < blocker.y) {	/* top of rect is above blocker */
			if (rect_b < blocker.y)
			...
			else {	/* rect is split into two pieces by blocker */
		...
		} else if (rect.y < blocker_b) {	/* top of rect is inside blocker */
			if (rect_b < blocker_b)
				/* rect is confined in blocker */
				return FALSE;
			else {	/* rect is half below blocker, clip the rest */
		...
		if (blocker.x <= rect.x && rect.x + rect.width <= blocker.x + blocker.width) {
			if (blocker.y <= rect.y && rect.y + rect.height <= blocker.y + blocker.height) {
				...
				if (blocker.y + blocker.height >= rect.y + rect.height)
					...
					rect.height = blocker.y - rect.y;
			} else {	/* rect must be out the top, clip to the blocker */
				rect.height = (rect.y + rect.height) - (blocker.y + blocker.height);
...
static
MBWindowManagerClient *hd_render_manager_get_wm_client_from_actor(ClutterActor * actor)
{
	MBWindowManager *wm;
	MBWindowManagerClient *c;
	...
	for (c = wm->stack_bottom; c; c = c->stacked_above)
		if (c->cm_client) {
			ClutterActor *cactor =
			    mb_wm_comp_mgr_clutter_client_get_actor(MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client));
			if (actor == cactor)
				return c;
		}
	return 0;
...
gboolean hd_render_manager_should_ignore_cm_client(MBWMCompMgrClutterClient * cm_client)
{
	 ...
	 */
	return (mb_wm_comp_mgr_clutter_client_get_flags(cm_client) & MBWMCompMgrClutterClientDontShow);
}
...
gboolean hd_render_manager_actor_opaque(ClutterActor * actor)
{
	...
	wm = MB_WM_COMP_MGR(render_manager->priv->comp_mgr)->wm;
	wm_client = hd_render_manager_get_wm_client_from_actor(actor);
	return wm && wm_client && !wm_client->is_argb32 && !mb_wm_theme_is_client_shaped(wm->theme, wm_client);
}
...
void hd_render_manager_append_geo_cb(ClutterActor * actor, gpointer data)
{
	...
	if (hd_render_manager_actor_opaque(actor)) {
		...
		cc = g_object_get_data(G_OBJECT(actor), "HD-MBWMCompMgrClutterClient");
		if (cc && cc->wm_client && (HD_IS_DIALOG(cc->wm_client) || HD_IS_APP_MENU(cc->wm_client))) {
...
void hd_render_manager_set_visibilities()
{
	...
	clutter_container_foreach(CLUTTER_CONTAINER(priv->app_top),
				  hd_render_manager_append_geo_cb, (gpointer) & blockers);
	...
	for (i = n_elements - 1; i >= 0; i--) {
		...
		if (child != CLUTTER_ACTOR(priv->blur_front)) {
			...
			} else {	/* Not visible, hide it unless... */
	...
	if (STATE_IS_APP(priv->state)
		...
		for (c = wm->stack_top; c && c != wm->desktop && !has_fullscreen; c = c->stacked_below) {
			if (!c->cm_client || c->desktop < 0 || !c->window)
				/* It's probably an unnecessary check. */
				continue;
			if (!hd_render_manager_is_client_visible(c))
				continue;
			has_fullscreen |= c->window->ewmh_state & MBWMClientWindowEWMHStateFullscreen;
		}
...
void hd_render_manager_blurred_changed()
{
	...
	if ((force || priv->home_radius.b != 0) && !force_not)
		tidy_blur_group_set_source_changed(CLUTTER_ACTOR(render_manager->priv->home_blur));
	else
		tidy_blur_group_hint_source_changed(CLUTTER_ACTOR(render_manager->priv->home_blur));
}
...
static gboolean hd_render_manager_captured_event_cb(ClutterActor * actor, ClutterEvent * event, gpointer * data)
{
	...
	if (render_manager && render_manager->priv->has_input_blocker) {
		...
		g_debug("%s: Input event blocked by " "hd_render_manager_add_input_blocker", __FUNCTION__);
...
static gboolean _hd_render_manager_remove_input_blocker_cb()
{
...
void hd_render_manager_add_input_blocker(void)
{
	...
	if (!priv->has_input_blocker) {
		...
		priv->has_input_blocker_timeout =
		    g_timeout_add(1000, (GSourceFunc) _hd_render_manager_remove_input_blocker_cb, 0);
	}
}
...
static void hd_render_manager_set_x_input_viewport_for_window(Display * xdpy, Window win, XserverRegion region)
{
	XSelectInput(xdpy, win, FocusChangeMask | ExposureMask
		     | PropertyChangeMask | ButtonPressMask | ButtonReleaseMask
		     | KeyPressMask | KeyReleaseMask | PointerMotionMask | SubstructureNotifyMask |
		     SubstructureRedirectMask | StructureNotifyMask | PropertyChangeMask);
...
static gboolean hd_render_manager_set_compositor_input_viewport_idle(gpointer data)
{
	HdRenderManagerPrivate *priv = render_manager->priv;
	MBWindowManager *wm = MB_WM_COMP_MGR(priv->comp_mgr)->wm;
	Window win = None;
	...
	/* If we're not actually changing the contents of the viewport, just
	   return now */
	if (priv->current_input_viewport && gdk_region_equal(priv->new_input_viewport, priv->current_input_viewport))
		return FALSE;

	/* Create an XFixes region from the GdkRegion. We use GdkRegion because
	 * we can check equality easily with it. */
	gdk_region_get_rectangles(priv->new_input_viewport, &rectangles, &n_rectangles);
	if (n_rectangles > 0)
	...
	clwin = clutter_x11_get_stage_window(CLUTTER_STAGE(clutter_stage_get_default()));
	/* On startup, wm->comp_mgr may not be set */
	if (wm->comp_mgr)
		win = mb_wm_comp_mgr_clutter_get_overlay_window(MB_WM_COMP_MGR_CLUTTER(wm->comp_mgr));

	if (win != None)
		hd_render_manager_set_x_input_viewport_for_window(wm->xdpy, win, region);
	if (clwin != None)
		hd_render_manager_set_x_input_viewport_for_window(wm->xdpy, clwin, region);
...
static void hd_render_manager_set_compositor_input_viewport(GdkRegion * region)
{
	HdRenderManagerPrivate *priv = render_manager->priv;
	...
	if (!priv->input_viewport_callback)
		priv->input_viewport_callback = g_idle_add_full(G_PRIORITY_DEFAULT + 20,
  ...
  * etc. */
static GdkRegion *hd_render_manager_get_foreground_region(MBWMClientType client_mask)
{
	...
	while (fg_client) {
		if (MB_WM_CLIENT_CLIENT_TYPE(fg_client) & client_mask)
			gdk_region_union_with_rect(region, (GdkRectangle *) (void *)&fg_client->window->geometry);
...
void hd_render_manager_set_input_viewport()
{
	HdRenderManagerPrivate *priv = render_manager->priv;
	GdkRegion *region = gdk_region_new();
	MBWindowManager *wm = MB_WM_COMP_MGR(priv->comp_mgr)->wm;
	...
	if (!hd_wm_has_modal_blockers(wm)) {
		...
		if (!STATE_NEED_WHOLE_SCREEN_INPUT(priv->state)
			...
			if (hd_render_manager_actor_is_visible(hd_home_get_edit_button(priv->home))) {
				...
				clutter_actor_get_geometry(hd_home_get_edit_button(priv->home), &geom);
		...
		if (STATE_UNGRAB_NOTES(hd_render_manager_get_state())) {
			GdkRegion *subtract =
			    hd_render_manager_get_foreground_region(MBWMClientTypeNote | MBWMClientTypeDialog);
		...
		if (STATE_NEED_DESKTOP(hd_render_manager_get_state())) {
			GdkRegion *unionregion = hd_render_manager_get_foreground_region(HdWmClientTypeHomeApplet);
	...
	for (c = wm->stack_top; c && c != wm->desktop; c = c->stacked_below)
		if (HD_IS_INCOMING_EVENT_PREVIEW_NOTE(c)
			...
			gdk_region_union_with_rect(region, (GdkRectangle *) (void *)&c->frame_geometry);
...
void hd_render_manager_flip_input_viewport()
{
	HdRenderManagerPrivate *priv = render_manager->priv;
	...
	gdk_region_get_rectangles(priv->current_input_viewport, &inputshape, &ninputshapes);
