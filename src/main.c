...
static void hd_mutex_lock(void)
{
...
static void hd_mutex_unlock(void)
{
	/*g_printerr ("%s, enabled %d\n", __func__, hd_clutter_mutex_enabled); */
	if (hd_clutter_mutex_enabled)
...
static void hd_mutex_init(void)
{
	/* g_printerr ("%s%d\n", __func__); */
	clutter_threads_set_lock_functions(hd_mutex_lock, hd_mutex_unlock);
}

static unsigned int theme_type_func(const char *theme_name, void *data)
{
...
static unsigned int theme_client_type_func(const char *type_name, void *user_data)
{
	if (!type_name)
		return 0;
...
static unsigned int theme_button_type_func(const char *type_name, void *user_data)
{
...
static void close_input_devices(Display * dpy)
{
	int i;
...
static void enumerate_input_devices(Display * dpy)
{
	XDeviceInfo *devinfo;
	int i, ndev;
	GArray *eclass;
...
static ClutterX11FilterReturn clutter_x11_event_filter(XEvent * xev, ClutterEvent * cev, gpointer data)
{
...
/* Debugging aids */
static gboolean dump_debug_info_when_idle(gpointer unused)
{
...
static void dump_debug_info_sighand(int unused)
{
...
static pid_t hd_already_running(Display * dpy)
{
	Window root;
	char *wmname;
	pid_t *wmpid, pid;
	...
	wmwin = hd_util_get_win_prop_data_and_validate(dpy, root, xpcheck, XA_WINDOW, 32, 1, NULL);
	if (!wmwin)
		goto out0;

	wmwin2 = hd_util_get_win_prop_data_and_validate(dpy, *wmwin, xpcheck, XA_WINDOW, 32, 1, NULL);
	if (!wmwin2) {
	...
	wmname = hd_util_get_win_prop_data_and_validate(dpy, *wmwin, xpname,
							XInternAtom(dpy, "UTF8_STRING", False), 8, 0, NULL);
	if (!wmname) {
	...
	wmpid = hd_util_get_win_prop_data_and_validate(dpy, *wmwin, xppid, XA_CARDINAL, 32, 1, NULL);
	if (!wmpid) {
...
static gboolean get_program_file(const gchar * link, gchar * path, size_t spath)
{
	gint n;
	gchar *p;
...
static gboolean same_file(const gchar * link1, const gchar * link2)
{
...
static void relaunch(int unused)
{
	char me[128];
	MBWMRootWindow *root;
...
static void try_to_relaunch(Display * dpy)
{
	pid_t mate;
	gchar *matepath;
...
static gboolean get_debug_info(Display * dpy)
{
	pid_t mate;
...
static void terminating(int unused)
{
	gtk_main_quit();
}
...
static void hd_fpu_set_mode(OSSO_FPU_MODE mode)
{
	...
	if (OSSO_FPU_FAST == mode) {
		int tmp;
		__asm__ volatile ("fmrx       %[tmp], fpscr\n" "orr        %[tmp], %[tmp], #(1 << 24)\n"	/* flush-to-zero */
				  "orr        %[tmp], %[tmp], #(1 << 25)\n"	/* default NaN */
...
int main(int argc, char **argv)
{
	...
	wm = MB_WINDOW_MANAGER(mb_wm_object_new(HD_TYPE_WM,
						...
						MBWMObjectPropArgv, argv, MBWMObjectPropDpy, dpy, NULL));
	if (wm == NULL)
	...
	XSelectExtensionEvent(dpy, RootWindow(dpy, clutter_x11_get_default_screen()), &class_presence, 1);
