...
static void hd_decor_remove_actors(HdDecor * decor);

static void hd_decor_class_init(MBWMObjectClass * klass)
{
...
static void hd_decor_destroy(MBWMObject * obj)
{
...
static int hd_decor_init(MBWMObject * obj, va_list vap)
{
...
HdDecor *hd_decor_new(MBWindowManager * wm, MBWMDecorType type)
{
	...
	decor = mb_wm_object_new(HD_TYPE_DECOR, MBWMObjectPropWm, wm, MBWMObjectPropDecorType, type, NULL);

	return HD_DECOR(decor);
}

int hd_decor_class_type()
{
	static int type = 0;
...
static gboolean hd_decor_window_check_prop(MBWindowManager * wm, Window w, HdAtoms atom)
{
	HdCompMgr *hmgr = HD_COMP_MGR(wm->comp_mgr);
	Atom progress_indicator = hd_comp_mgr_get_atom(hmgr, atom);
...
{
	return hd_decor_window_check_prop(wm, w, HD_ATOM_HILDON_WM_WINDOW_PROGRESS_INDICATOR);
}

gboolean hd_decor_window_has_menu_indicator(MBWindowManager * wm, Window w)
{
	return hd_decor_window_check_prop(wm, w, HD_ATOM_HILDON_WM_WINDOW_MENU_INDICATOR);
}
...
static ClutterActor *hd_decor_get_actor(HdDecor * decor)
{
	MBWindowManagerClient *client = hd_decor_get_client(decor);
	if (!client || !client->cm_client)
		return NULL;
	return mb_wm_comp_mgr_clutter_client_get_actor(MB_WM_COMP_MGR_CLUTTER_CLIENT(client->cm_client));
}
...
static void hd_decor_remove_actors(HdDecor * decor)
{
	...
	if (decor->progress_texture) {
		clutter_container_remove_actor(CLUTTER_CONTAINER(actor), decor->progress_texture);
		decor->progress_texture = 0;
	}
	if (decor->title_bar_actor) {
		clutter_container_remove_actor(CLUTTER_CONTAINER(actor), decor->title_bar_actor);
		decor->title_bar_actor = 0;
	}
	if (decor->title_actor) {
		clutter_container_remove_actor(CLUTTER_CONTAINER(actor), decor->title_actor);
		decor->title_actor = 0;
	}
}
...
static void hd_decor_create_actors(HdDecor * decor)
{
	...
	if (c->image_filename) {
		...
		decor->title_bar_actor =
		    hd_clutter_cache_get_sub_texture_for_area(c->image_filename, TRUE, &geo, &area);
	} else {
		decor->title_bar_actor = hd_clutter_cache_get_texture_for_area(HD_THEME_IMG_DIALOG_BAR, TRUE, &area);
	}
	...
	if (client->xwin_frame)
		clutter_actor_set_position(decor->title_bar_actor, mb_decor->geom.x, mb_decor->geom.y);
	else
	...
	if (d->show_title) {
		...
		if (title && strlen(title)) {
			...
			if (is_waiting)
				screen_width_avail -= HD_THEME_IMG_PROGRESS_SIZE + HD_TITLE_BAR_PROGRESS_MARGIN;

			hd_gtk_style_get_fg_color(HD_GTK_BUTTON_SINGLETON, GTK_STATE_NORMAL, &default_color);
			...
			clutter_container_add_actor(CLUTTER_CONTAINER(actor), decor->title_actor);

			snprintf(font_name, sizeof(font_name), "%s %i%s",
				 d->font_family ? d->font_family : "Sans",
				 d->font_size ? d->font_size : 18, d->font_units == MBWMXmlFontUnitsPoints ? "" : "px");
			...
			if (w > screen_width_avail) {
				...
				clutter_actor_set_width(CLUTTER_ACTOR(bar_title), screen_width_avail);
				clutter_actor_set_clip(CLUTTER_ACTOR(bar_title), 0, 0, screen_width_avail, h);
				w = screen_width_avail;
			}

			clutter_actor_set_position(CLUTTER_ACTOR(bar_title),
						   (screen_width_avail - w) / 2, (mb_decor->geom.height - h) / 2);
		}
	...
	if (is_waiting) {
		...
		gint x = 0;
		decor->progress_texture = hd_clutter_cache_get_sub_texture(HD_THEME_IMG_PROGRESS, TRUE, &progress_geo);
		if (decor->title_actor) {
			x = clutter_actor_get_x(CLUTTER_ACTOR(decor->title_actor)) +
			    clutter_actor_get_width(CLUTTER_ACTOR(decor->title_actor)) + HD_TITLE_BAR_PROGRESS_MARGIN;
		}
		clutter_container_add_actor(CLUTTER_CONTAINER(actor), decor->progress_texture);
		clutter_actor_set_position(decor->progress_texture,
					   x, (mb_decor->geom.height - HD_THEME_IMG_PROGRESS_SIZE) / 2);
		clutter_actor_set_size(decor->progress_texture, HD_THEME_IMG_PROGRESS_SIZE, HD_THEME_IMG_PROGRESS_SIZE);
		/* Get the timeline and set it running */
		decor->progress_timeline = g_object_ref(clutter_timeline_new(HD_THEME_IMG_PROGRESS_FRAMES,
		...
		g_signal_connect(decor->progress_timeline, "new-frame",
				 G_CALLBACK(on_decor_progress_timeline_new_frame), decor->progress_texture);
...
void hd_decor_sync(HdDecor * decor)
{
	...
	if (MB_WM_DECOR(decor)->geom.width > 0 &&
	    MB_WM_DECOR(decor)->geom.height > 0 && MB_WM_CLIENT_CLIENT_TYPE(client) != MBWMClientTypeApp) {
