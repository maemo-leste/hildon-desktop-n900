...
static Bool
hd_dialog_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometry, MBWMClientReqGeomType flags);

static void hd_dialog_realize(MBWindowManagerClient * client);

static void hd_dialog_class_init(MBWMObjectClass * klass)
{
...
static void hd_dialog_destroy(MBWMObject * this)
{
	HdDialog *dialog = HD_DIALOG(this);
	MBWindowManagerClient *c = MB_WM_CLIENT(this);
	MBWindowManager *wm = c->wmref;

	mb_wm_main_context_x_event_handler_remove(wm->main_ctx, ButtonRelease, dialog->release_cb_id);
}

static int hd_dialog_init(MBWMObject * this, va_list vap)
{
	 ...
	 */
	mb_wm_client_set_layout_hints(client, LayoutPrefPositionFree | LayoutPrefVisible);
...
hd_dialog_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometry, MBWMClientReqGeomType flags)
{
	...
	if (flags & MBWMClientReqGeomIsViaConfigureReq) {
		...
		client->frame_geometry.y = wm->xdpy_height - (client->frame_geometry.height);
