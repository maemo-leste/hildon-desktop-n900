...
HdRunningApp *hd_comp_mgr_client_get_app_key(HdCompMgrClient * client, HdCompMgr * hmgr);

static void hd_comp_mgr_check_do_not_disturb_flag(HdCompMgr * hmgr);

static gboolean hd_comp_mgr_client_prefers_compositing(MBWindowManagerClient * c);

static gboolean hd_comp_mgr_is_non_composited(MBWindowManagerClient * client, gboolean force_re_read);

static MBWindowManagerClient *hd_comp_mgr_determine_current_app(void);

static MBWMCompMgrClient *hd_comp_mgr_client_new(MBWindowManagerClient * client)
{
	MBWMObject *c;

	c = mb_wm_object_new(HD_TYPE_COMP_MGR_CLIENT, MBWMObjectPropClient, client, NULL);

	return MB_WM_COMP_MGR_CLIENT(c);
}

static void hd_comp_mgr_client_class_init(MBWMObjectClass * klass)
{
#if MBWM_WANT_DEBUG
...
static void hd_comp_mgr_client_process_hibernation_prop(HdCompMgrClient * hc)
{
	...
	hibernable = hd_util_get_win_prop_data_and_validate
	    (wm_client->wmref->xdpy,
	     wm_client->window->xwindow, hmgr->priv->atoms[HD_ATOM_HILDON_APP_KILLABLE], XA_STRING, 8, 0, NULL);
	...
	if (!hibernable) {
		...
		hibernable = hd_util_get_win_prop_data_and_validate
		    (wm_client->wmref->xdpy,
		     ...
		     hmgr->priv->atoms[HD_ATOM_HILDON_ABLE_TO_HIBERNATE], XA_STRING, 8, 0, NULL);
	}
...
HdRunningApp *hd_comp_mgr_client_get_app_key(HdCompMgrClient * client, HdCompMgr * hmgr)
{
	...
	app = hd_app_mgr_match_window(class_hint.res_name, class_hint.res_class, wm_client->window->pid);
	...
	if (app) {
		...
		role = hd_util_get_win_prop_data_and_validate
		    (wm_client->wmref->xdpy,
		     wm_client->window->xwindow, hmgr->priv->atoms[HD_ATOM_WM_WINDOW_ROLE], XA_STRING, 8, 0, NULL);
		...
		key = g_strdup_printf("%s/%s/%s/%d",
				      ...
				      class_hint.res_class ? class_hint.res_class : "", role ? role : "", level);
		g_debug("%s: app %s, window key: %s\n", __FUNCTION__, hd_running_app_get_id(app), key);
...
static int hd_comp_mgr_client_init(MBWMObject * obj, va_list vap)
{
	...
	if (app) {
		...
		guint windows = GPOINTER_TO_UINT(g_hash_table_lookup(hmgr->priv->shown_apps,
								     (gpointer) app));
		if (!windows)
			hd_app_mgr_app_opened(app);

		g_hash_table_insert(hmgr->priv->shown_apps, (gpointer) app, GUINT_TO_POINTER(++windows));
	}
...
static void hd_comp_mgr_client_destroy(MBWMObject * obj)
{
...
int hd_comp_mgr_client_class_type()
{
	static int type = 0;
	...
	if (UNLIKELY(type == 0)) {
		...
		type = mb_wm_object_register_class(&info, MB_WM_TYPE_COMP_MGR_CLUTTER_CLIENT, 0);
	}
...
static void hd_comp_mgr_register_client(MBWMCompMgr * mgr, MBWindowManagerClient * c, Bool activate);
...
static void hd_comp_mgr_unmap_notify(MBWMCompMgr * mgr, MBWindowManagerClient * c);
...
static void hd_comp_mgr_class_init(MBWMObjectClass * klass)
{
	...
	MBWMCompMgrClutterClass *clutter_klass = MB_WM_COMP_MGR_CLUTTER_CLASS(klass);
...
static Atom *hd_comp_mgr_get_atoms(MBWindowManager * wm)
{
...
static int hd_comp_mgr_init(MBWMObject * obj, va_list vap)
{
	...
	priv->render_manager = hd_render_manager_create(hmgr,
							hd_launcher_get(), HD_HOME(priv->home), hd_task_navigator);
	g_object_set(priv->home, "hdrm", priv->render_manager, NULL);
	clutter_container_add_actor(CLUTTER_CONTAINER(stage), CLUTTER_ACTOR(priv->render_manager));
	 ...
	 */
	priv->switcher_group = g_object_new(HD_TYPE_SWITCHER, "comp-mgr", cmgr, "task-nav", hd_task_navigator, NULL);
	 ...
	 */
	priv->shown_apps = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
	priv->hibernating_apps =
	    g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, (GDestroyNotify) mb_wm_object_unref);

	/* Be notified about all X window property changes around here. */
	priv->property_changed_cb_id = mb_wm_main_context_x_event_handler_add(cmgr->wm->main_ctx, None, PropertyNotify,
									      (MBWMXEventFunc)
...
static void hd_comp_mgr_destroy(MBWMObject * obj)
{
	...
	mb_wm_main_context_x_event_handler_remove(MB_WM_COMP_MGR(obj)->wm->main_ctx,
						  PropertyNotify, priv->property_changed_cb_id);
...
Bool hd_comp_mgr_client_property_changed(XPropertyEvent * event, HdCompMgr * hmgr)
{
	...
	able_to_hibernate = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ABLE_TO_HIBERNATE);
	...
	if (event->atom == wm->atoms[MBWM_ATOM_HILDON_LIVE_DESKTOP_BACKGROUND]) {
		...
		if (c) {
			...
			ClutterActor *actor;
			MBWMCompMgrClutterClient *cclient = MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client);
			...
			mb_wm_comp_mgr_clutter_client_set_flags(cclient, MBWMCompMgrClutterClientDontPosition);
			/* remove it from the switcher */
			if (hd_task_navigator_has_window(hd_task_navigator, actor))
				hd_switcher_remove_window_actor(priv->switcher_group, actor, cclient);
	...
	if (event->atom ==
	    ...
	    event->atom == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_WM_WINDOW_MENU_INDICATOR)) {
		...
		top = mb_wm_managed_client_from_xwindow(MB_WM_COMP_MGR(hmgr)->wm, event->window);
		if (top) {
	...
	non_comp_changed = event->atom == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_NON_COMPOSITED_WINDOW);
	if (event->atom == wm->atoms[MBWM_ATOM_NET_WM_STATE] || non_comp_changed) {
		...
		if (c && HD_IS_APP(c)) {
			...
			for (tmp = c->stacked_above; tmp; tmp = tmp->stacked_above)
				if (mb_wm_client_is_map_confirmed(tmp) && hd_comp_mgr_client_prefers_compositing(tmp)) {
					found = TRUE;
					break;
				}
			client_non_comp = hd_comp_mgr_is_non_composited(c, non_comp_changed);
			if (hd_render_manager_get_state() == HDRM_STATE_NON_COMPOSITED && !client_non_comp)
				hd_render_manager_set_state(HDRM_STATE_APP);
			else if (hd_render_manager_get_state() == HDRM_STATE_NON_COMP_PORT && !client_non_comp)
				hd_render_manager_set_state(HDRM_STATE_APP_PORTRAIT);
			else if (hd_render_manager_get_state() == HDRM_STATE_APP &&
				 !hd_transition_is_rotating() && client_non_comp && !found)
	...
	if (event->atom == killable || event->atom == able_to_hibernate) {
		...
		if (!app)
			return False;
		current_app = hd_comp_mgr_client_get_app(hd_comp_mgr_get_current_client(hmgr));
	...
	if (event->atom == nothread) {
		...
		str = event->state == PropertyNewValue
		    ? hd_util_get_x_window_string_property(wm, c->window->xwindow, HD_ATOM_NOTIFICATION_THREAD)
		    : NULL;
		if (event->state != PropertyNewValue || str)
			/* Otherwise don't mess up more. */
			hd_task_navigator_notification_thread_changed(hd_task_navigator, a, str);
		return False;
	}
	...
	if (STATE_IS_PORTRAIT(hd_render_manager_get_state())) {	/* Portrait => landscape? */
	...
	} else if (STATE_IS_PORTRAIT_CAPABLE(hd_render_manager_get_state())) {	/* Landscape => portrait? */
		hd_app_mgr_mce_activate_accel_if_needed(FALSE);
		if (value != 0 && hd_comp_mgr_should_be_portrait(hmgr))
...
static void hd_comp_mgr_turn_on(MBWMCompMgr * mgr)
{
	MBWMCompMgrClass *parent_klass = MB_WM_COMP_MGR_CLASS(MB_WM_OBJECT_GET_PARENT_CLASS(MB_WM_OBJECT(mgr)));
...
static unsigned cntchildren(MBWindowManagerClient * client)
{
	unsigned i;
	MBWMList *li;
 ...
 * into account. */
static MBWMStackLayerType layer_of(MBWindowManagerClient * client, gboolean needs_desktop, gboolean goto_app_state)
{
...
static gboolean
ordered(MBWindowManagerClient * lc, MBWindowManagerClient * rc, gboolean needs_desktop, gboolean goto_app_state)
{
	...
	return llc != lrc ? llc > lrc : !(MB_WM_CLIENT_CLIENT_TYPE(rc) & MBWMClientTypeDesktop);
}
...
static void lp_forecast(MBWindowManager * wm, MBWindowManagerClient * client)
{
	...
	if (client->transient_for && wm->stack_top == client) {
		...
		for (want = client->transient_for, li = want->transients;; want = li->data, li = li->next) {
			g_assert(li != NULL);
			if (li->data == client)
				break;
		}
		...
		if (dst != src) {
			g_assert(src < dst);
			memmove(&stack->pdata[src], &stack->pdata[src + 1], sizeof(stack->pdata[0]) * (dst - src));
	...
	for (l = 1, r = 2; l < stack->len; l = r++) {
		while (!ordered(stack->pdata[l - 1], stack->pdata[l], STATE_NEED_DESKTOP(state), goto_app_state)) {
			MBWindowManagerClient *tmp;
	...
	for (l = 0; stack->pdata[l] != wm->desktop; l++) {
		...
		gboolean supports_requests_portrait = c->portrait_supported || c->portrait_requested;
		PORTRAIT("%s: supports_requests_portrait: %d", __FUNCTION__, supports_requests_portrait);

		if (((!force_rotation && !whitelisted) && !supports_requests_portrait)
		    || hd_comp_mgr_is_orientationlock_enabled(wm, c)
		    || blacklisted || hd_launcher_is_editor_in_landscape()) {
...
static void hd_comp_mgr_unregister_client(MBWMCompMgr * mgr, MBWindowManagerClient * c)
{
	...
	if (hclient->priv->app) {
		...
		if (--windows == 0) {
			...
			g_hash_table_insert(priv->shown_apps, (gpointer) app, GUINT_TO_POINTER(windows));
		}
	}
	...
	if (hclient->priv->app &&
	    ...
	    !g_hash_table_lookup(priv->hibernating_apps, GUINT_TO_POINTER(hclient->priv->hibernation_key))) {
		/*
		 * We want to hold onto the CM client object, so we can continue using
		 * the actor.
		 */
		mb_wm_comp_mgr_clutter_client_set_flags(cclient, MBWMCompMgrClutterClientDontUpdate);
		mb_wm_object_ref(MB_WM_OBJECT(cclient));

		g_hash_table_insert(priv->hibernating_apps, GUINT_TO_POINTER(hclient->priv->hibernation_key), hclient);

		hd_switcher_hibernate_window_actor(priv->switcher_group, actor);
		...
		if (actor) {
			...
			if (app->stack_index > 0 && app->leader != app) {
				...
				app->leader->followers = g_list_remove(app->leader->followers, app);

				if (app->leader->followers)
					prev =
					    MB_WM_COMP_MGR_CLUTTER_CLIENT(MB_WM_CLIENT
									  (g_list_last(app->leader->followers)->data)
									  ->cm_client);
				else
					prev = MB_WM_COMP_MGR_CLUTTER_CLIENT(MB_WM_CLIENT(app->leader)->cm_client);
				...
				if (topmost) {	/* if we were on top, update the switcher */
					...
					if (pactor) {
						...
						g_debug("%s: REPLACE ACTOR %p WITH %p", __func__, actor, pactor);
						hd_switcher_replace_window_actor(priv->switcher_group, actor, pactor);
					} else
						g_warning("%s: leader or next secondary not found", __func__);
				}
			} else if (!(c->window->ewmh_state &
				     MBWMClientWindowEWMHStateSkipTaskbar) &&
				   (app->stack_index < 0 || (app->leader == app && !app->followers))) {
				g_debug("%p: NON-STACKABLE OR FOLLOWERLESS LEADER"
					" (index %d), REMOVE ACTOR %p", __func__, app->stack_index, actor);
				...
				new_leader->followers = g_list_remove(app->followers, new_leader);
				/* disconnect the app */
				...
				app->stack_index = -1;
					   ...
					   MBWMClientWindowEWMHStateSkipTaskbar */
				MBWindowManagerClient *current_client = hd_comp_mgr_determine_current_app();

				if (STATE_IS_APP(hd_render_manager_get_state()) &&
				    MB_WM_CLIENT_CLIENT_TYPE(current_client) & MBWMClientTypeDesktop)
					hd_render_manager_set_state(HDRM_STATE_TASK_NAV);
			}
		...
		if (applet)
			hd_home_unregister_applet(HD_HOME(priv->home), applet);
	}
...
static MBWindowManagerClient *hd_comp_mgr_get_client_transient_for(MBWindowManagerClient * c)
{
	Window xtransfor;
	...
	return xtransfor && xtransfor != c->window->xwindow
	    && xtransfor != c->wmref->root_win->xwindow ? mb_wm_managed_client_from_xwindow(c->wmref, xtransfor)
	    : NULL;
}

static void hd_comp_mgr_texture_update_area(HdCompMgr * hmgr, int x, int y, int width, int height, ClutterActor * actor)
{
	ClutterActor *parent;
...
static void hd_comp_mgr_hook_update_area(HdCompMgr * hmgr, ClutterActor * actor)
{
	if (CLUTTER_IS_GROUP(actor)) {
		...
		for (i = 0, child = clutter_group_get_nth_child(CLUTTER_GROUP(actor), 0);
		     child; child = clutter_group_get_nth_child(CLUTTER_GROUP(actor), ++i)) {
			if (CLUTTER_X11_IS_TEXTURE_PIXMAP(child)) {
				g_signal_connect_swapped(G_OBJECT(child), "update-area",
...
static void fix_transiency(MBWindowManagerClient * client)
{
	MBWindowManager *wm = client->wmref;
	MBWMClientWindow *win = client->window;

	if (win->xwin_transient_for
	    && win->xwin_transient_for != win->xwindow && win->xwin_transient_for != wm->root_win->xwindow) {
		MBWindowManagerClient *trans_parent;

		trans_parent = mb_wm_managed_client_from_xwindow(wm, win->xwin_transient_for);
...
void hd_comp_mgr_reset_overlay_shape(HdCompMgr * hmgr)
{
	...
	if (want_fs_comp) {
		...
		XSetWindowBackground(wm->xdpy, clutter_window, BlackPixel(wm->xdpy, DefaultScreen(wm->xdpy)));
		...
		mb_wm_comp_mgr_clutter_set_overlay_window(MB_WM_COMP_MGR_CLUTTER(hmgr), None);
	}
...
void hd_comp_mgr_unredirect_topmost_client(MBWindowManager * wm, gboolean force)
{
	MBWindowManagerClient *c;
	...
	for (c = wm->stack_top; c && c != wm->desktop; c = c->stacked_below) {
		...
		if (c->cm_client && c->window->net_type ==
		    ...
		    (c->window->ewmh_state & MBWMClientWindowEWMHStateFullscreen || force)) {
			if (!mb_wm_comp_mgr_clutter_client_is_unredirected(c->cm_client)) {
				mb_wm_comp_mgr_clutter_client_track_damage(MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client),
									   False);
...
static void hd_comp_mgr_unredirect_client(MBWindowManagerClient * c)
{
	if (c->cm_client && !mb_wm_comp_mgr_clutter_client_is_unredirected(c->cm_client)) {
		mb_wm_comp_mgr_clutter_client_track_damage(MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client), False);
		mb_wm_comp_mgr_clutter_set_client_redirection(c->cm_client, FALSE);
	} else
...
static gboolean hd_comp_mgr_is_non_composited(MBWindowManagerClient * client, gboolean force_re_read)
{
	...
	if (!HD_APP(client)->non_composited_read) {
		...
		if (ret && class_hint.res_class) {
			if (!strcmp(class_hint.res_class, "Chessui") || !strcmp(class_hint.res_class, "Mahjong")) {
	...
	ret = XGetWindowProperty(wm->xdpy, win->xwindow,
				 atom, 0, 1, False, XA_INTEGER, &actual_type, &format, &items, &left, &prop);
	mb_wm_util_async_untrap_x_errors();
	if (ret != Success)
		return FALSE;
 ...
 * with another. */
static void hd_comp_mgr_handle_stackable(MBWindowManagerClient * client, HdApp ** replaced, HdApp ** add_to_tn)
{
	MBWindowManager *wm = client->wmref;
	MBWMClientWindow *win = client->window;
	...
	*replaced = *add_to_tn = NULL;
	 ...
	 */
	ret = XGetWindowProperty(wm->xdpy, win->xwindow,
				 stack_atom, 0, 1, False, XA_INTEGER, &actual_type, &format, &items, &left, &prop);
	mb_wm_util_async_untrap_x_errors();
	if (ret != Success)
		...
		if (old_leader && app->stack_index <= old_leader->stack_index) {
			...
			if (app == old_leader)
				...
				g_critical("%s: app == old_leader == %p, "
					   "i'm about to do silly things", __FUNCTION__, app);
			...
			if (old_leader->stack_index == app->stack_index) {
				...
				*replaced = old_leader;
				...
				app->followers = g_list_prepend(old_leader->followers, old_leader);
			...
			app->leader->followers = g_list_append(old_leader->followers, client);
			...
			for (flink = old_leader->followers; flink; flink = flink->next) {
				...
				if (f->stack_index >= app->stack_index) {
					if (flink->prev && HD_APP(flink->prev->data)->stack_index == app->stack_index) {
						f = flink->prev->data;
						flink = flink->prev;
					}
					break;
			...
			if (!f && old_leader->followers &&
			    HD_APP(old_leader->followers->data)->stack_index == app->stack_index) {
			...
			if (!f) {
				...
				*add_to_tn = app;
				...
				if (f != app) {
					...
					old_leader->followers = g_list_insert_before(old_leader->followers, flink, app);
					old_leader->followers = g_list_remove_link(old_leader->followers, flink);
					g_list_free(flink);
				...
				*replaced = f;
				...
				g_debug("%s: %p PRECEEDS (index %d) A FOLLOWER (with index %d)"
					" OF THE STACK\n", __func__, app, app->stack_index, f->stack_index);
				old_leader->followers = g_list_insert_before(old_leader->followers, flink, app);
				...
				if (l && l->next) {
					...
					mb_wm_client_add_transient(MB_WM_CLIENT(app), MB_WM_CLIENT(l->next->data));
				}
				...
				if (flink && flink->next) {
					...
					old_leader->followers =
					    g_list_insert_before(old_leader->followers, flink->next, app);
...
static void create_stampfile(void)
{
...
static void hd_comp_mgr_map_notify(MBWMCompMgr * mgr, MBWindowManagerClient * c)
{
	...
	g_debug("%s: 0x%lx '%s'\n", __FUNCTION__, c && c->window ? c->window->xwindow : 0, mb_wm_client_get_name(c));
	create_stampfile();
	...
	if (c->window->live_background) {
		...
		hd_home_set_live_background(HD_HOME(priv->home), c);
		mb_wm_comp_mgr_clutter_client_set_flags(MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client),
	...
	if (HD_IS_INCOMING_EVENT_PREVIEW_NOTE(c)) {
		if (priv->do_not_disturb_flag || STATE_DISCARD_PREVIEW_NOTE(hd_render_manager_get_state())
		    || hd_dbus_tklock_on) {
	...
	if (priv->do_not_disturb_flag && HD_IS_BANNER_NOTE(c) &&
		...
		value = hd_util_get_win_prop_data_and_validate(c->wmref->xdpy,
							       c->window->xwindow, dnd_override, XA_INTEGER, 32, 1,
							       NULL);
	...
	if (hclient->priv->app)
		g_object_set_data(G_OBJECT(actor),
				  "HD-ApplicationId", (gchar *) hd_running_app_get_id(hclient->priv->app));
	...
	if ((hd_render_manager_get_state() == HDRM_STATE_HOME_EDIT
	    ...
	    && (ctype & (MBWMClientTypeDialog | HdWmClientTypeAppMenu | HdWmClientTypeStatusMenu))) {
		if (STATE_IS_PORTRAIT(hd_render_manager_get_state()))
	 ...
	 */
	if (!transient_for)
		if (ctype == MBWMClientTypeDialog || HD_IS_INFO_NOTE(c) || HD_IS_CONFIRMATION_NOTE(c))
	...
	if (priv->status_menu_client &&
	    ...
	    ctype != MBWMClientTypeOverride && !HD_IS_BANNER_NOTE(c))
	...
	if (HD_WM_CLIENT_TYPE(ctype) == HdWmClientTypeHomeApplet) {
		...
		return;
	} else if (HD_WM_CLIENT_TYPE(ctype) == HdWmClientTypeStatusMenu) {	/* Either status menu OR power menu. */
		if (STATE_ONE_OF(hd_render_manager_get_state(),
				 HDRM_STATE_LAUNCHER | HDRM_STATE_LAUNCHER_PORTRAIT |
				 HDRM_STATE_TASK_NAV | HDRM_STATE_TASK_NAV_PORTRAIT)) {
			if (STATE_IS_PORTRAIT(hd_render_manager_get_state()))
		...
		if (HD_IS_BANNER_NOTE(c)) {
			if (transient_for) {
				 ...
				 * a background window */
				MBWindowManagerClient *tmp;
				for (tmp = hd_comp_mgr_determine_current_app(); tmp; tmp = tmp->stacked_above)
		...
		if (HD_IS_INCOMING_EVENT_PREVIEW_NOTE(c)) {	/* let's be us who decide when the previews can be clicked */
			clutter_actor_set_reactive(actor, TRUE);
			g_signal_connect_swapped(actor, "button-press-event", G_CALLBACK(hd_note_clicked), c);
		}
		...
		if (unblank && priv->mce_proxy) {	/* Turn display backlight on for banner notes. */
			g_debug("%s. Call %s", __FUNCTION__, MCE_DISPLAY_ON_REQ);
			dbus_g_proxy_call_no_reply(priv->mce_proxy, MCE_DISPLAY_ON_REQ, G_TYPE_INVALID, G_TYPE_INVALID);
		}
		return;
		...
		if ((transfor = hd_comp_mgr_get_client_transient_for(c)) != NULL) {
			hd_switcher_add_dialog_explicit(HD_SWITCHER(priv->switcher_group), c, actor, transfor);
		}
		return;
	} else if (ctype == MBWMClientTypeOverride && STATE_IS_NON_COMP(hd_render_manager_get_state())) {
	...
	hclient_h = g_hash_table_lookup(priv->hibernating_apps, GUINT_TO_POINTER(hkey));
	...
	if (hclient_h) {
		...
		hd_switcher_replace_window_actor(priv->switcher_group, actor_h, actor);
	...
	if (app->stack_index >= 0 && !topmost) {
		...
		cclient = MB_WM_COMP_MGR_CLUTTER_CLIENT(c->cm_client);
		if (hd_task_navigator_has_window(hd_task_navigator, mb_wm_comp_mgr_clutter_client_get_actor(cclient)))
			in_tasknav = TRUE;
		else
			in_tasknav = FALSE;
		...
		if (!app->leader->followers && !in_tasknav)
			...
			for (l = app->leader->followers; l; l = l->next) {
				cclient = MB_WM_COMP_MGR_CLUTTER_CLIENT(MB_WM_CLIENT(l->data)->cm_client);
	...
	if (to_replace && to_replace->leader == NULL && to_replace->stack_index == -1) {
		 ...
		 */
		old_actor =
		    mb_wm_comp_mgr_clutter_client_get_actor(MB_WM_COMP_MGR_CLUTTER_CLIENT
							    (MB_WM_CLIENT(to_replace)->cm_client));

		g_debug("%s: REPLACE %p WITH %p and ADD %p BACK", __func__, old_actor, actor, old_actor);
		...
		old_actor =
		    mb_wm_comp_mgr_clutter_client_get_actor(MB_WM_COMP_MGR_CLUTTER_CLIENT
							    (MB_WM_CLIENT(to_replace)->cm_client));
		if (old_actor != actor) {
			g_debug("%s: REPLACE ACTOR %p WITH %p", __func__, old_actor, actor);
			hd_switcher_replace_window_actor(priv->switcher_group, old_actor, actor);
			/* and make sure we're in app mode and not transitioning as
			 * we'll want to show this new app right away*/
			if (!STATE_IS_APP(hd_render_manager_get_state()))
	...
	} else if (!to_replace && app->stack_index >= 0 && app->leader != app && topmost) {
		ClutterActor *old_actor;
		...
		if (app->leader->followers) {
			...
			for (l = app->leader->followers; l->next && l->next->data != app; l = l->next) ;
		...
		mb_wm_client_theme_change(c);
		actor_handled = TRUE;
	} else if (app->leader && app->leader != app && app->leader->followers && !topmost) {
		ClutterActor *old_actor;
		HdApp *last_follower;
		...
		g_debug("%s: 1 REPLACE %p WITH %p and ADD %p BACK\n", __func__, old_actor, actor, old_actor);
		hd_switcher_replace_window_actor(priv->switcher_group, old_actor, actor);
		hd_switcher_replace_window_actor(priv->switcher_group, actor, old_actor);
		actor_handled = TRUE;
	}

	if (!actor_handled && !(c->window->ewmh_state & MBWMClientWindowEWMHStateSkipTaskbar)
...
static MBWindowManagerClient *hd_comp_mgr_determine_current_app()
{
	extern MBWindowManager *hd_mb_wm;
	MBWindowManagerClient *c;
	...
	for (c = hd_mb_wm->stack_top; c; c = c->stacked_below) {
		...
		if (!mb_wm_client_is_map_confirmed(c) &&
		    mb_wm_client_is_geometry_requested(c) && !hd_comp_mgr_client_is_maximized(c->frame_geometry))
...
static void hd_comp_mgr_unmap_notify(MBWMCompMgr * mgr, MBWindowManagerClient * c)
{
	...
	if (HD_IS_APP(c) && HD_APP(c)->stack_index > 0 && HD_APP(c)->leader != HD_APP(c)) {
		GList *l;
		g_debug("%s: detransitise stackable secondary %lx\n", __FUNCTION__, c->window->xwindow);
		...
		if (l && l->next) {
			...
			if (l->prev) {
				...
				mb_wm_client_add_transient(MB_WM_CLIENT(l->prev->data), MB_WM_CLIENT(l->next->data));
			} else {
	...
	} else if (HD_IS_APP(c) && HD_APP(c)->stack_index >= 0 && HD_APP(c)->leader == HD_APP(c)) {
		...
		if (HD_APP(c)->followers) {
			 ...
			 * by libmatchbox2 */
			mb_wm_client_detransitise(MB_WM_CLIENT(HD_APP(c)->followers->data));
		}
	} else if (HD_IS_INCOMING_EVENT_NOTE(c)) {
		hd_switcher_remove_notification(priv->switcher_group, HD_NOTE(c));
		return;
	} else if (c_type == MBWMClientTypeNote || c_type == MBWMClientTypeDialog ||
		   c->window->net_type == c->wmref->atoms[MBWM_ATOM_NET_WM_WINDOW_TYPE_POPUP_MENU]) {
		ClutterActor *actor;
		actor = mb_wm_comp_mgr_clutter_client_get_actor(cclient);
...
static void hd_comp_mgr_effect(MBWMCompMgr * mgr, MBWindowManagerClient * c, MBWMCompMgrClientEvent event)
{
	...
	if (event == MBWMCompMgrClientEventUnmap) {
		...
		if (HD_WM_CLIENT_TYPE(c_type) == HdWmClientTypeStatusMenu)
		...
		else if (c_type == MBWMClientTypeDialog || HD_WM_CLIENT_TYPE(c_type) == HdWmClientTypeAppMenu) {
			...
			if (app->stack_index > 0 && app->leader != app) {
				...
				if (app->leader->followers && g_list_last(app->leader->followers)->prev)
					next = MB_WM_CLIENT(g_list_last(app->leader->followers)->prev->data);
				/* Start actual transition */
				hd_transition_subview(hmgr, c, next, MBWMCompMgrClientEventUnmap);
			} else if ((app->stack_index < 0 || (app->leader == app && !app->followers))
				...
				if (!app || !hd_running_app_is_hibernating(app)) {
		...
		if (HD_WM_CLIENT_TYPE(c_type) == HdWmClientTypeStatusMenu)
			...
			if (app->stack_index > 0 && !app->map_effect_before) {
				 ...
				 * Solves NB#112411 */
				if (c != next)
					hd_transition_subview(hmgr, c, next, MBWMCompMgrClientEventMap);
			}
...
gboolean hd_comp_mgr_restack(MBWMCompMgr * mgr)
{
	...
	if (STATE_NEED_TASK_NAV(hd_render_manager_get_state())) {
		...
		if (mgr->wm->desktop)
			priv->current_hclient = HD_COMP_MGR_CLIENT(mgr->wm->desktop->cm_client);
	...
	if (mgr->wm && mgr->wm->root_win && mgr->wm->desktop && !hd_transition_rotation_will_change_state()) {
		gboolean current_client_changed = FALSE;
		MBWindowManagerClient *current_client = hd_comp_mgr_determine_current_app();

		HdCompMgrClient *new_current_hclient = HD_COMP_MGR_CLIENT(current_client->cm_client);
		if (new_current_hclient != priv->current_hclient) {
			...
			if (priv->current_hclient && hd_comp_mgr_client_can_hibernate(priv->current_hclient)) {
				old_current_app = hd_comp_mgr_client_get_app(priv->current_hclient);
				if (old_current_app)
					hd_app_mgr_hibernatable(old_current_app, TRUE);
			}

			if (new_current_hclient) {
				new_current_app = hd_comp_mgr_client_get_app(new_current_hclient);
		...
		if (current_client_changed &&
		    !(MB_WM_CLIENT_CLIENT_TYPE(current_client) &
		      MBWMClientTypeDesktop) && !STATE_IS_APP(hd_render_manager_get_state()))
			hd_render_manager_set_state(HDRM_STATE_APP);
	}
...
void hd_comp_mgr_sync_stacking(HdCompMgr * hmgr)
{
	...
	if (!priv->stack_sync)
		...
		priv->stack_sync = g_idle_add_full(0, (GSourceFunc) hd_comp_mgr_restack, hmgr, NULL);
}
 ...
 */
void hd_comp_mgr_close_app(HdCompMgr * hmgr, MBWMCompMgrClutterClient * cc, gboolean close_all)
{
	HdCompMgrPrivate *priv = hmgr->priv;
	...
	if (hd_comp_mgr_client_is_hibernating(h_client)) {
		...
		g_hash_table_remove(priv->hibernating_apps, GUINT_TO_POINTER(h_client->priv->hibernation_key));
...
void hd_comp_mgr_close_client(HdCompMgr * hmgr, MBWMCompMgrClutterClient * cc)
{
	hd_comp_mgr_close_app(hmgr, cc, FALSE);
}

void hd_comp_mgr_wakeup_client(HdCompMgr * hmgr, HdCompMgrClient * hclient)
{
...
static gboolean hd_comp_mgr_may_be_portrait(HdCompMgr * hmgr, gboolean assume_requested)
{
	MBWindowManager *wm;
	MBWindowManagerClient *c;
	...
	for (c = wm->stack_top; c && c != wm->desktop; c = c->stacked_below) {
...
#if 0
		if (!c->portrait_requested && !c->portrait_requested_inherited) {	/* Client explicity !REQUESTED portrait, obey. */
			PORTRAIT("PROHIBITED?");
		 ...
		 * Part 2/2.
		 */
		if (c->portrait_requested && c->window && c->window->ewmh_state & MBWMClientWindowEWMHStateFullscreen) {
...
void hd_comp_mgr_set_pip_flags(HdCompMgr * hmgr, gboolean enabled, gboolean portrait)
{
 ...
 * otherwise NULL.
 */
void hd_comp_mgr_portrait_or_not_portrait(MBWMCompMgr * mgr, MBWindowManagerClient * c)
{
	HdCompMgrPrivate *priv = HD_COMP_MGR(mgr)->priv;
	 ...
	 */
	if (STATE_IS_PORTRAIT_CAPABLE(hd_render_manager_get_state())) {	/* Landscape -> portrait?
	...
	} else if (STATE_IS_PORTRAIT(hd_render_manager_get_state())) {	/* Portrait -> landscape?
...
gboolean hd_comp_mgr_client_supports_portrait(MBWindowManagerClient * mbwmc)
{
	...
	return hd_transition_get_int("thp_tweaks", "forcerotation", 0) ? TRUE : mbwmc->portrait_supported;
}
...
static void hd_comp_mgr_check_do_not_disturb_flag(HdCompMgr * hmgr)
{
	...
	if (xwindow && (xwindow != ~0) && wm->desktop && xwindow != wm->desktop->window->xwindow) {
		...
		value = hd_util_get_win_prop_data_and_validate(wm->xdpy, xwindow, dnd, XA_INTEGER, 32, 1, NULL);
...
static void dump_clutter_actor_tree(ClutterActor * actor, GString * indent)
{
	const gchar *name;
	...
	g_debug("actor[%" G_GSIZE_FORMAT "]: %s%p (type=%s, name=%s, win=0x%lx), "
		...
		CLUTTER_ACTOR_IS_VISIBLE(actor) != 0, CLUTTER_ACTOR_IS_REACTIVE(actor) != 0);
	if (CLUTTER_IS_CONTAINER(actor)) {
		g_string_append_c(indent, ' ');
		clutter_container_foreach(CLUTTER_CONTAINER(actor), (ClutterCallback) dump_clutter_actor_tree, indent);
...
void hd_comp_mgr_dump_debug_info(const gchar * tag)
{
	...
	mb_wm_stack_enumerate_reverse(root->wm, mbwmc) {
		...
		g_debug(" client=%p, type=%d, size=%dx%d%+d%+d, trfor=%p, layer=%d, "
			...
			mbwmc->window ? mbwmc->window->xwin_group : 0, mbwmc->window ? mbwmc->window->name : "<unset>");
...
void hd_comp_mgr_update_applets_on_current_desktop_property(HdCompMgr * hmgr)
{
	...
	for (a = applets; a; a = a->next) {
		...
		if (STATE_NEED_DESKTOP(hd_render_manager_get_state()) &&
		    STATE_SHOW_APPLETS(hd_render_manager_get_state()) && !hd_dbus_display_is_off) {
			XChangeProperty(wm_client->wmref->xdpy,
					wm_client->window->xwindow,
					hd_comp_mgr_get_atom(hmgr,
							     HD_ATOM_HILDON_APPLET_ON_CURRENT_DESKTOP),
					XA_CARDINAL, 32, PropModeReplace, (const guchar *)&on_desktop, 1);
		} else {
...
gboolean hd_comp_mgr_is_whitelisted(MBWindowManager * wm, MBWindowManagerClient * c)
{
	...
	PORTRAIT("Whitelist: WName %s; Supp: %d; Req: %d; SuppInh: %d, ReqInh: %d", wname, c->portrait_supported,
		 c->portrait_requested, c->portrait_supported_inherited, c->portrait_requested_inherited);
#ifdef DEBUG_WINDOWS
	if (c->transient_for)
		PORTRAIT("Whitelist: Parent Sup: %d Req: %d", c->transient_for->portrait_supported,
			 c->transient_for->portrait_requested);
#endif
...
gboolean hd_comp_mgr_is_blacklisted(MBWindowManager * wm, MBWindowManagerClient * c)
{
	...
	blacklisted_by_desktopfile =
	    hd_comp_mgr_is_blacklisted_parse_desktop_file(wname, class_hint.res_class, c->window->pid);
...
gboolean hd_comp_mgr_is_orientationlock_enabled(MBWindowManager * wm, MBWindowManagerClient * c)
{
	...
	if (c->portrait_requested || c->portrait_requested_inherited) {
		PORTRAIT
		    ("%s: window with flags! requested: %d, requested_inh: %d, supported: %d Do not even try to lock it.",
		     __FUNCTION__, c->portrait_requested, c->portrait_requested_inherited, c->portrait_supported);
		return FALSE;
	}
