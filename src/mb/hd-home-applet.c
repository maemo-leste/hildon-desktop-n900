...
static Bool
hd_home_applet_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometry, MBWMClientReqGeomType flags);

static void hd_home_applet_theme_change(MBWindowManagerClient * client);

static void hd_home_applet_show(MBWindowManagerClient * client)
{
...
static void hd_home_applet_class_init(MBWMObjectClass * klass)
{
...
static void hd_home_applet_destroy(MBWMObject * this)
{
...
static int hd_home_applet_init(MBWMObject * this, va_list vap)
{
	...
	applet_id = hd_util_get_win_prop_data_and_validate(wm->xdpy,
							   win->xwindow, applet_id_atom, utf8_atom, 8, 0, &n_items);

	if (applet_id && isprint(*applet_id) && !isblank(*applet_id)) {
		g_debug("%s. Applet id: %s", __FUNCTION__, applet_id);
		applet->applet_id = g_strdup(applet_id);
	} else {
		applet->applet_id = g_strdup("hd-applet-no-id");
		g_warning("%s. No applet id! Using '%s' as fallback", __FUNCTION__, applet->applet_id);
	}
	...
	modified = gconf_client_get_string(gconf_client, modified_key, NULL);
	...
	if (modified) {
		...
		gconf_client_set_string(gconf_client, modified_key, modified, &error);
		if (G_UNLIKELY(error)) {
	...
	XChangeProperty(wm->xdpy, win->xwindow,
			...
			XA_ATOM, 32, PropModeReplace, (unsigned char *)actions, sizeof(actions) / sizeof(actions[0]));

	mb_wm_client_set_layout_hints(client, LayoutPrefMovable | LayoutPrefOverlaps | LayoutPrefVisible);
...
hd_home_applet_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometry, MBWMClientReqGeomType flags)
{
	...
	change_size = (geom->width != new_geometry->width || geom->height != new_geometry->height);
