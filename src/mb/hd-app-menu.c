...
static void hd_app_menu_realize(MBWindowManagerClient * client);
static Bool
hd_app_menu_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometry, MBWMClientReqGeomType flags);

static void hd_app_menu_class_init(MBWMObjectClass * klass)
{
...
static void hd_app_menu_destroy(MBWMObject * this)
{
	HdAppMenu *menu = HD_APP_MENU(this);
	MBWindowManagerClient *c = MB_WM_CLIENT(this);
	MBWindowManager *wm = c->wmref;

	mb_wm_main_context_x_event_handler_remove(wm->main_ctx, ButtonRelease, menu->release_cb_id);
}

static int hd_app_menu_init(MBWMObject * this, va_list vap)
{
