...
void hd_animation_actor_show(MBWindowManagerClient * client);
static Bool
hd_animation_actor_request_geometry(MBWindowManagerClient * client,
				    MBGeometry * new_geometry, MBWMClientReqGeomType flags);

static void hd_animation_actor_client_message(XClientMessageEvent * xev, void *userdata)
{
	...
	if (xev->message_type == show_atom) {
		...
		switch (axis) {
		case 0:
			clutter_axis = CLUTTER_X_AXIS;
			break;
		case 1:
			clutter_axis = CLUTTER_Y_AXIS;
			break;
		default:
			clutter_axis = CLUTTER_Z_AXIS;
		}

		clutter_actor_set_rotationx(actor, clutter_axis, degrees, x, y, z);
		...
		switch (gravity) {
		case 1:
			clutter_gravity = CLUTTER_GRAVITY_NORTH;
			break;
		case 2:
			clutter_gravity = CLUTTER_GRAVITY_NORTH_EAST;
			break;
		case 3:
			clutter_gravity = CLUTTER_GRAVITY_EAST;
			break;
		case 4:
			clutter_gravity = CLUTTER_GRAVITY_SOUTH_EAST;
			break;
		case 5:
			clutter_gravity = CLUTTER_GRAVITY_SOUTH;
			break;
		case 6:
			clutter_gravity = CLUTTER_GRAVITY_SOUTH_WEST;
			break;
		case 7:
			clutter_gravity = CLUTTER_GRAVITY_WEST;
			break;
		case 8:
			clutter_gravity = CLUTTER_GRAVITY_NORTH_WEST;
			break;
		case 9:
			clutter_gravity = CLUTTER_GRAVITY_CENTER;
			break;

		default:
			clutter_gravity = CLUTTER_GRAVITY_NONE;
			break;
		}
		...
		if (clutter_gravity == CLUTTER_GRAVITY_NONE) {
			...
			clutter_actor_move_anchor_point_from_gravity(actor, clutter_gravity);
		}
...
static void hd_animation_actor_realize(MBWindowManagerClient * client)
{
	...
	if (!atoms_initialized) {
		...
		show_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ANIMATION_CLIENT_MESSAGE_SHOW);
		position_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ANIMATION_CLIENT_MESSAGE_POSITION);
		rotation_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ANIMATION_CLIENT_MESSAGE_ROTATION);
		scale_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ANIMATION_CLIENT_MESSAGE_SCALE);
		anchor_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ANIMATION_CLIENT_MESSAGE_ANCHOR);
		parent_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ANIMATION_CLIENT_MESSAGE_PARENT);
		ready_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_ANIMATION_CLIENT_READY);
	...
	self->client_message_handler_id =
	    mb_wm_main_context_x_event_handler_add(wm->main_ctx, window, ClientMessage, (MBWMXEventFunc)
						   hd_animation_actor_client_message, client);
...
static void hd_animation_actor_class_init(MBWMObjectClass * klass)
{
...
static void hd_animation_actor_destroy(MBWMObject * this)
{
...
static int hd_animation_actor_init(MBWMObject * this, va_list vap)
{
	MBWindowManagerClient *client = MB_WM_CLIENT(this);
	MBWMClientWindow *win = client->window;
	MBWindowManager *wm = client->wmref;
	...
	hd_animation_actor_request_geometry(client, &geom, MBWMClientReqGeomForced);
...
int hd_animation_actor_class_type()
{
	static int type = 0;
