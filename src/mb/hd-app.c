...
static Bool
hd_app_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometry, MBWMClientReqGeomType flags);

static void hd_app_detransitise(MBWindowManagerClient * client);

static void hd_app_class_init(MBWMObjectClass * klass)
{
#if MBWM_WANT_DEBUG
...
static void hd_app_destroy(MBWMObject * this)
{
}

static void delete_open_menus(MBWindowManager * wm)
{
...
static int hd_app_init(MBWMObject * this, va_list vap)
{
...
MBWindowManagerClient *hd_app_new(MBWindowManager * wm, MBWMClientWindow * win)
{
	...
	client
	    = MB_WM_CLIENT(mb_wm_object_new(HD_TYPE_APP, MBWMObjectPropWm, wm, MBWMObjectPropClientWindow, win, NULL));
...
hd_app_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometry, MBWMClientReqGeomType flags)
{
	...
	if ((flags & MBWMClientReqGeomIsViaLayoutManager)
	    && (hd_comp_mgr_is_portrait() || hd_transition_is_rotating_to_portrait())
	...
	if (client->decor && (decor = client->decor->data)
	    && decor->type == MBWMDecorTypeNorth && decor->buttons && (button = decor->buttons->data)
	    && button->type == MBWMDecorButtonClose) {
		MBGeometry geo;

		geo = button->geom;
		geo.width = new_geometry->width > new_geometry->height
		    ? HD_COMP_MGR_TOP_RIGHT_BTN_WIDTH : HD_COMP_MGR_TOP_RIGHT_BTN_WIDTH_SMALL;
...
static void hd_app_detransitise(MBWindowManagerClient * client)
{
	...
	app->detransitised_from = client->window ? client->window->xwin_transient_for : None;

	klass = MB_WM_CLIENT_CLASS(MB_WM_OBJECT_GET_PARENT_CLASS(MB_WM_OBJECT(client)));
