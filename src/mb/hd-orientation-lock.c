...
G_DEFINE_TYPE_WITH_CODE(HdOrientationLock, hd_orientation_lock, G_TYPE_OBJECT, G_ADD_PRIVATE(HdOrientationLock));
...
{
	if (G_UNLIKELY(!the_orientation_lock)) {	/* "Protect" against reentrancy. */
...
static void hd_orientation_lock_class_init(HdOrientationLockClass * klass)
{
...
static void hd_orientation_lock_init(HdOrientationLock * self)
{
	...
	if (priv->gconf_client) {
		...
		priv->orientation_lock_landscape = gconf_client_get_bool(priv->gconf_client,
									 GCONF_KEY_ORIENTATION_LOCK_POSITION, NULL);
		...
		gconf_client_notify_add(priv->gconf_client, GCONF_KEY_ORIENTATION_LOCK,
					hd_orientation_lock_gconf_value_changed, (gpointer) self, NULL, NULL);
	}
}
...
static void
hd_orientation_lock_gconf_value_changed(GConfClient * client, guint cnxn_id, GConfEntry * entry, gpointer user_data)
{
