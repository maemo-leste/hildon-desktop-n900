...
void hd_remote_texture_show(MBWindowManagerClient * client);
static Bool
hd_remote_texture_request_geometry(MBWindowManagerClient * client,
				   MBGeometry * new_geometry, MBWMClientReqGeomType flags);
static void hd_remote_texture_set_shm(HdRemoteTexture * tex, key_t key, guint width, guint height, guint bpp);

static Bool hd_remote_texture_client_message(XClientMessageEvent * xev, void *userdata)
{
	...
	if (xev->message_type == shm_atom) {
		...
		hd_remote_texture_set_shm(self, shm_key, shm_width, shm_height, shm_bpp);

		CM_DEBUG("RemoteTexture %p: shm(key=%d, width=%d, height=%d, bpp=%d)\n",
			 self, shm_key, shm_width, shm_height, shm_bpp);
		...
		CM_DEBUG("RemoteTexture %p: " "damage(x=%d, y=%d, width=%d, height=%d)\n", self, x, y, width, height);
		...
		clutter_actor_set_clip(CLUTTER_ACTOR(self->texture), 0, 0, width, height);
		...
		if (win != 0) {
			...
			parent_client = mb_wm_managed_client_from_xwindow(client->wmref, win);

			if (parent_client)
				parent_cclient = MB_WM_COMP_MGR_CLUTTER_CLIENT(parent_client->cm_client);
			...
			clutter_container_add_actor(CLUTTER_CONTAINER(actor), CLUTTER_ACTOR(self->texture));
		}
		...
		CM_DEBUG("RemoteTexture %p: UNKNOWN MESSAGE %lu (%lu,%lu,%lu,%lu,%lu)\n",
			 self,
			 xev->message_type,
			 xev->data.l[0], xev->data.l[1], xev->data.l[2], xev->data.l[3], xev->data.l[4]);
		return False;
	}
...
static void hd_remote_texture_realize(MBWindowManagerClient * client)
{
	...
	if (!atoms_initialized) {
		...
		shm_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_MESSAGE_SHM);
		damage_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_MESSAGE_DAMAGE);
		show_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_MESSAGE_SHOW);
		position_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_MESSAGE_POSITION);
		offset_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_MESSAGE_OFFSET);
		scale_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_MESSAGE_SCALE);
		parent_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_MESSAGE_PARENT);
		ready_atom = hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_TEXTURE_CLIENT_READY);
	...
	self->client_message_handler_id =
	    mb_wm_main_context_x_event_handler_add(wm->main_ctx, window, ClientMessage, (MBWMXEventFunc)
						   hd_remote_texture_client_message, client);
	...
	long val = 1;
	XChangeProperty(wm->xdpy, window, ready_atom, XA_ATOM, 32, PropModeReplace, (unsigned char *)&val, 1);
}

static void hd_remote_texture_class_init(MBWMObjectClass * klass)
{
...
static void hd_remote_texture_destroy(MBWMObject * this)
{
	...
	if (self->client_message_handler_id) {
		mb_wm_main_context_x_event_handler_remove(wm->main_ctx, ClientMessage, self->client_message_handler_id);
	}
...
static int hd_remote_texture_init(MBWMObject * this, va_list vap)
{
	...
	mb_wm_client_set_layout_hints(client, LayoutPrefPositionFree | LayoutPrefMovable | LayoutPrefVisible);
	hd_remote_texture_request_geometry(client, &geom, MBWMClientReqGeomForced);
				   ...
				   MBGeometry * new_geometry, MBWMClientReqGeomType flags)
{
	g_debug("%s: x=%d y=%d w=%d h=%d f=0x%x\n",
		__FUNCTION__, new_geometry->x, new_geometry->y, new_geometry->width, new_geometry->height, flags);
...
int hd_remote_texture_class_type()
{
	static int type = 0;
...
static void hd_remote_texture_set_shm(HdRemoteTexture * tex, key_t key, guint width, guint height, guint bpp)
{
	...
	if (tex->shm_addr) {
		tidy_mem_texture_set_data(tex->texture, 0, 0, 0, 0);
	...
	tidy_mem_texture_set_data(tex->texture, tex->shm_addr, tex->shm_width, tex->shm_height, tex->shm_bpp);
}
