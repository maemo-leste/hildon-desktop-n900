...
static MBWindowManagerClient *hd_wm_client_new(MBWindowManager *, MBWMClientWindow *);
static MBWMCompMgr *hd_wm_comp_mgr_new(MBWindowManager * wm);
static void hd_wm_client_responding(MBWindowManager * wm, MBWindowManagerClient * c);
static Bool hd_wm_client_hang(MBWindowManager * wm, MBWindowManagerClient * c);

static Bool hd_wm_client_activate(MBWindowManager * wm, MBWindowManagerClient * c);
...
static MBWMCompMgr *hd_wm_comp_mgr_new(MBWindowManager * wm)
{
	...
	mgr = (MBWMCompMgr *) mb_wm_object_new(HD_TYPE_COMP_MGR, MBWMObjectPropWm, wm, NULL);

	return mgr;
}

static MBWindowManagerClient *hd_wm_client_new(MBWindowManager * wm, MBWMClientWindow * win)
{
	HdCompMgr *hmgr = HD_COMP_MGR(wm->comp_mgr);
	MBWindowManagerClass *wm_class = MB_WINDOW_MANAGER_CLASS(MB_WM_OBJECT_GET_PARENT_CLASS(MB_WM_OBJECT(wm)));
	...
	if (win->override_redirect ||
	...
	else if (win->net_type == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_WM_WINDOW_TYPE_HOME_APPLET)) {
		g_debug("### is home applet ###");
		return hd_home_applet_new(wm, win);
	} else if (win->net_type == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_WM_WINDOW_TYPE_APP_MENU)) {
		g_debug("### is application menu ###");
		return hd_app_menu_new(wm, win);
	} else if (win->net_type == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_WM_WINDOW_TYPE_STATUS_AREA)) {
		g_debug("### is status area ###");
		return hd_status_area_new(wm, win);
	} else if (win->net_type == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_WM_WINDOW_TYPE_STATUS_MENU)) {
	...
	} else if (win->net_type == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_WM_WINDOW_TYPE_ANIMATION_ACTOR)) {
		g_debug("### is animation actor ###");
		return hd_animation_actor_new(wm, win);
	} else if (win->net_type == hd_comp_mgr_get_atom(hmgr, HD_ATOM_HILDON_WM_WINDOW_TYPE_REMOTE_TEXTURE)) {
		...
		if (name) {
			g_warning("### unhandled window type %s (%lx) ###", name, win->xwindow);
			XFree(name);
		} else
			g_warning("### unhandled window type [no net_type] (%lx) ###", win->xwindow);
		return hd_app_new(wm, win);
	}
}

#if 0
static gboolean show_info_note(gpointer data)
{
	gchar *s = data;
...
static void hd_wm_client_responding(MBWindowManager * wm, MBWindowManagerClient * client)
{
	HdWm *hdwm = HD_WM(wm);
	...
	if (hdwm->priv->hung_client_dialog) {
		...
		snprintf(buf, 200, _("tana_ib_apkil_responded"), name ? name : "NO NAME");
		...
		gtk_dialog_response(GTK_DIALOG(hdwm->priv->hung_client_dialog), GTK_RESPONSE_REJECT);
	}
}
...
static GtkWidget *hd_wm_make_dialog(MBWindowManagerClient * client)
{
	char buf[200];
	...
	if (!name)
		name = mb_wm_client_get_name(client);
	snprintf(buf, 200, _("tana_nc_apkil_notresponding"), name ? name : "NO NAME");
...
gboolean hd_wm_close_modal_blockers(const MBWindowManager * wm)
{
	...
	for (client = wm->stack_top; client && client != wm->desktop; client = client->stacked_below) {
...
void hd_wm_delete_temporaries(MBWindowManager * wm)
{
	...
	for (c = wm->stack_top; c; c = c->stacked_below)
		if (MB_WM_CLIENT_CLIENT_TYPE(c) & HdWmClientTypeAppMenu) {	/* HdAppMenu understands WM_DELETE. */
			mb_wm_client_deliver_delete(c);
			return;
		} else if (MB_WM_CLIENT_CLIENT_TYPE(c) & MBWMClientTypeMenu) {	/* GtkMenu wants DELETE_TEMPORARIES. */
