...
static char *get_x_window_string_property(HdNote * self, HdAtoms atom_id)
{
	return hd_util_get_x_window_string_property(MB_WM_CLIENT(self)->wmref,
						    MB_WM_CLIENT(self)->window->xwindow, atom_id);
}

static void resize_note(XConfigureEvent * xev, MBWindowManagerClient * client)
{
	int n, s, w, e;
	MBGeometry geom;
	MBWindowManager *wm;
	...
	/* Set geom.y */
	if (HD_NOTE(client)->note_type == HdNoteTypeBanner) {	/* Display right below the application title bar. */
		MBWMXmlClient *c;
		MBWMXmlDecor *d;
		...
		geom.y = wm->theme
		    ...
		    && (d = mb_wm_xml_decor_find_by_type(c->decors, MBWMDecorTypeNorth)) != NULL ? d->height : 40;
...
static void x_window_property_changed(XPropertyEvent * event, HdNote * self)
{
	guint i;
...
static void hd_note_show(MBWindowManagerClient * client)
{
...
static void hd_note_class_init(MBWMObjectClass * klass)
{
...
static void hd_note_destroy(MBWMObject * this)
{
	HdNote *note = HD_NOTE(this);

	if (note->modal_blocker_cb_id)
		mb_wm_main_context_x_event_handler_remove(MB_WM_CLIENT(this)->wmref->main_ctx,
							  ButtonRelease, note->modal_blocker_cb_id);
	if (note->note_type == HdNoteTypeBanner
	    || note->note_type == HdNoteTypeInfo || note->note_type == HdNoteTypeConfirmation)
		mb_wm_main_context_x_event_handler_remove(MB_WM_CLIENT(this)->wmref->main_ctx,
							  ConfigureNotify, note->screen_size_changed_cb_id);
	if (note->note_type == HdNoteTypeIncomingEvent) {
		guint i;

		mb_wm_main_context_x_event_handler_remove(MB_WM_CLIENT(this)->wmref->main_ctx,
							  PropertyNotify, note->property_changed_cb_id);
		for (i = 0; i < G_N_ELEMENTS(IEProperties); i++)
...
static int hd_note_init(MBWMObject * this, va_list vap)
{
	...
	prop = get_x_window_string_property(note, HD_ATOM_HILDON_NOTIFICATION_TYPE);
	if (prop != NULL) {
	...
	if (note->note_type == HdNoteTypeIncomingEvent) {
		...
		note->property_changed_cb_id =
		    mb_wm_main_context_x_event_handler_add(wm->main_ctx, client->window->xwindow, PropertyNotify,
	...
	if (note->note_type == HdNoteTypeIncomingEventPreview) {
		...
		note->screen_size_changed_cb_id =
		    mb_wm_main_context_x_event_handler_add(wm->main_ctx,
							   wm->root_win->xwindow,
							   ConfigureNotify, (MBWMXEventFunc) resize_note, client);
	}

	return 1;
}

int hd_note_class_type()
{
	static int type = 0;
...
static void hd_note_realize(MBWindowManagerClient * client)
{
	...
	if (HD_NOTE(client)->note_type == HdNoteTypeInfo)
		...
		HD_NOTE(client)->modal_blocker_cb_id = hd_util_modal_blocker_realize(client, FALSE);
	else if (HD_NOTE(client)->note_type == HdNoteTypeConfirmation)
		/* Ping confirmation notes when clicked outside. */
		HD_NOTE(client)->modal_blocker_cb_id = hd_util_modal_blocker_realize(client, TRUE);
}

static Bool
hd_note_request_geometry(MBWindowManagerClient * client, MBGeometry * new_geometryp, MBWMClientReqGeomType flags)
{
	 ...
	 */
	geom = (flags & MBWMClientReqGeomIsViaConfigureReq) ? &client->window->geometry : &client->frame_geometry;
	...
	if (change_size || (flags & MBWMClientReqGeomForced)) {
		...
		if (client->decor)
			mb_wm_theme_get_decor_dimensions(wm->theme, client, &north, &south, &west, &east);
		...
		if (flags & MBWMClientReqGeomIsViaConfigureReq) {
			...
			MBWM_DBG("ConfigureRequest [%d,%d;%dx%d] -> [%d,%d;%dx%d]\n",
				 ...
				 new_geometry.x, new_geometry.y, new_geometry.width, new_geometry.height);
 ...
 * an incoming event. */
MBWMStackLayerType hd_note_stacking_layer(MBWindowManagerClient * client)
{
	...
	if (dialog && dialog == client->window->xwindow) {
		...
		for (c = wm->stack_top; c && c != wm->desktop; c = c->stacked_below) {
			if (mb_wm_client_is_map_confirmed(c) && HD_WM_CLIENT_CLIENT_TYPE(c) == HdWmClientTypeAppMenu)
...
MBWindowManagerClient *hd_note_new(MBWindowManager * wm, MBWMClientWindow * win)
{
	...
	client
	    = MB_WM_CLIENT(mb_wm_object_new(HD_TYPE_NOTE, MBWMObjectPropWm, wm, MBWMObjectPropClientWindow, win, NULL));
...
static void hd_note_stack(MBWindowManagerClient * client, int flags)
{
	if ((HD_NOTE(client)->note_type == HdNoteTypeBanner ||
	     HD_NOTE(client)->note_type == HdNoteTypeInfo) && mb_wm_client_get_transient_for(client)) {
