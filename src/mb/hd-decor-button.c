...
static void hd_decor_button_class_init(MBWMObjectClass * klass)
{
...
static void hd_decor_button_destroy(MBWMObject * obj)
{
}

static int hd_decor_button_init(MBWMObject * obj, va_list vap)
{
	MBWMObjectProp prop;
				   ...
				   MBWMDecorButtonReleasedFunc release, MBWMDecorButtonFlags flags)
{
	...
	decorbutton = HD_DECOR_BUTTON(mb_wm_object_new(HD_TYPE_DECOR_BUTTON,
						       MBWMObjectPropWm, wm,
						       ...
						       MBWMObjectPropDecorButtonFlags, flags, NULL));

	return decorbutton;
}

int hd_decor_button_class_type()
{
	static int type = 0;
...
void hd_decor_button_sync(HdDecorButton * button)
{
	...
	hd_title_bar_right_pressed(bar, mbbutton->state != MBWMDecorButtonStateInactive);
}
