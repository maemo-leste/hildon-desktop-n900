...
void hd_comp_mgr_close_client(HdCompMgr * hmgr, MBWMCompMgrClutterClient * c);
void hd_comp_mgr_hibernate_client(HdCompMgr * hmgr, MBWMCompMgrClutterClient * c, gboolean force);
...
static inline guint hd_comp_mgr_get_current_screen_width(void)
{
	extern MBWindowManager *hd_mb_wm;
	return hd_mb_wm->xdpy_width;
}

static inline guint hd_comp_mgr_get_current_screen_height(void)
{
	extern MBWindowManager *hd_mb_wm;
	return hd_mb_wm->xdpy_height;
}

static inline HdCompMgr *hd_comp_mgr_get(void)
{
	extern MBWindowManager *hd_mb_wm;
	return HD_COMP_MGR(hd_mb_wm->comp_mgr);
}

static inline gboolean hd_comp_mgr_is_portrait(void)
{				/* This is a very typesafe macro. */
	extern MBWindowManager *hd_mb_wm;
	return hd_mb_wm->xdpy_width < hd_mb_wm->xdpy_height;
}

static inline gboolean hd_comp_mgr_client_is_maximized(MBGeometry geom)
{
	extern MBWindowManager *hd_mb_wm;

	if (geom.x > 0 || geom.y > 0)
		return FALSE;
	if (geom.x + geom.width >= hd_mb_wm->xdpy_width - 1 && geom.y + geom.height >= hd_mb_wm->xdpy_height - 1)
		return TRUE;
	if (geom.x + geom.width >= hd_mb_wm->xdpy_height - 1 && geom.y + geom.height >= hd_mb_wm->xdpy_width - 1)
