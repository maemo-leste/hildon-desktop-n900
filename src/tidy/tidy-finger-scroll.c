...
G_DEFINE_TYPE_WITH_CODE(TidyFingerScroll, tidy_finger_scroll, TIDY_TYPE_SCROLL_VIEW, G_ADD_PRIVATE(TidyFingerScroll))
...
static void tidy_finger_scroll_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec)
{
...
static void
tidy_finger_scroll_set_property(GObject * object, guint property_id, const GValue * value, GParamSpec * pspec)
{
	TidyFingerScrollPrivate *priv = TIDY_FINGER_SCROLL(object)->priv;
...
static void tidy_finger_scroll_dispose(GObject * object)
{
	TidyFingerScrollPrivate *priv = TIDY_FINGER_SCROLL(object)->priv;
...
static void tidy_finger_scroll_class_init(TidyFingerScrollClass * klass)
{
	...
	g_object_class_install_property(object_class,
					...
					g_param_spec_enum("mode",
							  ...
							  TIDY_FINGER_SCROLL_MODE_PUSH, G_PARAM_READWRITE));
	...
	g_object_class_install_property(object_class,
					...
					g_param_spec_uint("motion-buffer",
							  "Motion buffer",
							  "Amount of motion "
							  "events to buffer", 1, G_MAXUINT, 3, G_PARAM_READWRITE));
}

static gboolean motion_event_cb(ClutterActor * actor, ClutterMotionEvent * event, TidyFingerScroll * scroll)
{
	ClutterUnit x, y;
	...
	if (clutter_actor_transform_stage_point(actor,
						...
						CLUTTER_UNITS_FROM_DEVICE(event->y), &x, &y)) {
		...
		if (child) {
			...
			tidy_scrollable_get_adjustments(TIDY_SCROLLABLE(child), &hadjust, &vadjust);

			motion = &g_array_index(priv->motion_buffer, TidyFingerScrollMotion, priv->last_motion);
			dx = CLUTTER_UNITS_TO_FIXED(motion->x - x) + tidy_adjustment_get_valuex(hadjust);
			dy = CLUTTER_UNITS_TO_FIXED(motion->y - y) + tidy_adjustment_get_valuex(vadjust);
...
static void hfade_complete_cb(ClutterActor * scrollbar, TidyFingerScroll * scroll)
{
	scroll->priv->hscroll_timeline = NULL;
}

static void vfade_complete_cb(ClutterActor * scrollbar, TidyFingerScroll * scroll)
{
	scroll->priv->vscroll_timeline = NULL;
}

static void stop_scrollbars(TidyFingerScroll * scroll)
{
	TidyFingerScrollPrivate *priv = scroll->priv;
...
static void show_scrollbars(TidyFingerScroll * scroll, gboolean show)
{
	...
	if (!CLUTTER_ACTOR_IS_REACTIVE(hscroll))
		priv->hscroll_timeline = clutter_effect_fade(show ? priv->template_in : priv->template_out,
							     hscroll,
							     show ? 0xFF : 0x00,
							     (ClutterEffectCompleteFunc) hfade_complete_cb, scroll);

	if (!CLUTTER_ACTOR_IS_REACTIVE(vscroll))
		priv->vscroll_timeline = clutter_effect_fade(show ? priv->template_in : priv->template_out,
							     vscroll,
							     show ? 0xFF : 0x00,
							     (ClutterEffectCompleteFunc) vfade_complete_cb, scroll);
}

static void deceleration_completed_cb(ClutterTimeline * timeline, TidyFingerScroll * scroll)
{
...
get_next_delta(TidyFingerScroll * scroll,
	       ...
	       ClutterFixed value, ClutterFixed prevdiff, ClutterFixed upper, ClutterFixed highest)
{
	TidyFingerScrollPrivate *priv = scroll->priv;
	...
	decel_rate = in_lower_danger_zone || in_upper_danger_zone ? priv->bouncing_decel_rate : priv->decel_rate;
...
advance_value(ClutterFixed lowest, ClutterFixed lower,
	      ClutterFixed * valuep, ClutterFixed diff, ClutterFixed upper, ClutterFixed highest)
{
	static const gboolean likeable = FALSE;

	*valuep += diff;
	if (!likeable) {
		if (*valuep < lowest) {	/* Bounce back from the top. */
			diff -= *valuep - lowest;
			*valuep = lowest;
		} else if (*valuep < lower && 0 < diff && diff < CFX_ONE) {	/* Leaving the top danger zone, snap it to the boundary. */
			diff = 0;
			*valuep = lower;
		} else if (*valuep > upper && -CFX_ONE < diff && diff < 0) {	/* Same for the bottom danger zone. */
			diff = 0;
			*valuep = upper;
		} else if (*valuep > highest) {	/* Bounce back from the bottom. */
			diff -= *valuep - highest;
			*valuep = highest;
		}
	}
 ...
 */
static void deceleration_new_frame_cb(ClutterTimeline * timeline, gint frame_num, TidyFingerScroll * scroll)
{
	TidyFingerScrollPrivate *priv = scroll->priv;
	ClutterActor *child;
	TidyAdjustment *hadjust, *vadjust;
	...
	tidy_adjustment_get_valuesx(hadjust, &hvalue, &hlower, &hupper, NULL, NULL, &hpage);
	hupper -= hpage;

	tidy_adjustment_get_skirtx(vadjust, &vlowest, &vhighest);
	tidy_adjustment_get_valuesx(vadjust, &vvalue, &vlower, &vupper, NULL, NULL, &vpage);
	vupper -= vpage;

	/* We need to keep track of how many frames were skipped so we can
	 * make up for it... */
	for (; priv->deceleration_timeline_lastframe < frame_num; priv->deceleration_timeline_lastframe++) {
		priv->dx = advance_value(hlowest, hlower, &hvalue, priv->dx, hupper, hhighest);
		priv->dx = get_next_delta(scroll, hlowest, hlower, hvalue, priv->dx, hupper, hhighest);
		priv->dy = advance_value(vlowest, vlower, &vvalue, priv->dy, vupper, vhighest);
		priv->dy = get_next_delta(scroll, vlowest, vlower, vvalue, priv->dy, vupper, vhighest);
	}
	...
	if (vlower <= vvalue && vvalue <= vupper
	    ...
	    && hlower <= hvalue && hvalue <= hupper && -CFX_ONE < priv->dx && priv->dx < CFX_ONE)
 ...
 */
static ClutterFixed initial_speed(TidyFingerScroll * scroll, TidyAdjustment * adjust, ClutterFixed diff)
{
	TidyFingerScrollPrivate *priv = scroll->priv;
	ClutterFixed lower, value, upper, page;

	tidy_adjustment_get_valuesx(adjust, &value, &lower, &upper, NULL, NULL, &page);
	upper -= page;

	if (diff > 0 && value < lower && value + clutter_qmulx(diff, 1 - priv->bouncing_decel_rate) < lower)
		return clutter_qmulx(lower - value, priv->bounce_back_speed_rate);
	else if (diff < 0 && value > upper && value + clutter_qmulx(diff, 1 - priv->bouncing_decel_rate) > upper)
		return clutter_qmulx(upper - value, priv->bounce_back_speed_rate);
	else
		return diff;
}

static gboolean button_release_event_cb(ClutterActor * actor, ClutterButtonEvent * event, TidyFingerScroll * scroll)
{
	TidyFingerScrollPrivate *priv = scroll->priv;
	ClutterActor *child = tidy_scroll_view_get_child(TIDY_SCROLL_VIEW(scroll));
	...
	g_signal_handlers_disconnect_by_func(actor, motion_event_cb, scroll);
	g_signal_handlers_disconnect_by_func(actor, button_release_event_cb, scroll);
	...
	if ((priv->mode == TIDY_FINGER_SCROLL_MODE_KINETIC) && (child)) {
		...
		if (clutter_actor_transform_stage_point(actor,
			...
			x_origin = CLUTTER_UNITS_FROM_FIXED(clutter_qdivx(CLUTTER_UNITS_TO_FIXED(x_origin),
									  CLUTTER_INT_TO_FIXED(priv->last_motion)));
			y_origin = CLUTTER_UNITS_FROM_FIXED(clutter_qdivx(CLUTTER_UNITS_TO_FIXED(y_origin),
									  CLUTTER_INT_TO_FIXED(priv->last_motion)));
			...
			priv->dx = CLUTTER_UNITS_FROM_FIXED(clutter_qdivx(CLUTTER_UNITS_TO_FIXED(x_origin - x), frac));
			priv->dy = CLUTTER_UNITS_FROM_FIXED(clutter_qdivx(CLUTTER_UNITS_TO_FIXED(y_origin - y), frac));
			...
			if (ABS(CLUTTER_UNITS_TO_INT(priv->dx)) > 1 || ABS(CLUTTER_UNITS_TO_INT(priv->dy)) > 1) {
				...
				tidy_adjustment_get_values(hadjust, &value, &lower, NULL, &step_increment, NULL, NULL);
				d = ((rint(((value + d) - lower) / step_increment) * step_increment) + lower) - value;
				priv->dx = CLUTTER_UNITS_FROM_FLOAT(d / a);

				/* Solving for dy */
				d = a * CLUTTER_UNITS_TO_FLOAT(priv->dy);
				tidy_adjustment_get_values(vadjust, &value, &lower, NULL, &step_increment, NULL, NULL);
				d = ((rint(((value + d) - lower) / step_increment) * step_increment) + lower) - value;
				...
				tidy_adjustment_get_values(hadjust, &value, &lower, NULL, &step_increment, NULL, NULL);
				d = ((rint((value - lower) / step_increment) * step_increment) + lower) - value;
				priv->dx = CLUTTER_UNITS_FROM_FLOAT(d / a);
...
static gboolean after_event_cb(TidyFingerScroll * scroll)
{
	...
	if (clutter_get_pointer_grab() != CLUTTER_ACTOR(scroll)) {
		...
		g_signal_handlers_disconnect_by_func(scroll, button_release_event_cb, scroll);
...
static gboolean captured_event_cb(ClutterActor * actor, ClutterEvent * event)
{
	...
	if (event->type == CLUTTER_BUTTON_PRESS) {
		...
		if ((bevent->button == 1) &&
			 ...
			 */
			g_idle_add_full(G_PRIORITY_HIGH_IDLE, (GSourceFunc) after_event_cb, scroll, NULL);

			g_signal_connect(actor, "motion-event", G_CALLBACK(motion_event_cb), scroll);
			g_signal_connect(actor, "button-release-event", G_CALLBACK(button_release_event_cb), scroll);
		}
	}
...
static void hscroll_notify_reactive_cb(ClutterActor * bar, GParamSpec * pspec, TidyFingerScroll * scroll)
{
...
static void vscroll_notify_reactive_cb(ClutterActor * bar, GParamSpec * pspec, TidyFingerScroll * scroll)
{
	TidyFingerScrollPrivate *priv;
...
static void tidy_finger_scroll_init(TidyFingerScroll * self)
{
	...
	priv->motion_buffer = g_array_sized_new(FALSE, TRUE, sizeof(TidyFingerScrollMotion), 3);
	g_array_set_size(priv->motion_buffer, 3);
	priv->decel_rate = CLUTTER_FLOAT_TO_FIXED(hd_transition_get_double("launcher", "deceleration_rate", 0.99));
	priv->bouncing_decel_rate =
	    CLUTTER_FLOAT_TO_FIXED(hd_transition_get_double("launcher", "strong_deceleration_rate", 0.7));
	...
	priv->bounce_back_speed_rate = clutter_qdivx(CFX_ONE - priv->bouncing_decel_rate, CFX_ONE - qn);
	...
	g_signal_connect(scrollbar, "notify::reactive", G_CALLBACK(hscroll_notify_reactive_cb), self);

	scrollbar = tidy_scroll_view_get_vscroll_bar(TIDY_SCROLL_VIEW(self));
	clutter_actor_set_reactive(scrollbar, FALSE);
	clutter_actor_set_opacity(scrollbar, 0x00);
	g_signal_connect(scrollbar, "notify::reactive", G_CALLBACK(vscroll_notify_reactive_cb), self);

	effect_timeline_in = clutter_timeline_new_for_duration(TIDY_FINGER_SCROLL_FADE_SCROLLBAR_IN_TIME);
	priv->template_in = clutter_effect_template_new(effect_timeline_in, CLUTTER_ALPHA_RAMP_INC);
	effect_timeline_out = clutter_timeline_new_for_duration(TIDY_FINGER_SCROLL_FADE_SCROLLBAR_OUT_TIME);
	priv->template_out = clutter_effect_template_new(effect_timeline_out, CLUTTER_ALPHA_RAMP_INC);
}

ClutterActor *tidy_finger_scroll_new(TidyFingerScrollMode mode)
{
	return CLUTTER_ACTOR(g_object_new(TIDY_TYPE_FINGER_SCROLL, "mode", mode, NULL));
}
...
static gboolean _tidy_finger_scroll_show_scrollbars_cb(ClutterActor * scroll)
{
	TidyFingerScrollPrivate *priv;
...
static void _tidy_finger_scroll_hide_scrollbars_later(TidyFingerScroll * scroll)
{
	...
	if (priv->scrollbar_timeout)
		g_source_remove(priv->scrollbar_timeout);
	priv->scrollbar_timeout = g_timeout_add(TIDY_FINGER_SCROLL_INITIAL_SCROLLBAR_DELAY,
						(GSourceFunc) _tidy_finger_scroll_show_scrollbars_cb, scroll);
}

/* Show the scrollbars and then fade them out */
void tidy_finger_scroll_show_scrollbars(ClutterActor * scroll)
{
	TidyFingerScrollPrivate *priv;
...
void tidy_finger_scroll_hide_scrollbars_now(ClutterActor * actor)
{
	...
	clutter_actor_set_opacity(tidy_scroll_view_get_hscroll_bar(TIDY_SCROLL_VIEW(actor)), 0);
	clutter_actor_set_opacity(tidy_scroll_view_get_vscroll_bar(TIDY_SCROLL_VIEW(actor)), 0);
}
