...
G_DEFINE_TYPE_WITH_CODE(TidyCachedGroup, tidy_cached_group, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(TidyCachedGroup));

/* An implementation for the ClutterGroup::paint() vfunc,
   painting all the child actors: */
static void tidy_cached_group_paint(ClutterActor * actor)
{
	...
	if (priv->cache_amount < 0.01 || width == 0 || height == 0) {
		/* render direct */
		CLUTTER_ACTOR_CLASS(tidy_cached_group_parent_class)->paint(actor);
		return;
	}
#ifdef __i386__
	if (!cogl_features_available(COGL_FEATURE_OFFSCREEN)) {	/* If we can't render offscreen properly, just render normally. */
		CLUTTER_ACTOR_CLASS(tidy_cached_group_parent_class)->paint(actor);
		return;
	}
#endif
	...
	if (!priv->tex) {
		...
		priv->tex = cogl_texture_new_with_size(tex_width, tex_height, 0, FALSE /*mipmap */ ,
	...
	cogl_texture_rectangle(priv->tex,
			       0, 0, CLUTTER_INT_TO_FIXED(width), CLUTTER_INT_TO_FIXED(height), 0, 0, CFX_ONE, CFX_ONE);
	if (rotate_90) {
		cogl_pop_matrix();
	}
}

static void tidy_cached_group_dispose(GObject * gobject)
{
...
static void tidy_cached_group_class_init(TidyCachedGroupClass * klass)
{
...
static void tidy_cached_group_init(TidyCachedGroup * self)
{
...
void tidy_cached_group_set_downsampling_factor(ClutterActor * cached_group, float downsample)
{
	g_return_if_fail(downsample >= 0);
	TIDY_CACHED_GROUP(cached_group)->priv->downsample = downsample ? : TIDY_CACHED_GROUP_DEFAULT_DOWNSAMPLING;
}
