...
static void on_style_change(TidyStyle * style, TidyScrollBar * bar);
...
G_DEFINE_TYPE_WITH_CODE(TidyScrollBar, tidy_scroll_bar, TIDY_TYPE_ACTOR, G_ADD_PRIVATE(TidyScrollBar)
			G_IMPLEMENT_INTERFACE(TIDY_TYPE_STYLABLE, tidy_stylable_iface_init))
...
static void tidy_scroll_bar_get_property(GObject * gobject, guint prop_id, GValue * value, GParamSpec * pspec)
{
...
static void tidy_scroll_bar_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static void tidy_scroll_bar_dispose(GObject * gobject)
{
	TidyScrollBar *bar = TIDY_SCROLL_BAR(gobject);
...
static void tidy_scroll_bar_paint(ClutterActor * actor)
{
...
static void tidy_scroll_bar_pick(ClutterActor * actor, const ClutterColor * pick_color)
{
	TidyScrollBarPrivate *priv = TIDY_SCROLL_BAR(actor)->priv;

	CLUTTER_ACTOR_CLASS(tidy_scroll_bar_parent_class)->pick(actor, pick_color);

	if (priv->handle && CLUTTER_ACTOR_IS_VISIBLE(priv->handle) && priv->adjustment)
		clutter_actor_paint(priv->handle);
}

static void
tidy_scroll_bar_allocate(ClutterActor * actor, const ClutterActorBox * box, gboolean absolute_origin_changed)
{
	TidyScrollBarPrivate *priv = TIDY_SCROLL_BAR(actor)->priv;

	/* Chain up */
	CLUTTER_ACTOR_CLASS(tidy_scroll_bar_parent_class)->allocate(actor, box, absolute_origin_changed);

	if (priv->texture)
		clutter_actor_set_sizeu(priv->texture, box->x2 - box->x1, box->y2 - box->y1);
	...
	if (priv->adjustment) {
		...
		tidy_adjustment_get_valuesx(priv->adjustment, NULL, &lower, &upper, NULL, NULL, &page_size);
		...
		tidy_stylable_get(TIDY_STYLABLE(actor), "min-size", &min_size, "max-size", &max_size, NULL);
		...
		child_box.x2 = child_box.x1 + MIN(max_sizeu, MAX(min_sizeu, CLUTTER_UNITS_FROM_FIXED(size)));
		child_box.y2 = child_box.y1 + real_height;

		clutter_actor_allocate(priv->handle, &child_box, absolute_origin_changed);
	}
}
...
static void bar_reactive_notify_cb(GObject * gobject, GParamSpec * arg1, gpointer user_data)
{
	TidyScrollBar *bar = TIDY_SCROLL_BAR(gobject);

	clutter_actor_set_reactive(bar->priv->handle, clutter_actor_get_reactive(CLUTTER_ACTOR(bar)));
}

static GObject *tidy_scroll_bar_constructor(GType type, guint n_properties, GObjectConstructParam * properties)
{
	...
	g_signal_connect(bar, "notify::reactive", G_CALLBACK(bar_reactive_notify_cb), NULL);
	g_signal_connect(tidy_stylable_get_style(TIDY_STYLABLE(bar)), "changed", G_CALLBACK(on_style_change), bar);
...
static void tidy_scroll_bar_class_init(TidyScrollBarClass * klass)
{
	...
	g_object_class_install_property
	    (object_class,
	     ...
	     g_param_spec_object("adjustment",
				 "Adjustment", "The adjustment", TIDY_TYPE_ADJUSTMENT, TIDY_PARAM_READWRITE));
	...
	g_object_class_install_property
	    (object_class,
	     ...
	     g_param_spec_object("handle",
				 "Handle",
				 "Actor to use for the scrollbars handle", CLUTTER_TYPE_ACTOR, TIDY_PARAM_READWRITE));
}
...
static void tidy_stylable_iface_init(TidyStylableIface * iface)
{
	...
	if (!is_initialized) {
		...
		pspec = g_param_spec_uint("min-size",
					  ...
					  0, G_MAXUINT, 32, G_PARAM_READWRITE);
		tidy_stylable_iface_install_property(iface, TIDY_TYPE_SCROLL_BAR, pspec);

		pspec = g_param_spec_uint("max-size",
					  "Maximum grabber size",
					  "Maximum size of the scroll grabber, in px",
					  0, G_MAXINT16, G_MAXINT16, G_PARAM_READWRITE);
		tidy_stylable_iface_install_property(iface, TIDY_TYPE_SCROLL_BAR, pspec);
	}
}

static void move_slider(TidyScrollBar * bar, gint x, gint y, gboolean interpolate)
{
	...
	if (!clutter_actor_transform_stage_point(CLUTTER_ACTOR(bar),
						 CLUTTER_UNITS_FROM_DEVICE(x), CLUTTER_UNITS_FROM_DEVICE(y), &ux, NULL))
		return;

	width = clutter_actor_get_widthu(CLUTTER_ACTOR(bar)) - clutter_actor_get_widthu(priv->handle);
	...
	position =
	    clutter_qmulx(clutter_qdivx(CLUTTER_UNITS_TO_FIXED(ux),
					CLUTTER_UNITS_TO_FIXED(width)), upper - lower - page_size) + lower;
	...
	if (interpolate) {
		...
		tidy_adjustment_interpolatex(priv->adjustment, position, n_frames, fps);
		return;
	}
...
static gboolean motion_event_cb(TidyScrollBar * bar, ClutterMotionEvent * event, gpointer user_data)
{
...
static gboolean button_release_event_cb(TidyScrollBar * bar, ClutterButtonEvent * event, gpointer user_data)
{
	if (event->button != 1)
		return FALSE;
...
static gboolean button_press_event_cb(ClutterActor * actor, ClutterButtonEvent * event, TidyScrollBar * bar)
{
	TidyScrollBarPrivate *priv = bar->priv;
	...
	if (!clutter_actor_transform_stage_point(actor,
						 ...
						 CLUTTER_UNITS_FROM_DEVICE(event->y), &priv->x_origin, NULL))
		return FALSE;

	g_signal_connect_after(bar, "motion-event", G_CALLBACK(motion_event_cb), NULL);
	g_signal_connect_after(bar, "button-release-event", G_CALLBACK(button_release_event_cb), NULL);
...
static void tidy_scroll_bar_init(TidyScrollBar * self)
{
...
{
	return g_object_new(TIDY_TYPE_SCROLL_BAR, "adjustment", adjustment, NULL);
}

ClutterActor *tidy_scroll_bar_new_with_handle(TidyAdjustment * adjustment, ClutterActor * handle)
{
	return g_object_new(TIDY_TYPE_SCROLL_BAR, "adjustment", adjustment, "handle", handle, NULL);
}

static gboolean tidy_scroll_bar_refresh(TidyScrollBar * bar)
{
	...
	tidy_adjustment_get_valuesx(priv->adjustment, &value, &lower, &upper, NULL, NULL, &page_size);
	...
	clutter_actor_set_positionu(CLUTTER_ACTOR(priv->handle), CLUTTER_UNITS_FROM_FIXED(x), 0);
...
static void adjustment_changed_cb(TidyScrollBar * bar)
{
	TidyScrollBarPrivate *priv = bar->priv;

	if (!priv->refresh_source)
		priv->refresh_source = g_idle_add_full(CLUTTER_PRIORITY_REDRAW, (GSourceFunc)
						       tidy_scroll_bar_refresh, bar, NULL);
}
...
void tidy_scroll_bar_set_adjustment(TidyScrollBar * bar, TidyAdjustment * adjustment)
{
	...
	priv = bar->priv;
	if (priv->adjustment) {
		g_signal_handlers_disconnect_by_func(priv->adjustment, clutter_actor_queue_relayout, bar);
		g_signal_handlers_disconnect_by_func(priv->adjustment, adjustment_changed_cb, bar);
	...
	if (adjustment) {
		...
		g_signal_connect_swapped(priv->adjustment, "notify::value", G_CALLBACK(adjustment_changed_cb), bar);
		g_signal_connect_swapped(priv->adjustment, "changed", G_CALLBACK(clutter_actor_queue_relayout), bar);
...
void tidy_scroll_bar_set_handle(TidyScrollBar * bar, ClutterActor * handle)
{
	...
	if (priv->handle) {
		g_signal_handlers_disconnect_by_func(priv->handle, G_CALLBACK(button_press_event_cb), bar);
	...
	g_signal_connect_after(priv->handle, "button-press-event", G_CALLBACK(button_press_event_cb), bar);
