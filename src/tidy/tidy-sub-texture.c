...
G_DEFINE_TYPE_WITH_CODE(TidySubTexture, tidy_sub_texture, CLUTTER_TYPE_ACTOR, G_ADD_PRIVATE(TidySubTexture));
				     ...
				     ClutterUnit for_height, ClutterUnit * min_width_p, ClutterUnit * natural_width_p)
{
	...
	parent_texture_class->get_preferred_width(parent_texture, for_height, min_width_p, natural_width_p);
}
				      ...
				      ClutterUnit for_width, ClutterUnit * min_height_p, ClutterUnit * natural_height_p)
{
	...
	parent_texture_class = CLUTTER_ACTOR_GET_CLASS(parent_texture);
	parent_texture_class->get_preferred_height(parent_texture, for_width, min_height_p, natural_height_p);
}

static void tidy_sub_texture_paint(ClutterActor * self)
{
	...
	if (!priv->tiled) {
		...
		cogl_texture_rectangle(cogl_texture, 0, 0,
				       ...
				       CLUTTER_INT_TO_FIXED(height), t_x, t_y, t_x + t_w, t_y + t_h);
	} else {
		gint x, y, c, cx, cy;
		...
		if (cx > 0 && cy > 0) {
			...
			/* render! */
			cogl_texture_triangles(cogl_texture, 6 * c, verts, FALSE);
			g_free(verts);
		}
	}
}

static void set_parent_texture(TidySubTexture * ctexture, ClutterTexture * texture)
{
	...
	if (texture) {
		...
		if (CLUTTER_ACTOR_IS_VISIBLE(priv->parent_texture) && was_visible) {
...
static void tidy_sub_texture_dispose(GObject * object)
{
...
static void tidy_sub_texture_finalize(GObject * object)
{
	G_OBJECT_CLASS(tidy_sub_texture_parent_class)->finalize(object);
}

static void tidy_sub_texture_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static void tidy_sub_texture_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
	TidySubTexture *ctexture = TIDY_SUB_TEXTURE(object);
...
static void tidy_sub_texture_class_init(TidySubTextureClass * klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
	ClutterActorClass *actor_class = CLUTTER_ACTOR_CLASS(klass);

	actor_class->paint = tidy_sub_texture_paint;
	actor_class->get_preferred_width = tidy_sub_texture_get_preferred_width;
	actor_class->get_preferred_height = tidy_sub_texture_get_preferred_height;
...
static void tidy_sub_texture_init(TidySubTexture * self)
{
	TidySubTexturePrivate *priv;
...
TidySubTexture *tidy_sub_texture_new(ClutterTexture * texture)
{
	...
	return g_object_new(TIDY_TYPE_SUB_TEXTURE, "parent-texture", texture, NULL);
}
