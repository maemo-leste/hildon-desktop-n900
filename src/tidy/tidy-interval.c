...
G_DEFINE_TYPE_WITH_CODE(TidyInterval, tidy_interval, G_TYPE_INITIALLY_UNOWNED, G_ADD_PRIVATE(TidyInterval));

static void tidy_interval_finalize(GObject * gobject)
{
...
static void tidy_interval_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static void tidy_interval_get_property(GObject * gobject, guint prop_id, GValue * value, GParamSpec * pspec)
{
	TidyIntervalPrivate *priv = TIDY_INTERVAL_GET_PRIVATE(gobject);
...
static void tidy_interval_class_init(TidyIntervalClass * klass)
{
...
static void tidy_interval_init(TidyInterval * self)
{
...
static void tidy_interval_set_interval_valist(TidyInterval * interval, va_list var_args)
{
...
static void tidy_interval_get_interval_valist(TidyInterval * interval, va_list var_args)
{
	GType gtype = interval->priv->value_type;
	GValue value = { 0, };
	gchar *error;
...
TidyInterval *tidy_interval_new_with_values(GType gtype, const GValue * initial, const GValue * final)
{
	TidyInterval *retval;
...
TidyInterval *tidy_interval_clone(TidyInterval * interval)
{
	...
	tidy_interval_set_initial_value(retval, tidy_interval_peek_initial_value(interval));
	tidy_interval_set_final_value(retval, tidy_interval_peek_final_value(interval));
...
void tidy_interval_set_initial_value(TidyInterval * interval, const GValue * value)
{
	TidyIntervalPrivate *priv;
...
void tidy_interval_get_initial_value(TidyInterval * interval, GValue * value)
{
	TidyIntervalPrivate *priv;
...
void tidy_interval_set_final_value(TidyInterval * interval, const GValue * value)
{
	TidyIntervalPrivate *priv;
...
void tidy_interval_get_final_value(TidyInterval * interval, GValue * value)
{
	TidyIntervalPrivate *priv;
