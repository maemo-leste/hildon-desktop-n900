...
static void tidy_stylable_notify_dispatcher(GObject * gobject, guint n_pspecs, GParamSpec ** pspecs)
{
	guint i;

	for (i = 0; i < n_pspecs; i++)
		g_signal_emit(gobject, stylable_signals[STYLE_NOTIFY], g_quark_from_string(pspecs[i]->name), pspecs[i]);
}

static void tidy_stylable_base_finalize(gpointer g_iface)
{
	GList *list, *node;

	list = g_param_spec_pool_list_owned(style_property_spec_pool, G_TYPE_FROM_INTERFACE(g_iface));
...
static void tidy_stylable_base_init(gpointer g_iface)
{
	...
	if (G_UNLIKELY(!initialised)) {
		...
		property_notify_context.quark_notify_queue =
		    g_quark_from_static_string("TidyStylable-style-property-notify-queue");
       ...
       */
		g_object_interface_install_property(g_iface,
						    g_param_spec_object("style",
									"Style",
									"A style object",
									TIDY_TYPE_STYLE, TIDY_PARAM_READWRITE));
       ...
       */
		stylable_signals[STYLE_SET] =
		    g_signal_new(I_("style-set"),
				 ...
				 NULL, NULL, _tidy_marshal_VOID__OBJECT, G_TYPE_NONE, 1, TIDY_TYPE_STYLE);
		stylable_signals[STYLE_NOTIFY] =
		    g_signal_new(I_("style-notify"),
				 iface_type,
				 G_SIGNAL_RUN_FIRST | G_SIGNAL_NO_RECURSE | G_SIGNAL_DETAILED | G_SIGNAL_NO_HOOKS |
				 G_SIGNAL_ACTION, G_STRUCT_OFFSET(TidyStylableIface, style_notify), NULL, NULL,
				 _tidy_marshal_VOID__PARAM, G_TYPE_NONE, 1, G_TYPE_PARAM);
	}
}

GType tidy_stylable_get_type(void)
{
	...
	if (G_UNLIKELY(our_type == 0)) {
		...
		our_type = g_type_register_static(G_TYPE_INTERFACE, I_("TidyStylable"), &stylable_info, 0);
	}
...
void tidy_stylable_thaw_notify(TidyStylable * stylable)
{
	...
	nqueue = g_object_notify_queue_from_object(G_OBJECT(stylable), &property_notify_context);
...
void tidy_stylable_notify(TidyStylable * stylable, const gchar * property_name)
{
	GParamSpec *pspec;
	...
	pspec = g_param_spec_pool_lookup(style_property_spec_pool, property_name, G_OBJECT_TYPE(stylable), TRUE);

	if (!pspec)
		g_warning("%s: object class `%s' has no style property named `%s'",
			  G_STRFUNC, G_OBJECT_TYPE_NAME(stylable), property_name);
	else {
		GObjectNotifyQueue *nqueue;

		nqueue = g_object_notify_queue_freeze(G_OBJECT(stylable), &property_notify_context);
 ...
 */
void tidy_stylable_iface_install_property(TidyStylableIface * iface, GType owner_type, GParamSpec * pspec)
{
	...
	if (g_param_spec_pool_lookup(style_property_spec_pool, pspec->name, owner_type, FALSE)) {
		g_warning("%s: class `%s' already contains a style property named `%s'",
			  G_STRLOC, g_type_name(owner_type), pspec->name);
		return;
	}

	g_param_spec_ref_sink(pspec);
	g_param_spec_set_qdata_full(pspec, quark_real_owner, g_strdup(g_type_name(owner_type)), g_free);

	g_param_spec_pool_insert(style_property_spec_pool, pspec, owner_type);
}
 ...
 *   g_free() when done.
 */
GParamSpec **tidy_stylable_list_properties(TidyStylable * stylable, guint * n_props)
{
	...
	pspecs = g_param_spec_pool_list(style_property_spec_pool, G_OBJECT_TYPE(stylable), &n);
	if (n_props)
		*n_props = n;
 ...
 */
GParamSpec *tidy_stylable_find_property(TidyStylable * stylable, const gchar * property_name)
{
	g_return_val_if_fail(TIDY_IS_STYLABLE(stylable), NULL);
	g_return_val_if_fail(property_name != NULL, NULL);

	return g_param_spec_pool_lookup(style_property_spec_pool, property_name, G_OBJECT_TYPE(stylable), TRUE);
}

static inline void
tidy_stylable_set_property_internal(TidyStylable * stylable,
				    GParamSpec * pspec, const GValue * value, GObjectNotifyQueue * nqueue)
{
	GValue tmp_value = { 0, };
	...
	if (!g_value_transform(value, &tmp_value))
		g_warning("unable to set property `%s' of type `%s' from value of type `%s'",
			  pspec->name, g_type_name(G_PARAM_SPEC_VALUE_TYPE(pspec)), G_VALUE_TYPE_NAME(value));
	else if (g_param_value_validate(pspec, &tmp_value) && !(pspec->flags & G_PARAM_LAX_VALIDATION)) {
		gchar *contents = g_strdup_value_contents(value);

		g_warning("value \"%s\" of type `%s' is invalid or out of range for property `%s' of type `%s'",
			  contents, G_VALUE_TYPE_NAME(value), pspec->name, g_type_name(G_PARAM_SPEC_VALUE_TYPE(pspec)));
		g_free(contents);
	} else {
		TidyStyle *style = tidy_stylable_get_style(stylable);
		gchar *real_name;

		real_name = g_strconcat(g_param_spec_get_qdata(pspec, quark_real_owner), "::", pspec->name, NULL);

		if (!tidy_style_has_property(style, real_name))
			tidy_style_add_property(style, real_name, G_PARAM_SPEC_VALUE_TYPE(pspec));
...
static inline void tidy_stylable_get_property_internal(TidyStylable * stylable, GParamSpec * pspec, GValue * value)
{
	TidyStyle *style;
	GValue real_value = { 0, };
	gchar *real_name;
 ...
 * into @value.
 */
void tidy_stylable_get_property(TidyStylable * stylable, const gchar * property_name, GValue * value)
{
	GParamSpec *pspec;
	...
	if (!pspec) {
		g_warning("Stylable class `%s' doesn't have a property named `%s'",
			  g_type_name(G_OBJECT_TYPE(stylable)), property_name);
		return;
	}

	if (!(pspec->flags & G_PARAM_READABLE)) {
		g_warning("Style property `%s' of class `%s' is not readable",
			  pspec->name, g_type_name(G_OBJECT_TYPE(stylable)));
		return;
	}
 ...
 */
void tidy_stylable_set_property(TidyStylable * stylable, const gchar * property_name, const GValue * value)
{
	GObjectNotifyQueue *nqueue;
	GParamSpec *pspec;
...
static void tidy_stylable_get_valist(TidyStylable * stylable, const gchar * first_property_name, va_list varargs)
{
	const gchar *name;
	...
	while (name) {
		...
		if (!pspec) {
			g_warning("%s: no style property named `%s' found for class `%s'",
				  G_STRLOC, name, g_type_name(G_OBJECT_TYPE(stylable)));
			break;
		}
...
static void tidy_stylable_set_valist(TidyStylable * stylable, const gchar * first_property_name, va_list varargs)
{
	GObjectNotifyQueue *nqueue;
	const gchar *name;
 ...
 * </example>
 */
void tidy_stylable_get(TidyStylable * stylable, const gchar * first_property_name, ...)
{
	va_list args;
 ...
 */
void tidy_stylable_set(TidyStylable * stylable, const gchar * first_property_name, ...)
{
	va_list args;
 ...
 * will be emitted.
 */
void tidy_stylable_set_style(TidyStylable * stylable, TidyStyle * style)
{
	TidyStylableIface *iface;
	TidyStyle *old_style;
	...
	if (iface->set_style)
		...
		g_object_set_qdata_full(G_OBJECT(stylable), quark_style, g_object_ref_sink(style), g_object_unref);
	}
