...
G_DEFINE_TYPE_WITH_CODE(TidyStyle, tidy_style, G_TYPE_OBJECT, G_ADD_PRIVATE(TidyStyle));

static StyleProperty *style_property_new(const gchar * value_name, GType value_type)
{
...
static void style_property_free(gpointer data)
{
	if (G_LIKELY(data)) {
...
static StyleEffect *style_effect_new(const gchar * name)
{
	StyleEffect *retval;
...
static void style_effect_free(gpointer data)
{
	if (G_LIKELY(data)) {
...
static void init_defaults(TidyStyle * style)
{
...
static gboolean tidy_style_load_from_file(TidyStyle * style, const gchar * filename, GError ** error)
{
	GKeyFile *rc_file;
	...
	if (internal_error) {
		 ...
		 */
		if (internal_error->domain == G_FILE_ERROR && internal_error->code == G_FILE_ERROR_NOENT) {
...
static void tidy_style_load(TidyStyle * style)
{
	const gchar *env_var;
	gchar *rc_file = NULL;
	GError *error;
	...
	if (!rc_file)
		rc_file = g_build_filename(g_get_user_config_dir(), "tidy", "tidyrc", NULL);

	error = NULL;
	if (!tidy_style_load_from_file(style, rc_file, &error)) {
		g_critical("Unable to load resource file `%s': %s", rc_file, error->message);
		g_error_free(error);
	}
...
static void tidy_style_finalize(GObject * gobject)
{
...
static void tidy_style_class_init(TidyStyleClass * klass)
{
...
static void tidy_style_init(TidyStyle * style)
{
	TidyStylePrivate *priv;

	style->priv = priv = TIDY_STYLE_GET_PRIVATE(style);

	priv->properties = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, style_property_free);
	priv->effects = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, style_effect_free);
...
static StyleProperty *tidy_style_find_property(TidyStyle * style, const gchar * property_name)
{
	return g_hash_table_lookup(style->priv->properties, property_name);
}

gboolean tidy_style_has_property(TidyStyle * style, const gchar * property_name)
{
...
void tidy_style_add_property(TidyStyle * style, const gchar * property_name, GType property_type)
{
	...
	if (G_UNLIKELY(property)) {
		g_warning("A property named `%s', with type %s already exists.",
			  property->value_name, g_type_name(property->value_type));
		return;
	}
...
void tidy_style_get_property(TidyStyle * style, const gchar * property_name, GValue * value)
{
	StyleProperty *property;
...
void tidy_style_set_property(TidyStyle * style, const gchar * property_name, const GValue * value)
{
	StyleProperty *property;
...
static StyleEffect *tidy_style_find_effect(TidyStyle * style, const gchar * effect_name)
{
	return g_hash_table_lookup(style->priv->effects, effect_name);
}

void tidy_style_add_effect(TidyStyle * style, const gchar * effect_name)
{
	StyleEffect *effect;
	...
	if (G_UNLIKELY(effect)) {
		g_warning("An effect named `%s', with type %s already exists.",
			  effect->name, g_type_name(effect->behaviour_type));
		return;
	}

	effect = style_effect_new(effect_name);
	g_hash_table_replace(style->priv->effects, effect->name, effect);
}

gboolean tidy_style_has_effect(TidyStyle * style, const gchar * effect_name)
{
	g_return_val_if_fail(TIDY_IS_STYLE(style), FALSE);
...
tidy_style_set_effect_valist(TidyStyle * style,
			     StyleEffect * effect, const gchar * first_property_name, va_list varargs)
{
	GObjectClass *klass;
	const gchar *name;
	...
	while (name) {
		...
		if (!pspec) {
			g_warning("Unable to find the property `%s' for the "
				  "behaviour of type `%s'", name, g_type_name(effect->behaviour_type));
			break;
		}

		if (!(pspec->flags & G_PARAM_WRITABLE)) {
			g_warning("The property `%s' for the behaviour of type "
				  "`%s' is not writable", pspec->name, g_type_name(effect->behaviour_type));
			break;
		}
...
tidy_style_set_effect(TidyStyle * style,
		      ...
		      guint duration,
		      GType behaviour_type, ClutterAlphaFunc alpha_func, const gchar * first_property_name, ...)
{
	StyleEffect *effect;
	va_list args;
...
tidy_style_set_effectv(TidyStyle * style,
		       const gchar * effect_name,
		       guint duration,
		       GType behaviour_type, ClutterAlphaFunc alpha_func, guint n_parameters, GParameter * parameters)
{
	StyleEffect *effect;
	gint i;
...
static ClutterBehaviour *tidy_style_construct_effect(TidyStyle * style, const gchar * effect_name)
{
	...
	G_GNUC_BEGIN_IGNORE_DEPRECATIONS
	    behaviour = g_object_newv(effect->behaviour_type,
				      effect->parameters->len, (GParameter *) effect->parameters->data);
	G_GNUC_END_IGNORE_DEPRECATIONS clutter_behaviour_set_alpha(behaviour, alpha);

	/* we just unref the behaviour, which will take care of cleaning
	 * up everything (alpha+timeline)
	 */
	g_signal_connect_swapped(timeline, "completed", G_CALLBACK(g_object_unref), behaviour);

	return behaviour;
}

ClutterTimeline *tidy_style_get_effect(TidyStyle * style, const gchar * effect_name, ClutterActor * actor)
{
	ClutterBehaviour *behaviour;
	ClutterAlpha *alpha;
	ClutterTimeline *timeline;
