...
G_DEFINE_TYPE_WITH_CODE(TidyScrollView, tidy_scroll_view, TIDY_TYPE_ACTOR, G_ADD_PRIVATE(TidyScrollView)
...
static void tidy_scroll_view_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec)
{
...
static void tidy_scroll_view_set_property(GObject * object, guint property_id, const GValue * value, GParamSpec * pspec)
{
	switch (property_id) {
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, property_id, pspec);
...
static void tidy_scroll_view_dispose(GObject * object)
{
...
static void tidy_scroll_view_finalize(GObject * object)
{
	G_OBJECT_CLASS(tidy_scroll_view_parent_class)->finalize(object);
}

static void tidy_scroll_view_paint(ClutterActor * actor)
{
...
static void tidy_scroll_view_pick(ClutterActor * actor, const ClutterColor * color)
{
...
tidy_scroll_view_get_preferred_width(ClutterActor * actor,
				     ClutterUnit for_height, ClutterUnit * min_width_p, ClutterUnit * natural_width_p)
{
	TidyPadding padding;
	guint xthickness;
	...
	tidy_stylable_get(TIDY_STYLABLE(actor), "xthickness", &xthickness, NULL);

	/* Our natural width is the natural width of the child */
	clutter_actor_get_preferred_width(priv->child, for_height, NULL, natural_width_p);
	...
	if ((for_height >= 0) && natural_width_p) {
		...
		clutter_actor_get_preferred_height(priv->child,
						   -CLUTTER_UNITS_FROM_FIXED(CFX_ONE), NULL, &natural_height);
...
tidy_scroll_view_get_preferred_height(ClutterActor * actor,
				      ClutterUnit for_width, ClutterUnit * min_height_p, ClutterUnit * natural_height_p)
{
	TidyPadding padding;
	guint ythickness;
	...
	tidy_actor_get_padding(TIDY_ACTOR(actor), &padding);
	tidy_stylable_get(TIDY_STYLABLE(actor), "ythickness", &ythickness, NULL);

	/* Our natural height is the natural height of the child */
	clutter_actor_get_preferred_height(priv->child, for_width, NULL, natural_height_p);
	...
	if ((for_width >= 0) && natural_height_p) {
		...
		clutter_actor_get_preferred_width(priv->child,
						  -CLUTTER_UNITS_FROM_FIXED(CFX_ONE), NULL, &natural_width);
...
static void
tidy_scroll_view_allocate(ClutterActor * actor, const ClutterActorBox * box, gboolean absolute_origin_changed)
{
	TidyPadding padding;
	...
	/* Chain up */
	CLUTTER_ACTOR_CLASS(tidy_scroll_view_parent_class)->allocate(actor, box, absolute_origin_changed);

	tidy_actor_get_padding(TIDY_ACTOR(actor), &padding);

	tidy_stylable_get(TIDY_STYLABLE(actor), "xthickness", &xthickness, "ythickness", &ythickness, NULL);
	xthicknessu = CLUTTER_ACTOR_IS_VISIBLE(priv->vscroll) ? CLUTTER_UNITS_FROM_INT(xthickness) : 0;
	ythicknessu = CLUTTER_ACTOR_IS_VISIBLE(priv->hscroll) ? CLUTTER_UNITS_FROM_INT(ythickness) : 0;

	/* Vertical scrollbar */
	child_box.x1 = box->x2 - box->x1 - padding.right;
	child_box.x2 = MAX(0, (box->y2 - box->y1 - ythicknessu)) + child_box.x1 - padding.top - padding.bottom;
	child_box.y1 = padding.top;
	child_box.y2 = MIN(xthicknessu, box->x2 - box->x1) + padding.top;

	clutter_actor_allocate(priv->vscroll, &child_box, absolute_origin_changed);
	...
	clutter_actor_allocate(priv->hscroll, &child_box, absolute_origin_changed);
...
static void tidy_scroll_view_class_init(TidyScrollViewClass * klass)
{
	...
	g_object_class_install_property(object_class,
					...
					g_param_spec_object("hscroll",
							    ...
							    TIDY_TYPE_SCROLL_BAR, G_PARAM_READABLE));
	...
	g_object_class_install_property(object_class,
					...
					g_param_spec_object("child",
							    "ClutterActor",
							    "Child actor", CLUTTER_TYPE_ACTOR, G_PARAM_READABLE));
}
...
static void tidy_stylable_iface_init(TidyStylableIface * iface)
{
	...
	if (!is_initialized) {
		...
		pspec = g_param_spec_uint("xthickness",
					  ...
					  "Thickness of vertical scrollbar, in px", 0, G_MAXUINT, 8, G_PARAM_READWRITE);
		...
		pspec = g_param_spec_uint("ythickness",
					  ...
					  0, G_MAXUINT, 8, G_PARAM_READWRITE);
		tidy_stylable_iface_install_property(iface, TIDY_TYPE_SCROLL_VIEW, pspec);
	}
}

static void child_adjustment_changed_cb(TidyAdjustment * adjustment, ClutterActor * bar)
{
	...
	tidy_adjustment_get_valuesx(adjustment, NULL, &lower, &upper, NULL, NULL, &page_size);
...
static void child_hadjustment_notify_cb(GObject * gobject, GParamSpec * arg1, gpointer user_data)
{
	...
	if (hadjust)
		g_signal_handlers_disconnect_by_func(hadjust, child_adjustment_changed_cb, priv->hscroll);
	...
	if (hadjust) {
		...
		g_signal_connect(hadjust, "changed", G_CALLBACK(child_adjustment_changed_cb), priv->hscroll);
		child_adjustment_changed_cb(hadjust, priv->hscroll);
	}
}

static void child_vadjustment_notify_cb(GObject * gobject, GParamSpec * arg1, gpointer user_data)
{
	...
	if (vadjust)
		g_signal_handlers_disconnect_by_func(vadjust, child_adjustment_changed_cb, priv->vscroll);
	...
	if (vadjust) {
		...
		g_signal_connect(vadjust, "changed", G_CALLBACK(child_adjustment_changed_cb), priv->vscroll);
		child_adjustment_changed_cb(vadjust, priv->vscroll);
	}
}

static void tidy_scroll_view_init(TidyScrollView * self)
{
	...
	tidy_stylable_set_property(TIDY_STYLABLE(priv->hscroll), "bg-color", &transparent);
	tidy_stylable_set_property(TIDY_STYLABLE(priv->vscroll), "bg-color", &transparent);
...
static void tidy_scroll_view_add_actor(ClutterContainer * container, ClutterActor * actor)
{
	...
	if (priv->child) {
		g_warning("Attempting to add an actor of type %s to "
			  ...
			  "an actor of type %s.",
			  g_type_name(G_OBJECT_TYPE(actor)), g_type_name(G_OBJECT_TYPE(priv->child)));
	} else {
		if (TIDY_IS_SCROLLABLE(actor)) {
			...
			g_warning("Attempting to add an actor of type %s to "
				  ...
				  "not implement TidyScrollable.", g_type_name(G_OBJECT_TYPE(actor)));
		}
	}
}

static void tidy_scroll_view_remove_actor(ClutterContainer * container, ClutterActor * actor)
{
	...
	if (actor == priv->child) {
		...
		g_signal_handlers_disconnect_by_func(priv->child, child_hadjustment_notify_cb, container);
		g_signal_handlers_disconnect_by_func(priv->child, child_vadjustment_notify_cb, container);
...
static void tidy_scroll_view_foreach(ClutterContainer * container, ClutterCallback callback, gpointer callback_data)
{
	TidyScrollViewPrivate *priv = TIDY_SCROLL_VIEW(container)->priv;
...
static void tidy_scroll_view_lower(ClutterContainer * container, ClutterActor * actor, ClutterActor * sibling)
{
	/* single child */
}

static void tidy_scroll_view_raise(ClutterContainer * container, ClutterActor * actor, ClutterActor * sibling)
{
	/* single child */
}

static void tidy_scroll_view_sort_depth_order(ClutterContainer * container)
{
	/* single child */
}
