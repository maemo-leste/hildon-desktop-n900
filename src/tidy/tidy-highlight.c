...
G_DEFINE_TYPE_WITH_CODE(TidyHighlight, tidy_highlight, CLUTTER_TYPE_ACTOR, G_ADD_PRIVATE(TidyHighlight));
...
static void tidy_highlight_paint(ClutterActor * self)
{
	...
	if (priv->shader) {
		...
		clutter_shader_set_uniform_1f(priv->shader, "blurx", priv->amount / tex_width);
		clutter_shader_set_uniform_1f(priv->shader, "blury", priv->amount / tex_height);
	}
	...
	overlapx = CLUTTER_FLOAT_TO_FIXED(((x_2 - x_1) - tex_width) / (float)(tex_width * 2));
	overlapy = CLUTTER_FLOAT_TO_FIXED(((y_2 - y_1) - tex_height) / (float)(tex_height * 2));
...
static void set_parent_texture(TidyHighlight * ctexture, ClutterTexture * texture)
{
...
static void tidy_highlight_dispose(GObject * object)
{
...
static void tidy_highlight_finalize(GObject * object)
{
	G_OBJECT_CLASS(tidy_highlight_parent_class)->finalize(object);
}

static void tidy_highlight_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static void tidy_highlight_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
	TidyHighlight *ctexture = TIDY_HIGHLIGHT(object);
...
static void tidy_highlight_class_init(TidyHighlightClass * klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
	ClutterActorClass *actor_class = CLUTTER_ACTOR_CLASS(klass);

	actor_class->paint = tidy_highlight_paint;
...
static void tidy_highlight_init(TidyHighlight * self)
{
	TidyHighlightPrivate *priv;
