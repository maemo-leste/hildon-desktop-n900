...
G_DEFINE_TYPE_WITH_CODE(TidyFrame, tidy_frame, TIDY_TYPE_ACTOR, G_ADD_PRIVATE(TidyFrame)
			       ...
			       ClutterUnit for_height, ClutterUnit * min_width_p, ClutterUnit * natural_width_p)
{
	...
	if (priv->child) {
		...
		clutter_actor_get_preferred_width(priv->child, for_height, &child_min, &child_natural);
				...
				ClutterUnit for_width, ClutterUnit * min_height_p, ClutterUnit * natural_height_p)
{
	...
	if (priv->child) {
		...
		clutter_actor_get_preferred_height(priv->child, for_width, &child_min, &child_natural);
...
static void tidy_frame_allocate(ClutterActor * actor, const ClutterActorBox * box, gboolean origin_changed)
{
	TidyFramePrivate *priv = TIDY_FRAME(actor)->priv;
	...
	if (priv->child) {
		...
		clutter_actor_get_preferred_size(priv->child, NULL, NULL, &child_width, &child_height);
		...
		child_box.x1 = CLUTTER_FIXED_MUL((available_width - child_width), x_align)
		    + padding.left;
		child_box.y1 = CLUTTER_FIXED_MUL((available_height - child_height), y_align)
		    + padding.top;
...
static void tidy_frame_paint(ClutterActor * actor)
{
...
static void tidy_frame_pick(ClutterActor * actor, const ClutterColor * pick_color)
{
	TidyFramePrivate *priv = TIDY_FRAME(actor)->priv;
...
static void tidy_frame_dispose(GObject * gobject)
{
...
static void tidy_frame_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec)
{
	switch (prop_id) {
	case PROP_CHILD:
		clutter_container_add_actor(CLUTTER_CONTAINER(gobject), g_value_get_object(value));
		break;

	case PROP_TEXTURE:
		tidy_frame_set_texture(TIDY_FRAME(gobject), g_value_get_object(value));
		break;
...
static void tidy_frame_get_property(GObject * gobject, guint prop_id, GValue * value, GParamSpec * pspec)
{
	TidyFramePrivate *priv = TIDY_FRAME(gobject)->priv;
...
static void tidy_frame_class_init(TidyFrameClass * klass)
{
	...
	g_object_class_install_property(gobject_class,
					...
					g_param_spec_object("child",
							    ...
							    CLUTTER_TYPE_ACTOR, TIDY_PARAM_READWRITE));
	g_object_class_install_property(gobject_class,
					PROP_TEXTURE,
...
static void tidy_frame_init(TidyFrame * frame)
{
	frame->priv = TIDY_FRAME_GET_PRIVATE(frame);
}

static void tidy_frame_add_actor(ClutterContainer * container, ClutterActor * actor)
{
...
static void tidy_frame_remove_actor(ClutterContainer * container, ClutterActor * actor)
{
	TidyFramePrivate *priv = TIDY_FRAME(container)->priv;
...
static void tidy_frame_foreach(ClutterContainer * container, ClutterCallback callback, gpointer callback_data)
{
	TidyFramePrivate *priv = TIDY_FRAME(container)->priv;
...
static void tidy_frame_lower(ClutterContainer * container, ClutterActor * actor, ClutterActor * sibling)
{
	/* single child */
}

static void tidy_frame_raise(ClutterContainer * container, ClutterActor * actor, ClutterActor * sibling)
{
	/* single child */
}

static void tidy_frame_sort_depth_order(ClutterContainer * container)
{
	/* single child */
}
...
void tidy_frame_set_texture(TidyFrame * frame, ClutterActor * texture)
{
	...
	if (texture) {
		...
		if (G_UNLIKELY(parent != NULL)) {
			g_warning("Unable to set the background texture of type `%s' for "
				  ...
				  g_type_name(G_OBJECT_TYPE(frame)), g_type_name(G_OBJECT_TYPE(parent)));
			return;
		}
