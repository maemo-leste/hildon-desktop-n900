...
G_DEFINE_TYPE_WITH_CODE(TidyMemTexture, tidy_mem_texture, CLUTTER_TYPE_ACTOR, G_ADD_PRIVATE(TidyMemTexture));
...
static void tidy_mem_texture_free_data(TidyMemTexture * texture);
static void tidy_mem_texture_update_modified(TidyMemTexture * texture, TidyMemTextureTile * tile);
static void
tidy_mem_texture_tile_coords(TidyMemTexture * texture,
			     TidyMemTextureTile * tile,
			     ClutterFixed * x1, ClutterFixed * y1, ClutterFixed * x2, ClutterFixed * y2);
static gboolean
tidy_mem_texture_tile_visible(TidyMemTexture * texture, TidyMemTextureTile * tile, gint width, gint height);
/* ------------------------------------------------------------------------- */

static void tidy_mem_texture_paint(ClutterActor * self)
{
	...
	for (tiles = priv->tiles; tiles; tiles = tiles->next) {
		...
		if (tidy_mem_texture_tile_visible(texture, tile, width, height)) {
			...
			cogl_texture_rectangle(tile->texture, x1, y1, x2, y2, 0, 0, CFX_ONE, CFX_ONE);
		}
	}
}

static void tidy_mem_texture_dispose(GObject * object)
{
...
static void tidy_mem_texture_finalize(GObject * object)
{
	G_OBJECT_CLASS(tidy_mem_texture_parent_class)->finalize(object);
}

static void tidy_mem_texture_class_init(TidyMemTextureClass * klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
	ClutterActorClass *actor_class = CLUTTER_ACTOR_CLASS(klass);

	actor_class->paint = tidy_mem_texture_paint;

	gobject_class->finalize = tidy_mem_texture_finalize;
	gobject_class->dispose = tidy_mem_texture_dispose;
}

static void tidy_mem_texture_init(TidyMemTexture * self)
{
	TidyMemTexturePrivate *priv;
 ...
 */
TidyMemTexture *tidy_mem_texture_new()
{
	return g_object_new(TIDY_TYPE_MEM_TEXTURE, NULL);
}
			     ...
			     ClutterFixed * x1, ClutterFixed * y1, ClutterFixed * x2, ClutterFixed * y2)
{
	...
	*x1 = CFX_QMUL(priv->offset_x + CLUTTER_INT_TO_FIXED(tile->pos.x), priv->scale_x);
	*y1 = CFX_QMUL(priv->offset_y + CLUTTER_INT_TO_FIXED(tile->pos.y), priv->scale_y);
	*x2 = CFX_QMUL(priv->offset_x + CLUTTER_INT_TO_FIXED(tile->pos.x + tile->pos.width), priv->scale_x);
	*y2 = CFX_QMUL(priv->offset_y + CLUTTER_INT_TO_FIXED(tile->pos.y + tile->pos.height), priv->scale_y);
}
...
tidy_mem_texture_tile_visible(TidyMemTexture * texture, TidyMemTextureTile * tile, gint width, gint height)
{
	ClutterFixed x1, y1, x2, y2;
	tidy_mem_texture_tile_coords(texture, tile, &x1, &y1, &x2, &y2);
	return (x2 >= 0 && y2 >= 0 && x1 <= CLUTTER_INT_TO_FIXED(width) && y1 <= CLUTTER_INT_TO_FIXED(height));
}
...
static void tidy_mem_texture_update_modified(TidyMemTexture * texture, TidyMemTextureTile * tile)
{
...
#endif
	const guchar *ptr_src = &priv->texture_ptr[(tile->pos.x + tile->modified.x +
	...
	cogl_texture_set_region(tile->texture,
				...
				tile->modified.width, tile->modified.height,
				priv->texture_format, tile->modified.width * priv->texture_bpp, priv->tile_buffer);
#else
	cogl_texture_set_region(tile->texture,
				0, 0,
				tile->modified.x, tile->modified.y,
				tile->modified.width, tile->modified.height,
				tile->modified.width, tile->modified.height, priv->texture_format, rowstride, ptr_src);
#endif
	...
	if (priv->texture_ptr) {
		...
		for (y = 0; y < tiles_y; y++)
			for (x = 0; x < tiles_x; x++) {
				...
				/* alloc texture */
				tile->texture =
				    cogl_texture_new_with_size(tile->pos.width, tile->pos.height, -1 /* no waste */ ,
...
void tidy_mem_texture_damage(TidyMemTexture * texture, gint x, gint y, gint width, gint height)
{
	...
	for (tiles = priv->tiles; tiles; tiles = tiles->next) {
		...
		if (tile->pos.x <= x + width &&
		    tile->pos.y <= y + height &&
		    tile->pos.x + tile->pos.width >= x && tile->pos.y + tile->pos.height >= y) {
			...
			if (tidy_mem_texture_tile_visible(texture, tile, actor_width, actor_height))
				redraw = TRUE;
		}
	}
