...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE(TidyActor, tidy_actor, CLUTTER_TYPE_ACTOR, G_ADD_PRIVATE(TidyActor)
...
static void tidy_actor_set_property(GObject * gobject, guint prop_id, const GValue * value, GParamSpec * pspec)
{
	...
	case PROP_X_ALIGN:
		actor->priv->x_align = CLUTTER_FIXED_TO_FLOAT(g_value_get_double(value));
		break;

	case PROP_Y_ALIGN:
		actor->priv->y_align = CLUTTER_FIXED_TO_FLOAT(g_value_get_double(value));
		break;

	case PROP_STYLE:
		tidy_stylable_set_style(TIDY_STYLABLE(actor), g_value_get_object(value));
		break;
...
static void tidy_actor_get_property(GObject * gobject, guint prop_id, GValue * value, GParamSpec * pspec)
{
	TidyActor *actor = TIDY_ACTOR(gobject);
...
static void tidy_actor_dispose(GObject * gobject)
{
	TidyActor *actor = TIDY_ACTOR(gobject);
...
static void tidy_actor_class_init(TidyActorClass * klass)
{
   ...
   */
	g_object_class_install_property(gobject_class,
					...
					g_param_spec_boxed("padding",
							   ...
							   TIDY_TYPE_PADDING, TIDY_PARAM_READWRITE));
  /**
   * TidyActor:x-align:
   *
   ...
   */
	g_object_class_install_property(gobject_class,
					...
					g_param_spec_double("x-align",
							    ...
							    0.0, 1.0, 0.5, TIDY_PARAM_READWRITE));
  /**
   * TidyActor:y-align:
   *
...
static TidyStyle *tidy_actor_get_style(TidyStylable * stylable)
{
...
static void tidy_actor_set_style(TidyStylable * stylable, TidyStyle * style)
{
	TidyActorPrivate *priv = TIDY_ACTOR(stylable)->priv;
...
static void tidy_stylable_iface_init(TidyStylableIface * iface)
{
	...
	if (!is_initialized) {
		...
		pspec = g_param_spec_string("font-name",
					    "Font Name",
					    "The font to use for displaying text", "Sans 12px", G_PARAM_READWRITE);
		tidy_stylable_iface_install_property(iface, TIDY_TYPE_ACTOR, pspec);

		pspec = g_param_spec_boxed("bg-color",
					   "Background Color",
					   "The background color of an actor", CLUTTER_TYPE_COLOR, G_PARAM_READWRITE);
		tidy_stylable_iface_install_property(iface, TIDY_TYPE_ACTOR, pspec);

		pspec = g_param_spec_boxed("active-color",
					   "Active Color",
					   "The color of an active actor", CLUTTER_TYPE_COLOR, G_PARAM_READWRITE);
		tidy_stylable_iface_install_property(iface, TIDY_TYPE_ACTOR, pspec);

		pspec = g_param_spec_boxed("text-color",
					   "Text Color",
					   "The color of the text of an actor", CLUTTER_TYPE_COLOR, G_PARAM_READWRITE);
		tidy_stylable_iface_install_property(iface, TIDY_TYPE_ACTOR, pspec);
...
static void tidy_actor_init(TidyActor * actor)
{
...
static TidyPadding *tidy_padding_copy(const TidyPadding * padding)
{
	TidyPadding *copy;
...
static void tidy_padding_free(TidyPadding * padding)
{
	if (G_LIKELY(padding))
		g_slice_free(TidyPadding, padding);
}

GType tidy_padding_get_type(void)
{
