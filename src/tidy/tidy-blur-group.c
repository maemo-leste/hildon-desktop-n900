...
G_DEFINE_TYPE_WITH_CODE(TidyBlurGroup, tidy_blur_group, CLUTTER_TYPE_GROUP, G_ADD_PRIVATE(TidyBlurGroup));
...
static
gboolean tidy_blur_group_notify_modified_real(ClutterActor * actor, ClutterActor * child)
{
...
static void tidy_blur_group_allocate_textures(TidyBlurGroup * self)
{
	...
	priv->tex_b = cogl_texture_new_with_size(tex_width, tex_height, 0, FALSE /* mipmap */ ,
						 priv->use_alpha ? COGL_PIXEL_FORMAT_RGBA_8888 :
						 COGL_PIXEL_FORMAT_RGB_565);
...
static gboolean tidy_blur_group_children_visible(ClutterGroup * group)
{
	gint i;
	ClutterActor *actor;
...
static void tidy_blur_group_fallback_blur(TidyBlurGroup * group, int tex_width, int tex_height)
{
	...
	cogl_texture_rectangle(tex,
			       ...
			       CLUTTER_INT_TO_FIXED(tex_height), -diffx, 0, CFX_ONE - diffx, CFX_ONE);
	cogl_blend_func(CGL_ONE, CGL_ONE);
	cogl_texture_rectangle(tex,
			       0, 0,
			       CLUTTER_INT_TO_FIXED(tex_width),
			       CLUTTER_INT_TO_FIXED(tex_height), 0, diffy, CFX_ONE + diffx, CFX_ONE);
	cogl_texture_rectangle(tex,
			       0, 0,
			       CLUTTER_INT_TO_FIXED(tex_width),
			       CLUTTER_INT_TO_FIXED(tex_height), 0, -diffy, CFX_ONE, CFX_ONE - diffy);
	cogl_texture_rectangle(tex,
			       0, 0,
			       CLUTTER_INT_TO_FIXED(tex_width),
			       CLUTTER_INT_TO_FIXED(tex_height), 0, diffy, CFX_ONE, CFX_ONE + diffy);
...
static void tidy_blur_group_do_chequer(TidyBlurGroup * group, guint width, guint height)
{
	...
	cogl_texture_rectangle(priv->tex_chequer,
			       ...
			       0, 0, CFX_ONE * width / CHEQUER_SIZE, CFX_ONE * height / CHEQUER_SIZE);
}
...
static void tidy_blur_group_paint(ClutterActor * actor)
{
	...
	if (!tidy_blur_group_source_buffered(actor) || !tidy_blur_group_children_visible(group)) {
...
#ifdef __i386__
	if (!cogl_features_available(COGL_FEATURE_OFFSCREEN)) {	/* If we can't blur properly do something nicer instead :) */
		/* Otherwise crash... */
		CLUTTER_ACTOR_CLASS(tidy_blur_group_parent_class)->paint(actor);
	...
	while (priv->current_blur_step < priv->blur_step && steps_this_frame < MAX_STEPS_PER_FRAME) {
		/* blur one texture into the other */
		tidy_util_cogl_push_offscreen_buffer(priv->current_is_a ? priv->fbo_b : priv->fbo_a);
		...
		if (priv->use_shader && priv->shader_blur) {
			if (priv->tweaks_blurless) {
				...
				clutter_shader_set_uniform_1f(priv->shader_blur, "blurx", 1.0f / tex_width);
				clutter_shader_set_uniform_1f(priv->shader_blur, "blury", 1.0f / tex_height);
			}
		}
		...
		if (priv->use_shader) {
			...
			cogl_texture_rectangle(priv->current_is_a ? priv->tex_a : priv->tex_b,
					       0, 0,
					       CLUTTER_INT_TO_FIXED(tex_width),
					       CLUTTER_INT_TO_FIXED(tex_height), 0, 0, CFX_ONE, CFX_ONE);
			cogl_blend_func(CGL_SRC_ALPHA, CGL_ONE_MINUS_SRC_ALPHA);
		} else
 ...
 skip_progress:
	...
	if (priv->blur_step == 0 || (priv->blur_step < priv->max_blur_step)) {
		...
		cogl_push_matrix();
		cogl_translatex(CLUTTER_FLOAT_TO_FIXED(width * (1 - priv->zoom) / 2),
				CLUTTER_FLOAT_TO_FIXED(height * (1 - priv->zoom) / 2), 0);
		cogl_scale(CLUTTER_FLOAT_TO_FIXED(priv->zoom), CLUTTER_FLOAT_TO_FIXED(priv->zoom));

		cogl_clip_set(0, 0, CLUTTER_INT_TO_FIXED(width), CLUTTER_INT_TO_FIXED(height));
		CLUTTER_ACTOR_CLASS(tidy_blur_group_parent_class)->paint(actor);
		cogl_clip_unset();
			...
			for (y = 0; y < 3; y++)
				for (x = 0; x < 3; x++)
					if (x != 1 || y != 1) {
						...
						gint sy = (y == 1) ? 1 : -1;
						cogl_push_matrix();
						cogl_translatex(CLUTTER_FLOAT_TO_FIXED(width * (x - 1) + width / 2),
								CLUTTER_FLOAT_TO_FIXED(height * (y - 1) + height / 2),
								0);
						cogl_scale(sx * CFX_ONE, sy * CFX_ONE);
						cogl_translatex(CLUTTER_FLOAT_TO_FIXED(-width / 2),
								CLUTTER_FLOAT_TO_FIXED(-height / 2), 0);
						cogl_clip_set(0, 0,
							      CLUTTER_INT_TO_FIXED(width),
							      CLUTTER_INT_TO_FIXED(height));
						CLUTTER_ACTOR_CLASS(tidy_blur_group_parent_class)->paint(actor);
	...
	if (priv->use_shader && priv->shader_saturate) {
		...
		if (priv->tweaks_blurless) {
			clutter_shader_set_uniform_1f(priv->shader_saturate, "saturation", priv->blurless_saturation);
		} else {
			clutter_shader_set_uniform_1f(priv->shader_saturate, "saturation", priv->saturation);
		}
	}
	...
	if ((priv->zoom >= 1) || !priv->use_mirror) {
		cogl_texture_rectangle(current_tex, mx - zx, my - zy, mx + zx, my + zy, 0, 0, CFX_ONE, CFX_ONE);
	} else {
		gint vignette_amt;
		float edge_expand;
		...
		for (y = 0; y <= VIGNETTE_TILES; y++)
			for (x = 0; x <= VIGNETTE_TILES; x++) {
				...
				if (x == 0)
					fx = VIGNETTE_TILES - edge_expand;
				if (x == VIGNETTE_TILES)
					fx = edge_expand;
				if (y == 0)
					fy = VIGNETTE_TILES - edge_expand;
				if (y == VIGNETTE_TILES)
					fy = edge_expand;
		...
		/* render! */
		cogl_texture_triangles(current_tex, 6 * (VIGNETTE_TILES * VIGNETTE_TILES), verts, TRUE);
	}
...
static void tidy_blur_group_dispose(GObject * gobject)
{
...
static void tidy_blur_group_class_init(TidyBlurGroupClass * klass)
{
...
static void tidy_blur_group_init(TidyBlurGroup * self)
{
	...
	priv->tex_chequer = cogl_texture_new_from_data(CHEQUER_SIZE,
						       CHEQUER_SIZE,
						       0,
						       0,
						       COGL_PIXEL_FORMAT_A_8,
						       COGL_PIXEL_FORMAT_A_8, CHEQUER_SIZE, dither_data);

	if (priv->tweaks_blurless) {
		tidy_blur_group_check_shader(self, &priv->shader_blur,
					     BLUR_FRAGMENT_SHADER_BLURLESS, BLUR_VERTEX_SHADER);
	} else {
		tidy_blur_group_check_shader(self, &priv->shader_blur, BLUR_FRAGMENT_SHADER, BLUR_VERTEX_SHADER);
	}

	tidy_blur_group_check_shader(self, &priv->shader_saturate, SATURATE_FRAGMENT_SHADER, 0);

	g_signal_connect(self, "notify::allocation", G_CALLBACK(tidy_blur_group_allocate_textures), NULL);
}
