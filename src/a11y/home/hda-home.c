...
/* GObject */
static void hda_home_class_init(HdaHomeClass * klass);

static void hda_home_init(HdaHome * root);

/* AtkObject.h */

static void hda_home_initialize(AtkObject * obj, gpointer data);
/* AtkAction -> new actions */
static ClutterActor *_hda_home_get_home_container(CailActor * cail_actor);

static void _hda_home_next_home_action(CailActor * cail_actor);

static void _hda_home_previous_home_action(CailActor * cail_actor);
...
static void hda_home_init(HdaHome * home)
{
	CailActor *cail_actor = NULL;

	/* Adding extra actions */
	cail_actor = CAIL_ACTOR(home);
	cail_actor_add_action(cail_actor, "next-home", NULL, NULL, _hda_home_next_home_action);

	cail_actor_add_action(cail_actor, "previous-home", NULL, NULL, _hda_home_previous_home_action);
}
...
static ClutterActor *_hda_home_get_home_container(CailActor * cail_actor)
{
	...
	container = clutter_container_find_child_by_name(CLUTTER_CONTAINER(stage), "HdHome:view_container");
...
static void _hda_home_next_home_action(CailActor * cail_actor)
{
	...
	if (container != NULL) {
		hd_home_view_container_scroll_to_next(HD_HOME_VIEW_CONTAINER(container), 0);
	}
}
...
static void _hda_home_previous_home_action(CailActor * cail_actor)
{
	...
	if (container != NULL) {
		hd_home_view_container_scroll_to_previous(HD_HOME_VIEW_CONTAINER(container), 0);
	}
}
